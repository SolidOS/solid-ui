...
var i = "image/png", a = class extends e {
  ...
  `;
	...
	_renderViewport() {
		return this._previewUrl ? n`<img class="capture-preview" part="preview-image" src="${this._previewUrl}" alt="Captured photo preview" />` : n`<video class="capture-preview" part="preview-video" autoplay playsinline muted></video>`;
	}
	_renderStatus() {
		return this._errorMessage ? n`<div class="status error" part="error-message">${this._errorMessage}</div>` : this._startingPreview ? n`<div class="status" part="status-message">Opening camera…</div>` : this.value ? n`<div class="status" part="status-message">Review the photo before confirming it.</div>` : n`<div class="status" part="status-message">Preview the camera and take a photo when ready.</div>`;
	}
	_renderActions() {
		return n`
      <div class="actions" part="actions">
        ${this.showCancelButton ? n`
            <button
              class="cancel-button"
              part="cancel-button"
              type="button"
	...
	render() {
		return n`
      ${this.showTrigger || this.presentation === "dialog" ? n`
          <button
