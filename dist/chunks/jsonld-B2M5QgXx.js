import { z as e } from "./index.esm-Bu7WXI60.js";
...
var t = /* @__PURE__ */ e(((e, t) => {
	...
	n(), e.setImmediate = setImmediate, e.crypto = globalThis.crypto;
	var t = [];
	...
	var { bufferToHex: n, crypto: i } = r(), a = /* @__PURE__ */ new Map([
		["sha256", "SHA-256"],
		["SHA256", "SHA-256"],
		["SHA-256", "SHA-256"],
		["sha384", "SHA-384"],
	...
	t.exports = class {
		constructor(e) {
			if (!(i && i.subtle)) throw Error("crypto.subtle not found.");
			if (!a.has(e)) throw Error(`Unsupported algorithm "${e}".`);
			this.algorithm = a.get(e), this._content = "";
		}
		update(e) {
			this._content += e;
		}
		async digest() {
			...
			return n(await i.subtle.digest(this.algorithm, e));
		}
	};
})), a = /* @__PURE__ */ e(((e, t) => {
	t.exports = class {
		...
		static parse(e) {
			...
			for (let e of u) {
				...
				if (f.subject = u[1] === void 0 ? {
					...
					termType: i,
					value: v(u[4])
				}, f.graph = u[9] === void 0 ? u[10] === void 0 ? {
	...
	var s = t(), c = i(), l = a(), u = o(), { setImmediate: d } = r();
	n.exports = class {
		constructor({ createMessageDigest: e = null, messageDigestAlgorithm: t = "sha256", canonicalIdMap: n = /* @__PURE__ */ new Map(), maxWorkFactor: r = 1, maxDeepIterations: i = -1, signal: a = null } = {}) {
			this.name = "RDFC-1.0", this.blankNodeInfo = /* @__PURE__ */ new Map(), this.canonicalIssuer = new s("c14n", n), this.createMessageDigest = e || (() => new c(t)), this.maxWorkFactor = r, this.maxDeepIterations = i, this.remainingDeepIterations = 0, this.signal = a, this.quads = null;
		}
		async main(e) {
			this.quads = e;
			...
			let t = /* @__PURE__ */ new Map(), n = [...this.blankNodeInfo.keys()], r = 0;
			for (let e of n) ++r % 100 == 0 && await this._yield(), await this._hashAndTrackBlankNode({
				id: e,
				hashToBlankNodes: t
			});
			let i = [...t.keys()].sort(), a = [];
			...
			if (this.maxDeepIterations < 0) {
				if (this.maxWorkFactor === 0) this.maxDeepIterations = 0;
			...
			for (let e of a) {
				let t = [];
				for (let n of e) {
					if (this.canonicalIssuer.hasId(n)) continue;
					let e = new s("b");
					e.getId(n);
					let r = await this.hashNDegreeQuads(n, e);
					t.push(r);
				}
				t.sort(f);
				for (let e of t) {
			...
			for (let e of this.quads) {
				let t = u.serializeQuadComponents(this._componentWithCanonicalId(e.subject), e.predicate, this._componentWithCanonicalId(e.object), this._componentWithCanonicalId(e.graph));
				o.push(t);
			}
		...
		async hashFirstDegreeQuads(e) {
			...
			for (let n of r) t.push(u.serializeQuadComponents(this.modifyFirstDegreeComponent(e, n.subject, "subject"), n.predicate, this.modifyFirstDegreeComponent(e, n.object, "object"), this.modifyFirstDegreeComponent(e, n.graph, "graph")));
			t.sort();
		...
		async hashNDegreeQuads(e, t) {
			...
			let n = this.createMessageDigest(), r = await this.createHashToRelated(e, t), i = [...r.keys()].sort();
			for (let e of i) {
				n.update(e);
				let i = "", a, o = new l(r.get(e)), s = 0;
				for (; o.hasNext();) {
					let e = o.next();
					if (++s % 3 == 0) {
					...
					let n = t.clone(), r = "", c = [], l = !1;
					for (let t of e) if (this.canonicalIssuer.hasId(t) ? r += "_:" + this.canonicalIssuer.getId(t) : (n.hasId(t) || c.push(t), r += "_:" + n.getId(t)), i.length !== 0 && r > i) {
						l = !0;
						break;
					}
					if (!l) {
						for (let e of c) {
							...
							if (r += "_:" + n.getId(e), r += `<${t.hash}>`, n = t.issuer, i.length !== 0 && r > i) {
								l = !0;
								break;
							}
						}
						l || (i.length === 0 || r < i) && (i = r, a = n);
					}
				}
				n.update(i), t = a;
			}
	...
	var r = t(), s = i(), c = a(), l = o();
	n.exports = class {
		constructor({ createMessageDigest: e = null, messageDigestAlgorithm: t = "sha256", canonicalIdMap: n = /* @__PURE__ */ new Map(), maxWorkFactor: i = 1, maxDeepIterations: a = -1, timeout: o = 0 } = {}) {
			this.name = "RDFC-1.0", this.blankNodeInfo = /* @__PURE__ */ new Map(), this.canonicalIssuer = new r("c14n", n), this.createMessageDigest = e || (() => new s(t)), this.maxWorkFactor = i, this.maxDeepIterations = a, this.remainingDeepIterations = 0, this.timeout = o, o > 0 && (this.startTime = Date.now()), this.quads = null;
		}
		main(e) {
			this.quads = e;
			for (let t of e) this._addBlankNodeQuadInfo({
			...
			let t = /* @__PURE__ */ new Map(), n = [...this.blankNodeInfo.keys()];
			for (let e of n) this._hashAndTrackBlankNode({
				id: e,
				hashToBlankNodes: t
			});
			let i = [...t.keys()].sort(), a = [];
			...
			for (let e of a) {
				...
				for (let n of e) {
					...
					let e = new r("b");
					e.getId(n);
					let i = this.hashNDegreeQuads(n, e);
					t.push(i);
				}
				t.sort(u);
				for (let e of t) {
			...
			let o = [];
			for (let e of this.quads) {
				let t = l.serializeQuadComponents(this._componentWithCanonicalId(e.subject), e.predicate, this._componentWithCanonicalId(e.object), this._componentWithCanonicalId(e.graph));
				o.push(t);
			}
			return o.sort(), o.join("");
		}
		hashFirstDegreeQuads(e) {
			let t = [], n = this.blankNodeInfo.get(e), r = n.quads;
			for (let n of r) t.push(l.serializeQuadComponents(this.modifyFirstDegreeComponent(e, n.subject, "subject"), n.predicate, this.modifyFirstDegreeComponent(e, n.object, "object"), this.modifyFirstDegreeComponent(e, n.graph, "graph")));
			t.sort();
			let i = this.createMessageDigest();
			for (let e of t) i.update(e);
		...
		hashNDegreeQuads(e, t) {
			if (this.remainingDeepIterations === 0) throw Error(`Maximum deep iterations exceeded (${this.maxDeepIterations}).`);
			this.remainingDeepIterations--;
			let n = this.createMessageDigest(), r = this.createHashToRelated(e, t), i = [...r.keys()].sort();
			for (let e of i) {
				n.update(e);
				let i = "", a, o = new c(r.get(e)), s = 0;
				for (; o.hasNext();) {
					let e = o.next();
					if (++s % 3 == 0 && this.timeout > 0 && Date.now() - this.startTime > this.timeout) throw Error("Canonize timeout.");
					let n = t.clone(), r = "", c = [], l = !1;
					for (let t of e) if (this.canonicalIssuer.hasId(t) ? r += "_:" + this.canonicalIssuer.getId(t) : (n.hasId(t) || c.push(t), r += "_:" + n.getId(t)), i.length !== 0 && r > i) {
						l = !0;
						break;
	...
	e.NQuads = o(), e.IdentifierIssuer = t(), e.canonize = async function(e, t = {}) {
		let r = i(e, t);
		if (a(t), !("algorithm" in t)) throw Error("No RDF Dataset Canonicalization algorithm specified.");
		if (t.algorithm === "RDFC-1.0") return new n(t).main(r);
		if (t.algorithm === "URDNA2015" && !t.rejectURDNA2015) return l(), new n(t).main(r);
		throw Error("Invalid RDF Dataset Canonicalization algorithm: " + t.algorithm);
	}, e._canonizeSync = function(e, t = {}) {
		let n = i(e, t);
		if (a(t), !("algorithm" in t)) throw Error("No RDF Dataset Canonicalization algorithm specified.");
		if (t.algorithm === "RDFC-1.0") return new r(t).main(n);
		if (t.algorithm === "URDNA2015" && !t.rejectURDNA2015) return l(), new r(t).main(n);
		throw Error("Invalid RDF Dataset Canonicalization algorithm: " + t.algorithm);
	};
})), u = /* @__PURE__ */ e(((e, t) => {
	t.exports = l();
	...
	var n = f(), r = d(), i = u().IdentifierIssuer, a = p(), o = /^[a-zA-Z]{1,8}(-[a-zA-Z0-9]{1,8})*$/, s = /(?:<[^>]*?>|"[^"]*?"|[^,])+/g, c = /\s*<([^>]*?)>\s*(?:;\s*(.*))?/, l = /(.*?)=(?:(?:"([^"]*?)")|([^"]*?))\s*(?:(?:;\s*)|$)/g, m = /^@[a-zA-Z]+$/, h = { headers: { accept: "application/ld+json, application/json" } }, g = {};
	t.exports = g, g.IdentifierIssuer = i, g.REGEX_BCP47 = o, g.REGEX_KEYWORD = m, g.clone = function(e) {
		if (e && typeof e == "object") {
			let t;
			if (r.isArray(e)) {
				t = [];
				for (let n = 0; n < e.length; ++n) t[n] = g.clone(e[n]);
			} else if (e instanceof Map) {
				t = /* @__PURE__ */ new Map();
				for (let [n, r] of e) t.set(n, g.clone(r));
			} else if (e instanceof Set) {
				t = /* @__PURE__ */ new Set();
				for (let n of e) t.add(g.clone(n));
			} else if (r.isObject(e)) {
				t = {};
				for (let n in e) t[n] = g.clone(e[n]);
			} else t = e.toString();
			return t;
		}
		return e;
	}, g.asArray = function(e) {
		return Array.isArray(e) ? e : [e];
	}, g.buildHeaders = (e = {}) => {
		if (Object.keys(e).some((e) => e.toLowerCase() === "accept")) throw RangeError("Accept header may not be specified; only \"" + h.headers.accept + "\" is supported.");
		return Object.assign({ Accept: h.headers.accept }, e);
	}, g.parseLinkHeader = (e) => {
		let t = {}, n = e.match(s);
		for (let e = 0; e < n.length; ++e) {
			...
			let o = i.rel || "";
			Array.isArray(t[o]) ? t[o].push(i) : t[o] = t.hasOwnProperty(o) ? [t[o], i] : i;
		}
		return t;
	}, g.validateTypeValue = (e, t) => {
		if (!r.isString(e) && !(r.isArray(e) && e.every((e) => r.isString(e)))) {
			if (t && r.isObject(e)) switch (Object.keys(e).length) {
				case 0: return;
				case 1: if ("@default" in e && g.asArray(e["@default"]).every((e) => r.isString(e))) return;
			}
		...
		if (g.hasProperty(e, t)) {
			let a = e[t], o = n.isList(a);
			if (r.isArray(a) || o) {
				o && (a = a["@list"]);
				for (let e = 0; e < a.length; ++e) if (g.compareValues(i, a[e])) return !0;
			} else if (!r.isArray(i)) return g.compareValues(i, a);
		}
		return !1;
	}, g.addValue = (e, t, n, i) => {
		if (i ||= {}, "propertyIsArray" in i || (i.propertyIsArray = !1), "valueIsArray" in i || (i.valueIsArray = !1), "allowDuplicate" in i || (i.allowDuplicate = !0), "prependValue" in i || (i.prependValue = !1), i.valueIsArray) e[t] = n;
			...
			for (let r = 0; r < n.length; ++r) g.addValue(e, t, n[r], i);
		} else if (e.hasOwnProperty(t)) {
			let a = !i.allowDuplicate && g.hasValue(e, t, n);
			!r.isArray(e[t]) && (!a || i.propertyIsArray) && (e[t] = [e[t]]), a || (i.prependValue ? e[t].unshift(n) : e[t].push(n));
		} else e[t] = i.propertyIsArray ? [n] : n;
	}, g.getValues = (e, t) => [].concat(e[t] || []), g.removeProperty = (e, t) => {
		delete e[t];
	}, g.removeValue = (e, t, n, r) => {
		r ||= {}, "propertyIsArray" in r || (r.propertyIsArray = !1);
		let i = g.getValues(e, t).filter((e) => !g.compareValues(e, n));
		i.length === 0 ? g.removeProperty(e, t) : e[t] = i.length === 1 && !r.propertyIsArray ? i[0] : i;
	}, g.relabelBlankNodes = (e, t) => (t ||= {}, _(t.issuer || new i("_:b"), e)), g.compareValues = (e, t) => e === t || n.isValue(e) && n.isValue(t) && e["@value"] === t["@value"] && e["@type"] === t["@type"] && e["@language"] === t["@language"] && e["@index"] === t["@index"] ? !0 : r.isObject(e) && "@id" in e && r.isObject(t) && "@id" in t ? e["@id"] === t["@id"] : !1, g.compareShortestLeast = (e, t) => e.length < t.length ? -1 : t.length < e.length ? 1 : e === t ? 0 : e < t ? -1 : 1;
	function _(e, t) {
		if (r.isArray(t)) for (let n = 0; n < t.length; ++n) t[n] = _(e, t[n]);
		else if (n.isList(t)) t["@list"] = _(e, t["@list"]);
	...
	var n = "http://www.w3.org/1999/02/22-rdf-syntax-ns#", r = "http://www.w3.org/2001/XMLSchema#";
	t.exports = {
		...
		RDF_NIL: n + "nil",
		RDF_TYPE: n + "type",
		RDF_PLAIN_LITERAL: n + "PlainLiteral",
		RDF_XML_LITERAL: n + "XMLLiteral",
		RDF_JSON_LITERAL: n + "JSON",
		RDF_OBJECT: n + "object",
		RDF_LANGSTRING: n + "langString",
		XSD: r,
		XSD_BOOLEAN: r + "boolean",
		XSD_DOUBLE: r + "double",
		XSD_INTEGER: r + "integer",
		XSD_STRING: r + "string"
	};
})), g = /* @__PURE__ */ e(((e, t) => {
	t.exports = class {
	...
	t.exports = r, r.parsers = {
		...
		else if (a.authority = e.authority, i.path === "") a.path = e.path, a.query = i.query === null ? e.query : i.query;
		else {
	...
	var { parseLinkHeader: n, buildHeaders: r } = m(), { LINK_HEADER_CONTEXT: i } = h(), a = p(), o = g(), { prependBase: s } = _(), c = /(^|(\r\n))link:/i;
	...
	var n = x(), r = Symbol("max"), i = Symbol("length"), a = Symbol("lengthCalculator"), o = Symbol("allowStale"), s = Symbol("maxAge"), c = Symbol("dispose"), l = Symbol("noDisposeOnSet"), u = Symbol("lruList"), d = Symbol("cache"), f = Symbol("updateAgeOnGet"), p = () => 1, m = class {
		constructor(e) {
	...
	var { isArray: n, isObject: r, isString: i } = d(), { asArray: a } = m(), { prependBase: o } = _(), s = p(), c = C(), l = 10;
	t.exports = class {
	...
	var n = p(), { isArray: r } = d(), { asArray: i } = m(), a = {};
	...
	var n = m(), r = p(), { isArray: i, isObject: a, isString: o, isUndefined: s } = d(), { isAbsolute: c, isRelative: l, prependBase: u } = _(), { handleEvent: f } = E(), { REGEX_BCP47: h, REGEX_KEYWORD: g, asArray: v, compareShortestLeast: y } = m(), b = /* @__PURE__ */ new Map(), x = 1e4, S = {};
	t.exports = S, S.process = async ({ activeCtx: e, localCtx: t, options: n, propagate: s = !0, overrideProtected: d = !1, cycles: p = /* @__PURE__ */ new Set() }) => {
		if (a(t) && "@context" in t && i(t["@context"]) && (t = t["@context"]), v(t).length === 0) return e;
		let m = [], g = [({ event: e, next: t }) => {
			m.push(e), t();
		}];
		n.eventHandler && g.push(n.eventHandler);
		let _ = n;
		n = {
			...n,
			eventHandler: g
		};
		let y = await n.contextResolver.resolve({
			activeCtx: e,
			context: t,
			documentLoader: n.documentLoader,
			base: n.base
		});
		a(y[0].document) && typeof y[0].document["@propagate"] == "boolean" && (s = y[0].document["@propagate"]);
		let b = e;
		!s && !b.previousContext && (b = b.clone(), b.previousContext = e);
		for (let i of y) {
			let { document: s } = i;
			if (e = b, s === null) {
				if (!d && Object.keys(e.protected).length !== 0) throw new r("Tried to nullify a context with protected terms outside of a term definition.", "jsonld.SyntaxError", { code: "invalid context nullification" });
				b = e = S.getInitialContext(n).clone();
				continue;
			}
			let g = i.getProcessed(e);
			if (g) {
				if (_.eventHandler) for (let e of g.events) f({
					event: e,
					options: _
				});
				b = e = g.context;
			...
			if ("@version" in s) {
				...
				b.processingMode = "json-ld-1.1", b["@version"] = s["@version"], v.set("@version", !0);
			}
			if (b.processingMode = b.processingMode || e.processingMode, "@base" in s) {
				let e = s["@base"];
				if (!(e === null || c(e))) {
					if (l(e)) e = u(b["@base"], e);
				...
				b["@base"] = e, v.set("@base", !0);
			}
			if ("@vocab" in s) {
				let e = s["@vocab"];
				if (e === null) delete b["@vocab"];
				...
				else if (!c(e) && S.processingMode(b, 1)) throw new r("Invalid JSON-LD syntax; the value of \"@vocab\" in a @context must be an absolute IRI.", "jsonld.SyntaxError", {
					code: "invalid vocab mapping",
					context: s
				});
				else {
			...
			if ("@language" in s) {
				let e = s["@language"];
				if (e === null) delete b["@language"];
				else if (o(e)) e.match(h) || n.eventHandler && f({
					event: {
						type: ["JsonLdEvent"],
			...
			v.set("@protected", s["@protected"] || !1);
			for (let e in s) if (S.createTermDefinition({
				activeCtx: b,
				localCtx: s,
				term: e,
				defined: v,
		...
		return b;
	}, S.createTermDefinition = ({ activeCtx: e, localCtx: t, term: n, defined: s, options: l, overrideProtected: u = !1 }) => {
		if (s.has(n)) {
			if (s.get(n)) return;
		...
		s.set(n, !1);
		let d;
		if (t.hasOwnProperty(n) && (d = t[n]), n === "@type" && a(d) && (d["@container"] || "@set") === "@set" && S.processingMode(e, 1.1)) {
			let e = [
				"@container",
				"@id",
				"@protected"
		...
		} else if (S.isKeyword(n)) throw new r("Invalid JSON-LD syntax; keywords cannot be overridden.", "jsonld.SyntaxError", {
			code: "keyword redefinition",
			context: t,
			term: n
		});
		...
		let p = e.mappings.get(n);
		e.mappings.has(n) && e.mappings.delete(n);
		let m = !1;
		if ((o(d) || d === null) && (m = !0, d = { "@id": d }), !a(d)) throw new r("Invalid JSON-LD syntax; @context term values must be strings or objects.", "jsonld.SyntaxError", {
			code: "invalid term definition",
			context: t
		});
		let h = {};
		...
		S.processingMode(e, 1.1) && _.push("@context", "@direction", "@index", "@nest", "@prefix", "@protected");
		for (let e in d) if (!_.includes(e)) throw new r("Invalid JSON-LD syntax; a term definition must not contain " + e, "jsonld.SyntaxError", {
			code: "invalid term definition",
			context: t
		});
		let v = n.indexOf(":");
		if (h._termHasColon = v > 0, "@reverse" in d) {
			if ("@id" in d) throw new r("Invalid JSON-LD syntax; a @reverse term definition must not contain @id.", "jsonld.SyntaxError", {
				code: "invalid reverse property",
				context: t
			});
			if ("@nest" in d) throw new r("Invalid JSON-LD syntax; a @reverse term definition must not contain @nest.", "jsonld.SyntaxError", {
				code: "invalid reverse property",
				context: t
			});
			let i = d["@reverse"];
			...
			if (i.match(g)) {
				l.eventHandler && f({
					...
					options: l
				}), p ? e.mappings.set(n, p) : e.mappings.delete(n);
				return;
			}
			let a = C(e, i, {
				vocab: !0,
			...
			if (i === null) h["@id"] = null;
			else if (!S.isKeyword(i) && i.match(g)) {
				l.eventHandler && f({
					event: {
						type: ["JsonLdEvent"],
				...
				if (i = C(e, i, {
					vocab: !0,
					base: !1
				}, t, s, l), !c(i) && !S.isKeyword(i)) throw new r("Invalid JSON-LD syntax; a @context @id value must be an absolute IRI, a blank node identifier, or a keyword.", "jsonld.SyntaxError", {
					code: "invalid IRI mapping",
					context: t
				});
				if (n.match(/(?::[^:])|\//) && C(e, n, {
				...
				h["@id"] = i, h._prefix = m && !h._termHasColon && i.match(/[:\/\?#\[\]@]$/) !== null;
			}
		}
		if (!("@id" in h)) {
			if (h._termHasColon) {
				let r = n.substr(0, v);
				if (t.hasOwnProperty(r) && S.createTermDefinition({
					activeCtx: e,
					localCtx: t,
					term: r,
					defined: s,
					...
					let t = n.substr(v + 1);
					h["@id"] = e.mappings.get(r)["@id"] + t;
				} else h["@id"] = n;
			} else if (n === "@type") h["@id"] = n;
			else {
		...
		if ((d["@protected"] === !0 || s.get("@protected") === !0 && d["@protected"] !== !1) && (e.protected[n] = !0, h.protected = !0), s.set(n, !0), "@type" in d) {
			let n = d["@type"];
			...
			if (n === "@json" || n === "@none") {
				if (S.processingMode(e, 1)) throw new r(`Invalid JSON-LD syntax; an @context @type value must not be "${n}" in JSON-LD 1.0 mode.`, "jsonld.SyntaxError", {
					code: "invalid type mapping",
					context: t
				});
		...
		if ("@container" in d) {
			let n = o(d["@container"]) ? [d["@container"]] : d["@container"] || [], a = [
				"@list",
				"@set",
				"@index",
				"@language"
			], s = !0, c = n.includes("@set");
			if (S.processingMode(e, 1.1)) {
				...
				if (n.includes("@type") && (h["@type"] = h["@type"] || "@id", !["@id", "@vocab"].includes(h["@type"]))) throw new r("Invalid JSON-LD syntax; container: @type requires @type to be @id or @vocab.", "jsonld.SyntaxError", {
					code: "invalid type mapping",
					context: t
				});
			} else s &= !i(d["@container"]), s &= n.length <= 1;
			...
			if (h.reverse && !n.every((e) => ["@index", "@set"].includes(e))) throw new r("Invalid JSON-LD syntax; @context @container value for a @reverse type definition must be @index or @set.", "jsonld.SyntaxError", {
				code: "invalid reverse property",
				context: t
			});
			h["@container"] = n;
		...
		if ("@index" in d) {
			if (!("@container" in d) || !h["@container"].includes("@index")) throw new r(`Invalid JSON-LD syntax; @index without @index in @container: "${d["@index"]}" on term "${n}".`, "jsonld.SyntaxError", {
				code: "invalid term definition",
				context: t
			});
			if (!o(d["@index"]) || d["@index"].indexOf("@") === 0) throw new r(`Invalid JSON-LD syntax; @index must expand to an IRI: "${d["@index"]}" on term "${n}".`, "jsonld.SyntaxError", {
				code: "invalid term definition",
				context: t
			});
			h["@index"] = d["@index"];
		}
		if ("@context" in d && (h["@context"] = d["@context"]), "@language" in d && !("@type" in d)) {
			let e = d["@language"];
			if (e !== null && !o(e)) throw new r("Invalid JSON-LD syntax; @context @language value must be a string or null.", "jsonld.SyntaxError", {
				code: "invalid language mapping",
				context: t
			});
			e !== null && (e = e.toLowerCase()), h["@language"] = e;
		}
		if ("@prefix" in d) {
			...
			if (S.isKeyword(h["@id"])) throw new r("Invalid JSON-LD syntax; keywords may not be used as prefixes", "jsonld.SyntaxError", {
				code: "invalid term definition",
				context: t
			});
			if (typeof d["@prefix"] == "boolean") h._prefix = d["@prefix"] === !0;
		...
		let y = h["@id"];
		if (y === "@context" || y === "@preserve") throw new r("Invalid JSON-LD syntax; @context and @preserve cannot be aliased.", "jsonld.SyntaxError", {
			code: "invalid keyword alias",
			context: t
		});
		if (p && p.protected && !u && (e.protected[n] = !0, h.protected = !0, !w(p, h))) throw new r("Invalid JSON-LD syntax; tried to redefine a protected term.", "jsonld.SyntaxError", {
			code: "protected term redefinition",
			context: t,
			term: n
		});
	}, S.expandIri = (e, t, n, r) => C(e, t, n, void 0, void 0, r);
	function C(e, t, n, r, i, s) {
		if (t === null || !o(t) || S.isKeyword(t)) return t;
		if (t.match(g)) return null;
		if (r && r.hasOwnProperty(t) && i.get(t) !== !0 && S.createTermDefinition({
			activeCtx: e,
			localCtx: r,
			term: t,
			defined: i,
		...
		if (l > 0) {
			...
			r && r.hasOwnProperty(n) && S.createTermDefinition({
				activeCtx: e,
				localCtx: r,
				term: n,
				defined: i,
	...
	S.getInitialContext = (e) => {
		let t = JSON.stringify({ processingMode: e.processingMode }), r = b.get(t);
		if (r) return r;
		let i = {
		...
		return b.size === x && b.clear(), b.set(t, i), i;
		function a() {
			let e = this;
			if (e.inverse) return e.inverse;
			let t = e.inverse = {}, n = e.fastCurieMap = {}, r = {}, i = (e["@language"] || "@none").toLowerCase(), a = e["@direction"], c = e.mappings, l = [...c.keys()].sort(y);
			for (let e of l) {
				let o = c.get(e);
				if (o === null) continue;
	...
	}, S.processingMode = (e, t) => t.toString() >= "1.1" ? !e.processingMode || e.processingMode >= "json-ld-" + t.toString() : e.processingMode === "json-ld-1.0", S.isKeyword = (e) => {
		if (!o(e) || e[0] !== "@") return !1;
		switch (e) {
			case "@base":
			case "@container":
	...
	function w(e, t) {
		...
		for (let n in e) {
			let r = e[n], i = t[n];
			if (n === "@container" && Array.isArray(r) && Array.isArray(i) && (r = r.slice().sort(), i = i.slice().sort()), !w(r, i)) return !1;
		}
		return !0;
	}
})), O = /* @__PURE__ */ e(((e, t) => {
	var n = p(), { isArray: r, isObject: i, isEmptyObject: a, isString: o, isUndefined: s } = d(), { isList: c, isValue: l, isGraph: u, isSubject: h } = f(), { expandIri: g, getContextValue: v, isKeyword: y, process: b, processingMode: x } = D(), { isAbsolute: S } = _(), { REGEX_BCP47: C, REGEX_KEYWORD: w, addValue: T, asArray: O, getValues: k, validateTypeValue: A } = m(), { handleEvent: j } = E(), M = {};
	t.exports = M, M.expand = async ({ activeCtx: e, activeProperty: t = null, element: c, options: l = {}, insideList: u = !1, insideIndex: d = !1, typeScopedContext: f = null }) => {
		if (c == null) return null;
		if (t === "@default" && (l = Object.assign({}, l, { isFrame: !1 })), !r(c) && !i(c)) return !u && (t === null || g(e, t, { vocab: !0 }, l) === "@graph") ? (l.eventHandler && j({
			event: {
				type: ["JsonLdEvent"],
		...
		if (r(c)) {
			let n = [], i = v(e, t, "@container") || [];
			u ||= i.includes("@list");
			for (let i = 0; i < c.length; ++i) {
				let a = await M.expand({
					activeCtx: e,
					activeProperty: t,
					element: c[i],
					options: l,
		...
		let p = g(e, t, { vocab: !0 }, l), m = v(e, t, "@context");
		f ||= e.previousContext ? e : null;
		let h = Object.keys(c).sort(), _ = !d;
		if (_ && f && h.length <= 2 && !h.includes("@context")) for (let t of h) {
			let n = g(f, t, { vocab: !0 }, l);
			if (n === "@value") {
				_ = !1, e = f;
				break;
			}
		...
		_ && (e = e.revertToPreviousContext()), s(m) || (e = await b({
			activeCtx: e,
			localCtx: m,
			propagate: !0,
			overrideProtected: !0,
			options: l
		})), "@context" in c && (e = await b({
			activeCtx: e,
			localCtx: c["@context"],
			options: l
		})), f = e;
		let y = null;
		for (let t of h) if (g(e, t, { vocab: !0 }, l) === "@type") {
			y ||= t;
		...
		let w = h.length;
		if ("@value" in C) {
			if ("@type" in C && ("@language" in C || "@direction" in C)) throw new n("Invalid JSON-LD syntax; an element containing \"@value\" may not contain both \"@type\" and either \"@language\" or \"@direction\".", "jsonld.SyntaxError", {
				code: "invalid value object",
				element: C
			});
			let t = w - 1;
			if ("@type" in C && --t, "@index" in C && --t, "@language" in C && --t, "@direction" in C && --t, t !== 0) throw new n("Invalid JSON-LD syntax; an element containing \"@value\" may only have an \"@index\" property and either \"@type\" or either or both \"@language\" or \"@direction\".", "jsonld.SyntaxError", {
				code: "invalid value object",
				element: C
			});
			let r = C["@value"] === null ? [] : O(C["@value"]), i = k(C, "@type");
			if (!(x(e, 1.1) && i.includes("@json") && i.length === 1)) {
				if (r.length === 0) l.eventHandler && j({
					event: {
						type: ["JsonLdEvent"],
				...
				else if (!r.every((e) => o(e) || a(e)) && "@language" in C) throw new n("Invalid JSON-LD syntax; only strings may be language-tagged.", "jsonld.SyntaxError", {
					code: "invalid language-tagged value",
					element: C
				});
				else if (!i.every((e) => S(e) && !(o(e) && e.indexOf("_:") === 0) || a(e))) throw new n("Invalid JSON-LD syntax; an element containing \"@value\" and \"@type\" must have an absolute IRI for the value of \"@type\".", "jsonld.SyntaxError", {
					code: "invalid typed value",
					element: C
				});
			}
		} else if ("@type" in C && !r(C["@type"])) C["@type"] = [C["@type"]];
		else if ("@set" in C || "@list" in C) {
			if (w > 1 && !(w === 2 && "@index" in C)) throw new n("Invalid JSON-LD syntax; if an element has the property \"@set\" or \"@list\", then it can have at most one other property that is \"@index\".", "jsonld.SyntaxError", {
				code: "invalid set or list object",
				element: C
			});
			"@set" in C && (C = C["@set"], h = Object.keys(C), w = h.length);
		} else w === 1 && "@language" in C && (l.eventHandler && j({
			event: {
				type: ["JsonLdEvent"],
		...
		return i(C) && !l.keepFreeFloatingNodes && !u && (t === null || p === "@graph" || (v(e, t, "@container") || []).includes("@graph")) && (C = N({
			value: C,
			count: w,
			options: l
		})), C;
	};
	function N({ value: e, count: t, options: n }) {
		if (t === 0 || "@value" in e || "@list" in e || t === 1 && "@id" in e) {
			if (n.eventHandler) {
				let r, i;
				t === 0 ? (r = "empty object", i = "Dropping empty object.") : "@value" in e ? (r = "object with only @value", i = "Dropping object with only @value.") : "@list" in e ? (r = "object with only @list", i = "Dropping object with only @list.") : t === 1 && "@id" in e && (r = "object with only @id", i = "Dropping object with only @id."), j({
					event: {
						type: ["JsonLdEvent"],
						code: r,
						level: "warning",
	...
	async function P({ activeCtx: e, activeProperty: t, expandedActiveProperty: u, element: d, expandedParent: f, options: p = {}, insideList: m, typeKey: _, typeScopedContext: w }) {
		let E = Object.keys(d).sort(), D = [], k, F = d[_] && g(e, r(d[_]) ? d[_][0] : d[_], { vocab: !0 }, {
			...p,
			typeExpansion: !0
		}) === "@json";
		for (let m of E) {
			...
			if (y(P)) {
				...
				if (P in f && P !== "@included" && P !== "@type") throw new n("Invalid JSON-LD syntax; colliding keywords detected.", "jsonld.SyntaxError", {
			...
			if (P === "@id") {
				if (!o(_)) {
					if (!p.isFrame) throw new n("Invalid JSON-LD syntax; \"@id\" value must a string.", "jsonld.SyntaxError", {
						code: "invalid @id value",
						value: _
					});
					if (i(_)) {
						if (!a(_)) throw new n("Invalid JSON-LD syntax; \"@id\" value an empty object or array of strings, if framing", "jsonld.SyntaxError", {
							code: "invalid @id value",
							value: _
						});
					} else if (r(_)) {
						if (!_.every((e) => o(e))) throw new n("Invalid JSON-LD syntax; \"@id\" value an empty object or array of strings, if framing", "jsonld.SyntaxError", {
							code: "invalid @id value",
							value: _
						});
				...
				T(f, "@id", O(_).map((t) => {
					if (o(t)) {
						let n = g(e, t, { base: !0 }, p);
						return p.eventHandler && (n === null ? j(t === null ? {
							event: {
								type: ["JsonLdEvent"],
								code: "null @id value",
								level: "warning",
					...
					return t;
				}), { propertyIsArray: p.isFrame });
				continue;
			}
			if (P === "@type") {
				i(_) && (_ = Object.fromEntries(Object.entries(_).map(([e, t]) => [g(w, e, { vocab: !0 }), O(t).map((e) => g(w, e, {
					base: !0,
					vocab: !0
				}, {
					...p,
					typeExpansion: !0
				}))]))), A(_, p.isFrame), T(f, "@type", O(_).map((e) => {
					if (o(e)) {
						let t = g(w, e, {
							base: !0,
							vocab: !0
						...
						return t !== "@json" && !S(t) && p.eventHandler && j({
							event: {
								type: ["JsonLdEvent"],
			...
			if (P === "@included" && x(e, 1.1)) {
				let r = O(await M.expand({
					activeCtx: e,
					activeProperty: t,
					element: _,
					options: p
				}));
				if (!r.every((e) => h(e))) throw new n("Invalid JSON-LD syntax; values of @included must expand to node objects.", "jsonld.SyntaxError", {
					code: "invalid @included value",
					value: _
				});
				T(f, "@included", r, { propertyIsArray: !0 });
				continue;
			}
			if (P === "@graph" && !(i(_) || r(_))) throw new n("Invalid JSON-LD syntax; \"@graph\" value must not be an object or an array.", "jsonld.SyntaxError", {
				code: "invalid @graph value",
				value: _
			});
			if (P === "@value") {
				k = _, F && x(e, 1.1) ? f["@value"] = _ : T(f, "@value", _, { propertyIsArray: p.isFrame });
				continue;
			}
			if (P === "@language") {
				if (_ === null) continue;
				if (!o(_) && !p.isFrame) throw new n("Invalid JSON-LD syntax; \"@language\" value must be a string.", "jsonld.SyntaxError", {
					code: "invalid language-tagged string",
					value: _
				});
				_ = O(_).map((e) => o(e) ? e.toLowerCase() : e);
				for (let e of _) o(e) && !e.match(C) && p.eventHandler && j({
					event: {
						type: ["JsonLdEvent"],
						code: "invalid @language value",
						level: "warning",
				...
				T(f, "@language", _, { propertyIsArray: p.isFrame });
				continue;
			}
			if (P === "@direction") {
				if (!o(_) && !p.isFrame) throw new n("Invalid JSON-LD syntax; \"@direction\" value must be a string.", "jsonld.SyntaxError", {
					code: "invalid base direction",
					value: _
				});
				_ = O(_);
				for (let e of _) if (o(e) && e !== "ltr" && e !== "rtl") throw new n("Invalid JSON-LD syntax; \"@direction\" must be \"ltr\" or \"rtl\".", "jsonld.SyntaxError", {
					code: "invalid base direction",
					value: _
				});
				T(f, "@direction", _, { propertyIsArray: p.isFrame });
				continue;
			}
			if (P === "@index") {
				if (!o(_)) throw new n("Invalid JSON-LD syntax; \"@index\" value must be a string.", "jsonld.SyntaxError", {
			...
			if (P === "@reverse") {
				if (!i(_)) throw new n("Invalid JSON-LD syntax; \"@reverse\" value must be an object.", "jsonld.SyntaxError", {
				...
				if (E = await M.expand({
					...
					element: _,
					options: p
				}), "@reverse" in E) for (let e in E["@reverse"]) T(f, e, E["@reverse"][e], { propertyIsArray: !0 });
				let t = f["@reverse"] || null;
				for (let e in E) {
					if (e === "@reverse") continue;
					t === null && (t = f["@reverse"] = {}), T(t, e, [], { propertyIsArray: !0 });
					let r = E[e];
			...
			let B = v(e, m, "@container") || [];
			if (B.includes("@language") && i(_)) {
				let e = v(R, m, "@direction");
				E = I(R, _, e, p);
			} else if (B.includes("@index") && i(_)) {
				let t = B.includes("@graph"), n = v(R, m, "@index") || "@index", r = n !== "@index" && g(e, n, { vocab: !0 }, p);
				E = await L({
					activeCtx: R,
					options: p,
					activeProperty: m,
			...
			} else if (B.includes("@id") && i(_)) {
				let e = B.includes("@graph");
				E = await L({
					activeCtx: R,
					options: p,
					activeProperty: m,
			...
			} else if (B.includes("@type") && i(_)) E = await L({
				activeCtx: R.revertToPreviousContext(),
				options: p,
				activeProperty: m,
				value: _,
				asGraph: !1,
				...
				if (n || P === "@set") {
					let e = t;
					n && u === "@graph" && (e = null), E = await M.expand({
						activeCtx: R,
						activeProperty: e,
						element: _,
						options: p,
			...
			if (E !== null || P === "@value") {
				if (P !== "@list" && !c(E) && B.includes("@list") && (E = { "@list": O(E) }), B.includes("@graph") && !B.some((e) => e === "@id" || e === "@index")) {
					if (E = O(E), p.isFrame || (E = E.filter((e) => {
				...
				if (R.mappings.has(m) && R.mappings.get(m).reverse) {
					let e = f["@reverse"] = f["@reverse"] || {};
					E = O(E);
					for (let t = 0; t < E.length; ++t) {
						...
						T(e, P, r, { propertyIsArray: !0 });
					}
					continue;
				}
				T(f, P, E, { propertyIsArray: !0 });
			}
		}
		if ("@value" in f && !(f["@type"] === "@json" && x(e, 1.1)) && (i(k) || r(k)) && !p.isFrame) throw new n("Invalid JSON-LD syntax; \"@value\" value must not be an object or an array.", "jsonld.SyntaxError", {
		...
		for (let a of D) {
			let o = r(d[a]) ? d[a] : [d[a]];
			for (let r of o) {
				if (!i(r) || Object.keys(r).some((t) => g(e, t, { vocab: !0 }, p) === "@value")) throw new n("Invalid JSON-LD syntax; nested value must be a node object.", "jsonld.SyntaxError", {
					code: "invalid @nest value",
					value: r
				});
				await P({
	...
	function F({ activeCtx: e, activeProperty: t, value: n, options: r }) {
		if (n == null) return null;
		let i = g(e, t, { vocab: !0 }, r);
		if (i === "@id") return g(e, n, { base: !0 }, r);
		if (i === "@type") return g(e, n, {
			vocab: !0,
			base: !0
		}, {
			...r,
		...
		let a = v(e, t, "@type");
		if ((a === "@id" || i === "@graph") && o(n)) {
			let i = g(e, n, { base: !0 }, r);
			return i === null && n.match(w) && r.eventHandler && j({
				event: {
					type: ["JsonLdEvent"],
					code: "reserved @id value",
					level: "warning",
		...
		if (a === "@vocab" && o(n)) return { "@id": g(e, n, {
			vocab: !0,
			base: !0
		}, r) };
		if (y(i)) return n;
		...
		if (a && ![
			...
			let n = v(e, t, "@language");
			n !== null && (s["@language"] = n);
			let r = v(e, t, "@direction");
	...
	function I(e, t, i, a) {
		...
		for (let l of c) {
			...
			for (let e of u) {
				...
				let r = { "@value": e };
				c !== "@none" && (l.match(C) || a.eventHandler && j({
					event: {
						type: ["JsonLdEvent"],
						code: "invalid @language value",
						level: "warning",
	...
	async function L({ activeCtx: e, options: t, activeProperty: i, value: a, asGraph: o, indexKey: c, propertyIndex: d }) {
		let f = [], p = Object.keys(a).sort(), m = c === "@type";
		for (let h of p) {
			if (m) {
				let n = v(e, h, "@context");
				s(n) || (e = await b({
			...
			_ = d ? h === "@none" ? "@none" : F({
				...
				value: h,
				options: t
			}) : g(e, h, { vocab: !0 }, t), c === "@id" ? h = g(e, h, { base: !0 }, t) : m && (h = _);
			for (let e of p) {
				if (o && !u(e) && (e = { "@graph": [e] }), c === "@type") _ === "@none" || (e["@type"] ? e["@type"] = [h].concat(e["@type"]) : e["@type"] = [h]);
				else if (l(e) && ![
					"@language",
					"@type",
					"@index"
	...
	var { isKeyword: n } = D(), r = f(), i = d(), a = m(), o = p(), s = {};
	...
	var { isSubjectReference: n } = f(), { createMergedNodeMap: r } = k(), i = {};
	...
	var n = p(), r = f(), i = d(), { REGEX_BCP47: a, addValue: o } = m(), { handleEvent: s } = E(), { RDF_LIST: c, RDF_FIRST: l, RDF_REST: u, RDF_NIL: g, RDF_TYPE: _, RDF_JSON_LITERAL: v, XSD_BOOLEAN: y, XSD_DOUBLE: b, XSD_INTEGER: x, XSD_STRING: S } = h(), C = {};
	t.exports = C, C.fromRDF = async (e, t) => {
		let { useRdfType: a = !1, useNativeTypes: s = !1, rdfDirection: d = null } = t, f = {}, p = { "@default": f }, m = {};
		if (d) {
			if (d === "compound-literal") throw new n("Unsupported rdfDirection value.", "jsonld.InvalidRdfDirection", { value: d });
			if (d !== "i18n-datatype") throw new n("Unknown rdfDirection value.", "jsonld.InvalidRdfDirection", { value: d });
		}
		for (let n of e) {
			let e = n.graph.termType === "DefaultGraph" ? "@default" : n.graph.value;
			e in p || (p[e] = {}), e !== "@default" && !(e in f) && (f[e] = { "@id": e });
			let r = p[e], i = T(n.subject), c = n.predicate.value, l = n.object;
			i in r || (r[i] = { "@id": i });
			let u = r[i], h = T(l), v = !!h;
			if (v && !(h in r) && (r[h] = { "@id": h }), c === _ && !a && v) {
				o(u, "@type", h, { propertyIsArray: !0 });
				continue;
			}
			let y = w(l, s, d, t);
			if (o(u, c, y, { propertyIsArray: !0 }), v) {
				if (h === g) {
					let e = r[h];
		...
		for (let e in p) {
			...
			if (n.usages) {
				for (let e of n.usages) {
					...
					for (; a === u && i.isObject(m[n["@id"]]) && i.isArray(n[l]) && n[l].length === 1 && i.isArray(n[u]) && n[u].length === 1 && (f === 3 || f === 4 && i.isArray(n["@type"]) && n["@type"].length === 1 && n["@type"][0] === c) && (s.push(n[l][0]), d.push(n["@id"]), e = m[n["@id"]], n = e.node, a = e.property, o = e.value, f = Object.keys(n).length, r.isBlankNode(n)););
		...
		let h = [], v = Object.keys(f).sort();
		for (let e of v) {
			let t = f[e];
			if (e in p) {
				let n = t["@graph"] = [], i = p[e], a = Object.keys(i).sort();
				for (let e of a) {
					let t = i[e];
					r.isSubjectReference(t) || n.push(t);
				}
			}
			r.isSubjectReference(t) || h.push(t);
		}
		return h;
	};
	function w(e, t, r, o) {
		...
		if (e.language) e.language.match(a) || o.eventHandler && s({
			...
			if (t) {
				if (c === y) l["@value"] === "true" || l["@value"] === "1" ? l["@value"] = !0 : l["@value"] === "false" || l["@value"] === "0" ? l["@value"] = !1 : l["@type"] = c;
				else if (c === x) {
					if (i.isNumeric(l["@value"])) {
						let e = parseInt(l["@value"], 10);
						e.toFixed(0) === l["@value"] && (l["@value"] = e);
					} else l["@type"] = c;
				} else c === b && i.isNumeric(l["@value"]) ? l["@value"] = parseFloat(l["@value"]) : l["@type"] = c;
	...
	var { createNodeMap: n } = k(), { isKeyword: r } = D(), i = f(), a = M(), o = p(), s = d(), c = m(), { handleEvent: l } = E(), { RDF_FIRST: u, RDF_REST: g, RDF_NIL: v, RDF_TYPE: y, RDF_JSON_LITERAL: b, RDF_LANGSTRING: x, XSD_BOOLEAN: S, XSD_DOUBLE: C, XSD_INTEGER: w, XSD_STRING: T } = h(), { isAbsolute: O } = _(), A = {};
	t.exports = A, A.toRDF = (e, t) => {
		...
		for (let s of o) {
			...
			for (let t of c) {
				...
				for (let r of c) {
					...
					let u = P(r, i, e, n, a.rdfDirection, a);
					u && e.push({
						subject: o,
						predicate: c,
						object: u,
	...
	function P(e, t, n, r, c, u) {
		...
		if (i.isValue(e)) {
			...
			if (n === "@json") d.value = a(t), d.datatype.value = b;
			else if (s.isBoolean(t)) d.value = t.toString(), d.datatype.value = n || S;
			else if (s.isDouble(t) || n === C) s.isDouble(t) || (t = parseFloat(t)), d.value = t.toExponential(15).replace(/(\d)0*e\+?/, "$1E"), d.datatype.value = n || C;
			else if (s.isNumber(t)) d.value = t.toFixed(0), d.datatype.value = n || w;
				...
				options: u
			}), d.value = t, d.datatype.value = n || x, d.language = e["@language"]) : ("@direction" in e && !c && u.eventHandler && l({
				event: {
					type: ["JsonLdEvent"],
					code: "rdfDirection not set",
					level: "warning",
				...
				options: u
			}), d.value = t, d.datatype.value = n || T);
		} else if (i.isList(e)) {
			let i = N(e["@list"], t, n, r, c, u);
			d = {
				termType: i.termType,
				value: i.value
			};
		} else d = F(s.isObject(e) ? e["@id"] : e);
		return d.termType === "NamedNode" && !O(d.value) ? (u.eventHandler && l({
			event: {
				type: ["JsonLdEvent"],
	...
	var { isKeyword: n } = D(), r = f(), i = d(), a = m(), o = _(), s = p(), { createNodeMap: c, mergeNodeMapGraphs: l } = k(), u = {};
	t.exports = u, u.frameMergedOrDefault = (e, t, n) => {
		...
		let o = [];
		return u.frame(r, Object.keys(r.subjects).sort(), t, o), n.pruneBlankNodeIdentifiers && (n.bnodesToClear = Object.keys(r.bnodeMap).filter((e) => r.bnodeMap[e].length === 1)), n.link = {}, C(o, n);
	}, u.frame = (e, t, o, c, l = null) => {
		y(o), o = o[0];
		let d = e.options, f = {
			embed: v(o, d, "embed"),
			explicit: v(o, d, "explicit"),
			requireAll: v(o, d, "requireAll")
		};
		e.link.hasOwnProperty(e.graph) || (e.link[e.graph] = {});
		let p = e.link[e.graph], m = b(e, t, o, f), _ = Object.keys(m).sort();
		for (let y of _) {
			let _ = m[y];
			if (l === null ? e.uniqueEmbeds = { [e.graph]: {} } : e.uniqueEmbeds[e.graph] = e.uniqueEmbeds[e.graph] || {}, f.embed === "@link" && y in p) {
				w(c, l, p[y]);
				continue;
			}
			let b = { "@id": y };
			if (y.indexOf("_:") === 0 && a.addValue(e.bnodeMap, y, b, { propertyIsArray: !0 }), p[y] = b, (f.embed === "@first" || f.embed === "@last") && e.is11) throw new s("Invalid JSON-LD syntax; invalid value of @embed.", "jsonld.SyntaxError", {
				code: "invalid @embed value",
				frame: o
			});
			if (!(!e.embedded && e.uniqueEmbeds[e.graph].hasOwnProperty(y))) {
				if (e.embedded && (f.embed === "@never" || g(_, e.graph, e.subjectStack))) {
					w(c, l, b);
					continue;
				}
				if (e.embedded && (f.embed == "@first" || f.embed == "@once") && e.uniqueEmbeds[e.graph].hasOwnProperty(y)) {
					w(c, l, b);
					continue;
				}
				if (f.embed === "@last" && y in e.uniqueEmbeds[e.graph] && S(e, y), e.uniqueEmbeds[e.graph][y] = {
					parent: c,
					property: l
				}, e.subjectStack.push({
					subject: _,
					...
					let t = !1, n = null;
					"@graph" in o ? (n = o["@graph"][0], t = y !== "@merged" && y !== "@default", i.isObject(n) || (n = {})) : (t = e.graph !== "@merged", n = {}), t && u.frame({
						...e,
						graph: y,
						embedded: !1
					}, Object.keys(e.graphMap[y]).sort(), [n], b, "@graph");
				}
				"@included" in o && u.frame({
					...e,
					embedded: !1
				}, t, o["@included"], b, "@included");
				for (let t of Object.keys(_).sort()) {
					if (n(t)) {
						if (b[t] = a.clone(_[t]), t === "@type") for (let t of _["@type"]) t.indexOf("_:") === 0 && a.addValue(e.bnodeMap, t, b, { propertyIsArray: !0 });
						continue;
					}
					if (!(f.explicit && !(t in o))) for (let n of _[t]) {
						let i = t in o ? o[t] : h(f);
						if (r.isList(n)) {
							let i = o[t] && o[t][0] && o[t][0]["@list"] ? o[t][0]["@list"] : h(f), s = { "@list": [] };
							w(b, t, s);
							let c = n["@list"];
							for (let t of c) r.isSubjectReference(t) ? u.frame({
								...e,
								embedded: !0
							}, [t["@id"]], i, s, "@list") : w(s, "@list", a.clone(t));
						} else r.isSubjectReference(n) ? u.frame({
							...e,
							embedded: !0
						}, [n["@id"]], i, b, t) : E(i[0], n) && w(b, t, a.clone(n));
					}
				}
				for (let e of Object.keys(o).sort()) {
					...
					let t = o[e][0] || {};
					if (!v(t, d, "omitDefault") && !(e in b)) {
						let n = "@null";
						"@default" in t && (n = a.clone(t["@default"])), i.isArray(n) || (n = [n]), b[e] = [{ "@preserve": n }];
					}
				}
				for (let t of Object.keys(o["@reverse"] || {}).sort()) {
					let n = o["@reverse"][t];
					for (let r of Object.keys(e.subjects)) a.getValues(e.subjects[r], t).some((e) => e["@id"] === y) && (b["@reverse"] = b["@reverse"] || {}, a.addValue(b["@reverse"], t, [], { propertyIsArray: !0 }), u.frame({
						...e,
						embedded: !0
					}, [r], n, b["@reverse"][t], l));
				}
				w(c, l, b), e.subjectStack.pop();
			}
		}
	}, u.cleanupNull = (e, t) => {
	...
	function x(e, t, o, s) {
		...
		for (let u in o) {
			...
			if (u === "@id") {
				...
				if (t && (y([t]), n = "@default" in t), c = !1, f.length === 0 && n) continue;
				...
				if (t === void 0) {
					...
					if (r.isList(f[0])) {
						let t = f[0]["@list"];
						r.isValue(n) ? d = t.some((e) => E(n, e)) : (r.isSubject(n) || r.isSubjectReference(n)) && (d = t.some((t) => T(e, n, t, s)));
					}
				} else d = r.isValue(t) ? f.some((e) => E(t, e)) : r.isSubjectReference(t) ? f.some((n) => T(e, t, n, s)) : i.isObject(t) ? f.length > 0 : !1;
			}
	...
	function C(e, t) {
		if (i.isArray(e)) return e.map((e) => C(e, t));
		if (i.isObject(e)) {
			if ("@preserve" in e) return e["@preserve"][0];
			if (r.isValue(e)) return e;
			if (r.isList(e)) return e["@list"] = C(e["@list"], t), e;
			if ("@id" in e) {
				let n = e["@id"];
				if (t.link.hasOwnProperty(n)) {
					let r = t.link[n].indexOf(e);
	...
	var n = p(), { isArray: r, isObject: i, isString: a, isUndefined: o } = d(), { isList: s, isValue: c, isGraph: l, isSimpleGraph: u, isSubjectReference: h } = f(), { expandIri: g, getContextValue: v, isKeyword: y, process: b, processingMode: x } = D(), { removeBase: S, prependBase: C } = _(), { REGEX_KEYWORD: w, addValue: T, asArray: E, compareShortestLeast: O } = m(), k = {};
	t.exports = k, k.compact = async ({ activeCtx: e, activeProperty: t = null, element: d, options: f = {} }) => {
		if (r(d)) {
			let n = [];
			for (let r = 0; r < d.length; ++r) {
				let i = await k.compact({
					activeCtx: e,
					activeProperty: t,
					element: d[r],
					options: f
			...
			return f.compactArrays && n.length === 1 && (v(e, t, "@container") || []).length === 0 && (n = n[0]), n;
		}
		let p = v(e, t, "@context");
		if (o(p) || (e = await b({
			activeCtx: e,
			...
			if (f.link && "@id" in d && f.link.hasOwnProperty(d["@id"])) {
				let e = f.link[d["@id"]];
				for (let t = 0; t < e.length; ++t) if (e[t].expanded === d) return e[t].compacted;
			}
			if (c(d) || h(d)) {
				let n = k.compactValue({
					activeCtx: e,
					activeProperty: t,
					value: d,
					options: f
				});
				return f.link && h(d) && (f.link.hasOwnProperty(d["@id"]) || (f.link[d["@id"]] = []), f.link[d["@id"]].push({
					expanded: d,
					compacted: n
				})), n;
			}
			if (s(d) && (v(e, t, "@container") || []).includes("@list")) return k.compact({
				activeCtx: e,
				activeProperty: t,
				element: d["@list"],
				options: f
			});
			let p = t === "@reverse", m = {}, g = e;
			!c(d) && !h(d) && (e = e.revertToPreviousContext());
			let _ = v(g, t, "@context");
			o(_) || (e = await b({
				activeCtx: e,
				localCtx: _,
				...
				overrideProtected: !0,
				options: f
			})), f.link && "@id" in d && (f.link.hasOwnProperty(d["@id"]) || (f.link[d["@id"]] = []), f.link[d["@id"]].push({
				expanded: d,
				compacted: m
			}));
			let S = d["@type"] || [];
			S.length > 1 && (S = Array.from(S).sort());
			let C = e;
			for (let t of S) {
				let n = v(g, k.compactIri({
					activeCtx: C,
					iri: t,
					relativeTo: { vocab: !0 }
				}), "@context");
			...
			let w = Object.keys(d).sort();
			for (let o of w) {
				let h = d[o];
				if (o === "@id") {
					let t = E(h).map((t) => k.compactIri({
						activeCtx: e,
						iri: t,
						relativeTo: { vocab: !1 },
						base: f.base
				...
				if (o === "@type") {
					...
					let n = k.compactIri({
						...
						iri: "@type",
						relativeTo: { vocab: !0 }
					}), i = (v(e, n, "@container") || []).includes("@set") && x(e, 1.1) || r(t) && h.length === 0;
					T(m, n, t, { propertyIsArray: i });
					continue;
				}
				if (o === "@reverse") {
					let t = await k.compact({
						activeCtx: e,
						activeProperty: "@reverse",
						element: h,
						options: f
					...
					for (let n in t) if (e.mappings.has(n) && e.mappings.get(n).reverse) {
						let r = t[n];
						T(m, n, r, { propertyIsArray: (v(e, n, "@container") || []).includes("@set") || !f.compactArrays }), delete t[n];
					}
					Object.keys(t).length > 0 && T(m, k.compactIri({
						activeCtx: e,
						iri: o,
						relativeTo: { vocab: !0 }
					}), t);
				...
				if (o === "@preserve") {
					let n = await k.compact({
						activeCtx: e,
						activeProperty: t,
						element: h,
						options: f
					});
					r(n) && n.length === 0 || T(m, o, n);
					continue;
				}
				if (o === "@index") {
					if ((v(e, t, "@container") || []).includes("@index")) continue;
					T(m, k.compactIri({
						activeCtx: e,
						iri: o,
						relativeTo: { vocab: !0 }
				...
				if (o !== "@graph" && o !== "@list" && o !== "@included" && y(o)) {
					T(m, k.compactIri({
						activeCtx: e,
						iri: o,
						relativeTo: { vocab: !0 }
				...
				if (!r(h)) throw new n("JSON-LD expansion error; expanded value must be an array.", "jsonld.SyntaxError");
				if (h.length === 0) {
					let t = k.compactIri({
						activeCtx: e,
						iri: o,
						...
						relativeTo: { vocab: !0 },
						reverse: p
					}), n = e.mappings.has(t) ? e.mappings.get(t)["@nest"] : null, r = m;
					n && (j(e, n, f), i(m[n]) || (m[n] = {}), r = m[n]), T(r, t, h, { propertyIsArray: !0 });
				}
				for (let t of h) {
					let n = k.compactIri({
						activeCtx: e,
						...
						relativeTo: { vocab: !0 },
						reverse: p
					}), d = e.mappings.has(n) ? e.mappings.get(n)["@nest"] : null, h = m;
					d && (j(e, d, f), i(m[d]) || (m[d] = {}), h = m[d]);
					let g = v(e, n, "@container") || [], _ = l(t), y = s(t), b;
					y ? b = t["@list"] : _ && (b = t["@graph"]);
					let x = await k.compact({
						activeCtx: e,
						activeProperty: n,
						element: y || _ ? b : t,
						options: f
					...
					if (y) {
						if (r(x) || (x = [x]), !g.includes("@list")) x = { [k.compactIri({
							activeCtx: e,
							iri: "@list",
							relativeTo: { vocab: !0 }
						})]: x }, "@index" in t && (x[k.compactIri({
							activeCtx: e,
							iri: "@index",
							relativeTo: { vocab: !0 }
						})] = t["@index"]);
					...
					if (_) {
						if (g.includes("@graph") && (g.includes("@id") || g.includes("@index") && u(t))) {
							let r;
							h.hasOwnProperty(n) ? r = h[n] : h[n] = r = {};
							let i = (g.includes("@id") ? t["@id"] : t["@index"]) || k.compactIri({
								activeCtx: e,
								iri: "@none",
								relativeTo: { vocab: !0 }
							});
							T(r, i, x, { propertyIsArray: !f.compactArrays || g.includes("@set") });
						} else g.includes("@graph") && u(t) ? (r(x) && x.length > 1 && (x = { "@included": x }), T(h, n, x, { propertyIsArray: !f.compactArrays || g.includes("@set") })) : (r(x) && x.length === 1 && f.compactArrays && (x = x[0]), x = { [k.compactIri({
							activeCtx: e,
							iri: "@graph",
							relativeTo: { vocab: !0 }
						})]: x }, "@id" in t && (x[k.compactIri({
							activeCtx: e,
							iri: "@id",
							relativeTo: { vocab: !0 }
						})] = t["@id"]), "@index" in t && (x[k.compactIri({
							activeCtx: e,
							iri: "@index",
							relativeTo: { vocab: !0 }
						})] = t["@index"]), T(h, n, x, { propertyIsArray: !f.compactArrays || g.includes("@set") }));
					} else if (g.includes("@language") || g.includes("@index") || g.includes("@id") || g.includes("@type")) {
						let r;
						h.hasOwnProperty(n) ? r = h[n] : h[n] = r = {};
						let i;
						if (g.includes("@language")) c(x) && (x = x["@value"]), i = t["@language"];
						else if (g.includes("@index")) {
							let r = v(e, n, "@index") || "@index", o = k.compactIri({
								activeCtx: e,
								iri: r,
								relativeTo: { vocab: !0 }
							});
							if (r === "@index") i = t["@index"], delete x[o];
							else {
								let e;
								if ([i, ...e] = E(x[r] || []), !a(i)) i = null;
							...
							Object.keys(x).length === 1 && "@id" in t && (x = await k.compact({
								activeCtx: e,
								activeProperty: n,
								element: { "@id": t["@id"] },
								options: f
						...
						i ||= k.compactIri({
							...
							iri: "@none",
							relativeTo: { vocab: !0 }
						}), T(r, i, x, { propertyIsArray: g.includes("@set") });
					} else {
						let e = !f.compactArrays || g.includes("@set") || g.includes("@list") || r(x) && x.length === 0 || o === "@list" || o === "@graph";
						T(h, n, x, { propertyIsArray: e });
					}
				}
			}
			return m;
		...
		return d;
	}, k.compactIri = ({ activeCtx: e, iri: t, value: r = null, relativeTo: a = { vocab: !1 }, reverse: o = !1, base: u = null }) => {
		if (t === null) return t;
		e.isPropertyTermScoped && e.previousContext && (e = e.previousContext);
		let d = e.getInverse();
		if (y(t) && t in d && "@none" in d[t] && "@type" in d[t]["@none"] && "@none" in d[t]["@none"]["@type"]) return d[t]["@none"]["@type"]["@none"];
		if (a.vocab && t in d) {
		...
		let f = null, p = [], m = e.fastCurieMap, h = t.length - 1;
		for (let e = 0; e < h && t[e] in m; ++e) m = m[t[e]], "" in m && p.push(m[""][0]);
		for (let n = p.length - 1; n >= 0; --n) {
			let i = p[n], a = i.terms;
			for (let n of a) {
				let a = n + ":" + t.substr(i.iri.length);
				e.mappings.get(n)._prefix && (!e.mappings.has(a) || r === null && e.mappings.get(a)["@id"] === t) && (f === null || O(a, f) < 0) && (f = a);
			}
		}
		if (f !== null) return f;
		...
		return t;
	}, k.compactValue = ({ activeCtx: e, activeProperty: t, value: n, options: r }) => {
		if (c(n)) {
			let r = v(e, t, "@type"), i = v(e, t, "@language"), o = v(e, t, "@direction"), s = v(e, t, "@container") || [], c = "@index" in n && !s.includes("@index");
			...
			let l = Object.keys(n).length, u = l === 1 || l === 2 && "@index" in n && !c, d = "@language" in e, f = a(n["@value"]), p = e.mappings.has(t) && e.mappings.get(t)["@language"] === null;
			if (u && r !== "@none" && (!d || !f || p)) return n["@value"];
			let m = {};
			return c && (m[k.compactIri({
				activeCtx: e,
				iri: "@index",
				relativeTo: { vocab: !0 }
			})] = n["@index"]), "@type" in n ? m[k.compactIri({
				activeCtx: e,
				iri: "@type",
				relativeTo: { vocab: !0 }
			})] = k.compactIri({
				...
				iri: n["@type"],
				relativeTo: { vocab: !0 }
			}) : "@language" in n && (m[k.compactIri({
				activeCtx: e,
				iri: "@language",
				relativeTo: { vocab: !0 }
			})] = n["@language"]), "@direction" in n && (m[k.compactIri({
				activeCtx: e,
				iri: "@direction",
				relativeTo: { vocab: !0 }
			})] = n["@direction"]), m[k.compactIri({
				activeCtx: e,
				iri: "@value",
				relativeTo: { vocab: !0 }
			})] = n["@value"], m;
		}
		let i = g(e, t, { vocab: !0 }, r), o = v(e, t, "@type"), s = k.compactIri({
			activeCtx: e,
			iri: n["@id"],
			relativeTo: { vocab: o === "@vocab" },
			base: r.base
		});
		return o === "@id" || o === "@vocab" || i === "@graph" ? s : { [k.compactIri({
			activeCtx: e,
			iri: "@id",
			relativeTo: { vocab: !0 }
		})]: s };
	...
	function j(e, t, r) {
		if (g(e, t, { vocab: !0 }, r) !== "@nest") throw new n("JSON-LD compact error; nested property must have an @nest value resolving to @nest.", "jsonld.SyntaxError", { code: "invalid @nest value" });
	}
})), I = /* @__PURE__ */ e(((e, t) => {
	t.exports = (e) => {
		class t {
	...
	var n = u(), r = y(), i = m(), a = w(), o = i.IdentifierIssuer, s = p(), c = S(), l = T(), { expand: h } = O(), { flatten: v } = A(), { fromRDF: b } = j(), { toRDF: x } = N(), { frameMergedOrDefault: C, cleanupNull: M } = P(), { isArray: L, isObject: R, isString: z } = d(), { isSubjectReference: B } = f(), { expandIri: V, getInitialContext: H, process: U, processingMode: W } = D(), { compact: G, compactIri: K } = F(), { createNodeMap: q, createMergedNodeMap: J, mergeNodeMaps: Y } = k(), { logEventHandler: X, logWarningEventHandler: Z, safeEventHandler: ee, setDefaultEventHandler: te, setupEventHandler: ne, strictEventHandler: re, unhandledEventHandler: ie } = E(), Q = function(e) {
		let t = {}, u = new c({ max: 100 });
		e.compact = async function(t, n, r) {
			...
			r = d(r, {
				...
				link: !1,
				issuer: new o("_:b"),
				contextResolver: new a({ sharedCache: u })
			}), r.link && (r.skipExpansion = !0), r.compactToRelative || delete r.base;
			let c;
			c = r.skipExpansion ? t : await e.expand(t, r);
			let l = await e.processContext(H(r), n, r), f = await G({
				activeCtx: l,
				element: c,
				options: r
			});
			r.compactArrays && !r.graph && L(f) ? f.length === 1 ? f = f[0] : f.length === 0 && (f = {}) : r.graph && R(f) && (f = [f]), R(n) && "@context" in n && (n = n["@context"]), n = i.clone(n), L(n) || (n = [n]);
			let p = n;
			n = [];
			for (let e = 0; e < p.length; ++e) (!R(p[e]) || Object.keys(p[e]).length > 0) && n.push(p[e]);
			let m = n.length > 0;
			if (n.length === 1 && (n = n[0]), L(f)) {
				let e = K({
					activeCtx: l,
					iri: "@graph",
					relativeTo: { vocab: !0 }
				}), t = f;
				f = {}, m && (f["@context"] = n), f[e] = t;
			} else if (R(f) && m) {
				let e = f;
				f = { "@context": n };
				for (let t in e) f[t] = e[t];
			}
			return f;
			...
			if ("expandContext" in n) {
				...
				r.expandContext = R(e) && "@context" in e ? e : { "@context": e }, o.push(r.expandContext);
			}
			let s;
			if (!z(t)) r.input = i.clone(t);
			else {
			...
			return R(l) && "@graph" in l && Object.keys(l).length === 1 ? l = l["@graph"] : l === null && (l = []), L(l) || (l = [l]), l;
		}, e.flatten = async function(t, n, r) {
			if (arguments.length < 1) return /* @__PURE__ */ TypeError("Could not flatten, too few arguments.");
			typeof n == "function" ? n = null : n ||= null, r = d(r, {
				base: z(t) ? t : "",
				contextResolver: new a({ sharedCache: u })
			});
			let i = v(await e.expand(t, r));
			...
			if (r = d(r, {
				...
				if (n = t.document, t.contextUrl) {
					let e = n["@context"];
					e ? L(e) ? e.push(t.contextUrl) : e = [e, t.contextUrl] : e = t.contextUrl, n["@context"] = e;
				}
			}
			let i = n && n["@context"] || {}, o = await e.processContext(H(r), i, r);
			r.hasOwnProperty("omitGraph") || (r.omitGraph = W(o, 1.1)), r.hasOwnProperty("pruneBlankNodeIdentifiers") || (r.pruneBlankNodeIdentifiers = W(o, 1.1));
			let s = await e.expand(t, r), c = { ...r };
			c.isFrame = !0, c.keepFreeFloatingNodes = !0;
			let l = await e.expand(n, c);
			c.merged = !Object.keys(n).map((e) => V(o, e, { vocab: !0 })).includes("@graph"), c.is11 = W(o, 1.1);
			let f = C(s, l, c);
			c.graph = !r.omitGraph, c.skipExpansion = !0, c.link = {}, c.framing = !0;
			let p = await e.compact(f, i, c);
			return c.link = {}, p = M(p, c), p;
			...
			return n = d(n, {
				base: z(t) ? t : "",
				contextResolver: new a({ sharedCache: u })
			}), J(await e.expand(t, n), n);
		}, e.merge = async function(t, n, r) {
			if (arguments.length < 1) throw TypeError("Could not merge, too few arguments.");
			if (!L(t)) throw TypeError("Could not merge, \"docs\" must be an array.");
			typeof n == "function" ? n = null : n ||= null, r = d(r, { contextResolver: new a({ sharedCache: u }) });
			let s = await Promise.all(t.map((t) => {
				let n = { ...r };
				return e.expand(t, n);
			})), c = !0;
			"mergeNodes" in r && (c = r.mergeNodes);
			let l = r.issuer || new o("_:b"), f = { "@default": {} };
			for (let e = 0; e < s.length; ++e) {
				let t = i.relabelBlankNodes(s[e], { issuer: new o("_:b" + e + "-") }), n = c || e === 0 ? f : { "@default": {} };
				if (q(t, n, "@default", l), n !== f) for (let e in n) {
					let t = n[e];
					if (!(e in f)) {
						f[e] = t;
						continue;
			...
			let p = Y(f), m = [], h = Object.keys(p).sort();
			for (let e = 0; e < h.length; ++e) {
				let t = p[h[e]];
				B(t) || m.push(t);
			}
			return n === null ? m : (r.graph = !0, r.skipExpansion = !0, await e.compact(m, n, r));
			...
			try {
				...
				z(i.document) && (i.document = JSON.parse(i.document));
			} catch (e) {
			...
			return n = d(n, {
				base: "",
				contextResolver: new a({ sharedCache: u })
			}), t === null ? H(n) : (t = i.clone(t), R(t) && "@context" in t || (t = { "@context": t }), U({
				activeCtx: e,
				localCtx: t,
				options: n
			}));
		}, e.getContextValue = D().getContextValue, e.documentLoaders = {}, e.useDocumentLoader = function(t) {
			...
			delete t[e];
		}, e.registerRDFParser("application/n-quads", l.parse), e.url = _(), e.logEventHandler = X, e.logWarningEventHandler = Z, e.safeEventHandler = ee, e.setDefaultEventHandler = te, e.strictEventHandler = re, e.unhandledEventHandler = ie, e.util = i, Object.assign(e, i), e.promises = e, e.RequestQueue = g(), e.JsonLdProcessor = I()(e), r.setupGlobals(e), r.setupDocumentLoaders(e);
		function d(t, { documentLoader: n = e.documentLoader, ...r }) {
...
//# sourceMappingURL=jsonld-B2M5QgXx.js.map