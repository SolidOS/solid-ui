...
var i = "image/png", a = class extends r {
  ...
  `;
	...
	constructor() {
		super(), this.label = "Take Photo", this.heading = "Take a photo", this.captureLabel = "Take Photo", this.confirmLabel = "Use Photo", this.retakeLabel = "Retake", this.cancelLabel = "Cancel", this.presentation = "inline", this.theme = "light", this.facingMode = "user", this.constraints = "", this.captureFormat = i, this.captureQuality = void 0, this.open = !1, this.disabled = !1, this.name = "", this.required = !1, this.showTrigger = !1, this.showCancelButton = !0, this.autoCloseOnCapture = !1, this.fileNamePrefix = "", this.mediaConstraints = void 0, this._errorMessage = "", this._previewUrl = "", this._startingPreview = !1, this._internals = typeof this.attachInternals == "function" ? this.attachInternals() : null;
	}
	get value() {
		return this._value;
	}
	...
	_createFileFromBlob(e) {
		let t = e.type || this.captureFormat || i, n = this._fileExtensionForMimeType(t), r = (this.fileNamePrefix || this.name || "photo").trim() || "photo";
	...
	async _captureSnapshot() {
		...
		let o = await new Promise((e) => {
			r.toBlob(e, this.captureFormat || i, this.captureQuality);
		});
		if (!o) {
	...
	_confirmPhoto() {
		!this.value || !this._previewUrl || (this._dispatchValueEvents(), this.dispatchEvent(new CustomEvent("photo-captured", {
			detail: {
				...
				contentType: this.value.type || this.captureFormat || i
			},
			bubbles: !0,
			composed: !0
	...
	_renderViewport() {
		return this._previewUrl ? t`<img class="capture-preview" part="preview-image" src="${this._previewUrl}" alt="Captured photo preview" />` : t`<video class="capture-preview" part="preview-video" autoplay playsinline muted></video>`;
	}
	_renderStatus() {
		return this._errorMessage ? t`<div class="status error" part="error-message">${this._errorMessage}</div>` : this._startingPreview ? t`<div class="status" part="status-message">Opening camera…</div>` : this.value ? t`<div class="status" part="status-message">Review the photo before confirming it.</div>` : t`<div class="status" part="status-message">Preview the camera and take a photo when ready.</div>`;
	}
	_renderActions() {
		return t`
	...
	render() {
		return t`
      ${this.showTrigger || this.presentation === "dialog" ? t`
          <button
...
}, o = "solid-ui-photo-capture";
customElements.get(o) || customElements.define(o, a);
//#endregion
export { a as t };

//# sourceMappingURL=photo-capture-DzK6wZF4.js.map