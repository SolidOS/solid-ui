...
var At = class e extends xt {
	...
	statementsMatching(e, t, n, r, i) {
		...
		for (u = 0; u < 4; u++) o[u] = this.canon(St.fromValue(a[u])), o[u] ? (l.push(u), s[u] = this.id(o[u])) : c.push(u);
...
async function Sn(e, t, n) {
	let r = n && Object.prototype.hasOwnProperty.call(n, "termType") ? n.value : n, i = await import("./jsonld-xV-qWuVa.js").then((e) => /* @__PURE__ */ u(e.default));
...
}, { xsd: Nn } = Mn, Pn = /\\u([a-fA-F0-9]{4})|\\U([a-fA-F0-9]{8})|\\([^])/g, Fn = {
	"\\": "\\",
	"'": "'",
	"\"": "\"",
	n: "\n",
...
var zn = {
	...
	constructor(e) {
		if (this._iri = /^<((?:[^ <>{}\\]|\\[uU])+)>[ \t]*/, this._unescapedIri = /^<([^\x00-\x20<>\\"\{\}\|\^\`]*)>[ \t]*/, this._simpleQuotedString = /^"([^"\\\r\n]*)"(?=[^"])/, this._simpleApostropheString = /^'([^'\\\r\n]*)'(?=[^'])/, this._langcode = /^@([a-z]+(?:-[a-z0-9]+)*)(?=[^a-z0-9])/i, this._dircode = /^--(?:(ltr)|(rtl))/, this._prefix = /^((?:[A-Za-z\xc0-\xd6\xd8-\xf6\xf8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd]|[\ud800-\udb7f][\udc00-\udfff])(?:\.?[\-0-9A-Z_a-z\xb7\xc0-\xd6\xd8-\xf6\xf8-\u037d\u037f-\u1fff\u200c\u200d\u203f\u2040\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd]|[\ud800-\udb7f][\udc00-\udfff])*)?:(?=[#\s<])/, this._prefixed = /^((?:[A-Za-z\xc0-\xd6\xd8-\xf6\xf8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd]|[\ud800-\udb7f][\udc00-\udfff])(?:\.?[\-0-9A-Z_a-z\xb7\xc0-\xd6\xd8-\xf6\xf8-\u037d\u037f-\u1fff\u200c\u200d\u203f\u2040\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd]|[\ud800-\udb7f][\udc00-\udfff])*)?:((?:(?:[0-:A-Z_a-z\xc0-\xd6\xd8-\xf6\xf8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd]|[\ud800-\udb7f][\udc00-\udfff]|%[0-9a-fA-F]{2}|\\[!#-\/;=?\-@_~])(?:(?:[\.\-0-:A-Z_a-z\xb7\xc0-\xd6\xd8-\xf6\xf8-\u037d\u037f-\u1fff\u200c\u200d\u203f\u2040\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd]|[\ud800-\udb7f][\udc00-\udfff]|%[0-9a-fA-F]{2}|\\[!#-\/;=?\-@_~])*(?:[\-0-:A-Z_a-z\xb7\xc0-\xd6\xd8-\xf6\xf8-\u037d\u037f-\u1fff\u200c\u200d\u203f\u2040\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd]|[\ud800-\udb7f][\udc00-\udfff]|%[0-9a-fA-F]{2}|\\[!#-\/;=?\-@_~]))?)?)(?:[ \t]+|(?=\.?[,;!\^\s#()\[\]\{\}"'<>]))/, this._variable = /^\?(?:(?:[A-Z_a-z\xc0-\xd6\xd8-\xf6\xf8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd]|[\ud800-\udb7f][\udc00-\udfff])(?:[\-0-:A-Z_a-z\xb7\xc0-\xd6\xd8-\xf6\xf8-\u037d\u037f-\u1fff\u200c\u200d\u203f\u2040\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd]|[\ud800-\udb7f][\udc00-\udfff])*)(?=[.,;!\^\s#()\[\]\{\}"'<>])/, this._blank = /^_:((?:[0-9A-Z_a-z\xc0-\xd6\xd8-\xf6\xf8-\u02ff\u0370-\u037d\u037f-\u1fff\u200c\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd]|[\ud800-\udb7f][\udc00-\udfff])(?:\.?[\-0-9A-Z_a-z\xb7\xc0-\xd6\xd8-\xf6\xf8-\u037d\u037f-\u1fff\u200c\u200d\u203f\u2040\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff\uf900-\ufdcf\ufdf0-\ufffd]|[\ud800-\udb7f][\udc00-\udfff])*)(?:[ \t]+|(?=\.?[,;:!\^\s#()\[\]\{\}"'<>]))/, this._number = /^[\-+]?(?:(\d+\.\d*|\.?\d+)[eE][\-+]?|\d*(\.)?)\d+(?=\.?[,;:!\^\s#()\[\]\{\}"'<>])/, this._boolean = /^(?:true|false)(?=[.,;!\^\s#()\[\]\{\}"'<>])/, this._atKeyword = /^@[a-z]+(?=[\s#<:])/i, this._keyword = /^(?:PREFIX|BASE|VERSION|GRAPH)(?=[\s#<])/i, this._n3Id = /^id(?=[\s#<])/, this._shortPredicates = /^a(?=[\s#()\[\]\{\}"'<>])/, this._newline = /^[ \t]*(?:#[^\n\r]*)?(?:\r\n|\n|\r)[ \t]*/, this._comment = /#([^\n\r]*)/, this._whitespace = /^[ \t]+/, this._endOfFile = /^(?:#[^\n\r]*)?$/, e ||= {}, this._isImpliedBy = e.isImpliedBy, this._lineMode = !!e.lineMode) {
			this._n3Mode = !1;
			for (let e in this) !(e in zn) && this[e] instanceof RegExp && (this[e] = Bn);
	...
	_tokenizeToEnd(e, t) {
		...
		for (;;) {
			let e, o;
			for (; e = this._newline.exec(n);) this.comments && (o = this._comment.exec(e[0])) && i("comment", o[1], "", this._line, e[0].length), n = n.slice(e[0].length), r = n.length, this._line++;
			if (!e && (e = this._whitespace.exec(n)) && (n = n.slice(e[0].length)), this._endOfFile.test(n)) return t && (this.comments && (o = this._comment.exec(n)) && i("comment", o[1], "", this._line, n.length), n = null, i("eof", "", "", this._line, 0)), this._input = n;
			...
			switch (c) {
				...
				case "<":
					if (f = this._unescapedIri.exec(n)) l = "IRI", u = f[1];
						...
						if (u = this._unescape(f[1], Fn), u === null || Ln.test(u)) return a(this);
						l = "IRI";
				...
				case "f":
				case "t":
					(f = this._boolean.exec(n)) ? (l = "literal", u = f[0], d = Nn.boolean) : m = !0;
					break;
				case "a":
					(f = this._shortPredicates.exec(n)) ? (l = "abbreviation", u = "a") : m = !0;
					break;
				case "i":
					this._n3Mode && (f = this._n3Id.exec(n)) ? l = "id" : m = !0;
					break;
				case "=":
			...
			if (m && ((this._previousMarker === "@prefix" || this._previousMarker === "PREFIX") && (f = this._prefix.exec(n)) ? (l = "prefix", u = f[1] || "") : ((f = this._prefixed.exec(n)) || t && (f = this._prefixed.exec(`${n} `))) && (l = "prefixed", d = f[1] || "", u = this._unescape(f[2], In))), this._previousMarker === "^^") switch (l) {
				case "prefixed":
					l = "type";
					break;
				case "IRI":
			...
			this.previousToken = g, this._previousMarker = l, n = n.slice(h);
		}
	...
	_unescape(e, t) {
		let n = !1, r = e.replace(Pn, (e, r, i, a) => {
			if (typeof r == "string") {
				let e = Number.parseInt(r, 16);
				return Rn(e) ? String.fromCharCode(e) : (n = !0, "");
			}
			if (typeof i == "string") {
				let e = Number.parseInt(i, 16);
				return Rn(e) ? e <= 65535 ? String.fromCharCode(Number.parseInt(i, 16)) : String.fromCharCode(55296 + ((e -= 65536) >> 10), 56320 + (e & 1023)) : (n = !0, "");
			}
	...
	tokenize(e, t) {
		if (this._line = 1, typeof e == "string") {
			...
			this._input !== null && e.length !== 0 && (this._pendingBuffer &&= (e = On.Buffer.concat([this._pendingBuffer, e]), null), e[e.length - 1] & 128 ? this._pendingBuffer = e : (this._input === void 0 ? this._input = this._readStartingBom(typeof e == "string" ? e : e.toString()) : this._input += e, this._tokenizeToEnd(t, !1)));
		}), e.on("end", () => {
			typeof this._input == "string" && this._tokenizeToEnd(t, !0);
		}), e.on("error", t);
	}
}, { rdf: Hn, xsd: Un } = Mn, Wn, G = 0, K = {
	namedNode: J,
	blankNode: Y,
	variable: Zn,
	literal: Xn,
	...
	get datatype() {
		return new Kn(this.datatypeString);
	}
	get datatypeString() {
		let e = this.id, t = e.lastIndexOf("\"") + 1, n = t < e.length ? e[t] : "";
		return n === "^" ? e.substr(t + 2) : n === "@" ? e.indexOf("--", t) > 0 ? Hn.dirLangString : Hn.langString : Un.string;
	}
	equals(t) {
...
Wn = new class extends Gn {
	constructor() {
		return super(""), Wn || this;
	}
	get termType() {
		return "DefaultGraph";
	}
...
var Yn = class extends Gn {
	constructor(e, t, n, r) {
		super(""), this._subject = e, this._predicate = t, this._object = n, this._graph = r || Wn;
	}
	get termType() {
		return "Quad";
	}
...
function Xn(e, t) {
	if (typeof t == "string") return new qn(`"${e}"@${t.toLowerCase()}`);
	if (t !== void 0 && !("termType" in t)) return new qn(`"${e}"@${t.language.toLowerCase()}${t.direction ? `--${t.direction.toLowerCase()}` : ""}`);
	let n = t ? t.value : "";
	return n === "" && (typeof e == "boolean" ? n = Un.boolean : typeof e == "number" ? Number.isFinite(e) ? n = Number.isInteger(e) ? Un.integer : Un.double : (n = Un.double, Number.isNaN(e) || (e = e > 0 ? "INF" : "-INF")) : e instanceof Date && !Number.isNaN(e.getTime()) && (n = Un.dateTime, e = e.toISOString())), n === "" || n === Un.string ? new qn(`"${e}"`) : new qn(`"${e}"^^${n}`);
}
function Zn(e) {
	return new Jn(e);
}
...
function $n(e) {
	...
	switch (e.termType) {
		case "NamedNode": return J(e.value);
		case "BlankNode": return Y(e.value);
		case "Variable": return Zn(e.value);
		case "DefaultGraph": return Wn;
		case "Literal": return Xn(e.value, e.language || e.datatype);
...
function er(e) {
	...
	return X($n(e.subject), $n(e.predicate), $n(e.object), $n(e.graph));
}
//#endregion
//#region node_modules/n3/src/N3Parser.js
var tr = 0, nr = class e {
	constructor(e) {
		this._contextStack = [], this._graph = null, e ||= {}, this._setBase(e.baseIRI), e.factory && ir(this, e.factory);
	...
	_readPunctuation(e) {
		...
		switch (e.type) {
			...
			case "~":
				...
				break;
			case "{|":
				i !== null && (this._tripleTerm = null), this._subject = this._readTripleTerm(), this._validAnnotation = !1, r = !0, t = this._readPredicate;
				break;
			case "|}":
				...
				this._subject = null, this._annotation = !1, t = this._getContextEndReader();
				break;
			default:
	...
	_readBlankNodePunctuation(e) {
		...
		return this._subject === null ? this._error("Expected ] to follow annotation", e) : (this._emit(this._subject, this._predicate, this._object, this._graph), t);
	}
	...
	_readTripleTerm() {
		...
		return this._reifier = null, this._tripleTerm = this._tripleTerm || this._factory.quad(this._subject, this._predicate, this._object), this._emit(r, this.RDF_REIFIES, this._tripleTerm, n || this._graph || this.DEFAULTGRAPH), r;
	}
	...
	parse(e, t, n, r) {
		let i, a, o, s;
		if (t && (t.onQuad || t.onPrefix || t.onComment || t.onVersion) ? (i = t.onQuad, a = t.onPrefix, o = t.onComment, s = t.onVersion) : (i = t, a = n, s = r), this._readCallback = this._readBeforeTopContext, this._sparqlStyle = !1, this._prefixes = Object.create(null), this._prefixes._ = this._blankNodePrefix ? this._blankNodePrefix.substr(2) : `b${tr++}_`, this._n3Mode && this._implicitEmptyPrefix && this._base && (this._prefixes[""] = this._resolveIRI("#")), this._prefixCallback = a || rr, this._versionCallback = s || rr, this._inversePredicate = !1, this._quantified = Object.create(null), this._emptyFormula = !1, !i) {
			let t = [], n;
		...
		let c = (e, t) => {
			e === null ? this._readCallback &&= this._readCallback(t) : (this._callback(e), this._callback = rr);
		};
		o && (this._lexer.comments = !0, c = (e, t) => {
			e === null ? this._readCallback && (t.type === "comment" ? o(t.value) : this._readCallback = this._readCallback(t)) : (this._callback(e), this._callback = rr);
...
if (nr.SUPPORTED_VERSIONS = [
	"1.2",
	"1.2-basic",
	"1.1"
], ir(nr.prototype, K), Z === void 0) var Z = {
	ELEMENT_NODE: 1,
	ATTRIBUTE_NODE: 2,
	TEXT_NODE: 3,
...
var ar = class e {
	...
	ancestorPath(e) {
		for (var t = ""; e && e.nodeType !== Z.DOCUMENT_NODE;) t = "/" + e.localName + t, e = e.parentNode;
		return t;
	}
	copyMappings(e) {
		var t = {};
	...
	process(t, n) {
		n ||= {};
		var r;
		t.nodeType === Z.DOCUMENT_NODE ? (t.baseURI && !n.baseURI && (n.baseURI = t.baseURI), r = t.baseURI, t = t.documentElement, t.baseURI || (t.baseURI = r), this.setContext(t)) : t.parentNode.nodeType === Z.DOCUMENT_NODE && this.setContext(t);
		var i = [], a = function(e) {
		...
		for (i.push({
			...
			for (var de = l.lastChild; de; de = de.previousSibling) de.nodeType === Z.ELEMENT_NODE && i.unshift({
				current: de,
				context: le
			});
		}
...
ar.XMLLiteralURI = "http://www.w3.org/1999/02/22-rdf-syntax-ns#XMLLiteral", ar.HTMLLiteralURI = "http://www.w3.org/1999/02/22-rdf-syntax-ns#HTML", ar.PlainLiteralURI = "http://www.w3.org/1999/02/22-rdf-syntax-ns#PlainLiteral", ar.objectURI = "http://www.w3.org/1999/02/22-rdf-syntax-ns#object", ar.typeURI = "http://www.w3.org/1999/02/22-rdf-syntax-ns#type", ar.nameChar = "[-A-Z_a-zÀ-ÖØ-öø-˿Ͱ-ͽͿ-῿‌-‍⁰-↏Ⰰ-⿯、-퟿豈-﷏ﷰ-�က0-F.0-9·̀-ͯ‿-⁀]", ar.nameStartChar = "[A-Za-zÀ-ÖØ-öø-ÿĀ-ıĴ-ľŁ-ňŊ-žƀ-ǃǍ-ǰǴ-ǵǺ-ȗɐ-ʨʻ-ˁΆΈ-ΊΌΎ-ΡΣ-ώϐ-ϖϚϜϞϠϢ-ϳЁ-ЌЎ-яё-ќў-ҁҐ-ӄӇ-ӈӋ-ӌӐ-ӫӮ-ӵӸ-ӹԱ-Ֆՙա-ֆא-תװ-ײء-غف-يٱ-ڷں-ھۀ-ێې-ۓەۥ-ۦअ-हऽक़-ॡঅ-ঌএ-ঐও-নপ-রলশ-হড়-ঢ়য়-ৡৰ-ৱਅ-ਊਏ-ਐਓ-ਨਪ-ਰਲ-ਲ਼ਵ-ਸ਼ਸ-ਹਖ਼-ੜਫ਼ੲ-ੴઅ-ઋઍએ-ઑઓ-નપ-રલ-ળવ-હઽૠଅ-ଌଏ-ଐଓ-ନପ-ରଲ-ଳଶ-ହଽଡ଼-ଢ଼ୟ-ୡஅ-ஊஎ-ஐஒ-கங-சஜஞ-டண-தந-பம-வஷ-ஹఅ-ఌఎ-ఐఒ-నప-ళవ-హౠ-ౡಅ-ಌಎ-ಐಒ-ನಪ-ಳವ-ಹೞೠ-ೡഅ-ഌഎ-ഐഒ-നപ-ഹൠ-ൡก-ฮะา-ำเ-ๅກ-ຂຄງ-ຈຊຍດ-ທນ-ຟມ-ຣລວສ-ຫອ-ຮະາ-ຳຽເ-ໄཀ-ཇཉ-ཀྵႠ-Ⴥა-ჶᄀᄂ-ᄃᄅ-ᄇᄉᄋ-ᄌᄎ-ᄒᄼᄾᅀᅌᅎᅐᅔ-ᅕᅙᅟ-ᅡᅣᅥᅧᅩᅭ-ᅮᅲ-ᅳᅵᆞᆨᆫᆮ-ᆯᆷ-ᆸᆺᆼ-ᇂᇫᇰᇹḀ-ẛẠ-ỹἀ-ἕἘ-Ἕἠ-ὅὈ-Ὅὐ-ὗὙὛὝὟ-ώᾀ-ᾴᾶ-ᾼιῂ-ῄῆ-ῌῐ-ΐῖ-Ίῠ-Ῥῲ-ῴῶ-ῼΩK-Å℮ↀ-ↂぁ-ゔァ-ヺㄅ-ㄬ가-힣一-龥〇〡-〩_]", ar.NCNAME = RegExp("^" + ar.nameStartChar + ar.nameChar + "*$"), ar.dateTimeTypes = [
	{
...
var or = ar.parseRDFaDOM, sr = class e {
	constructor(e) {
...
function lr(e, t, n, r = "text/turtle", i) {
	r ||= "text/turtle", r = r.split(";")[0];
	try {
		if (r === "text/n3" || r === "text/turtle") hn(t, t, n, n, null, null, "", null).loadBuf(e), o();
		else if (r === "application/rdf+xml") new sr(t).parse(ot(e), n, t.sym(n)), o();
		else if (r === "application/xhtml+xml") or(ot(e, { contentType: M }), t, n), o();
		else if (r === "text/html") or(ot(e, { contentType: D }), t, n), o();
		else if (r === "application/sparql-update" || r === "application/sparql-update-single-match") cr(e, t, n), o();
		else if (r === "application/ld+json") Sn(e, t, n).then(o).catch(s);
		else if (r === "application/n-quads" || r === "application/nquads") {
			var a = new nr({ factory: vn });
			c(null, e);
...
var ur = /* @__PURE__ */ u((/* @__PURE__ */ s(((e, t) => {
...
var gr = class extends hr {
	...
	parse(e, t, n) {
		...
		return n.noMeta || r.add(n.original, mr.rdf("type"), mr.link("RDFDocument"), e.appNode), e.doneFetch(n, this.response);
	}
};
gr.pattern = /* @__PURE__ */ RegExp("application/rdf\\+xml");
var _r = class extends hr {
	...
	parse(e, t, n) {
		...
		o.length > 0 && a.add(n.resource, mr.dc("title"), a.rdfFactory.literal(o[0].textContent), n.resource);
		...
		for (let e = 0; e < c.length; e++) {
			...
			dr[t] && (lr(c[e].textContent, a, n.original.value, t), lr(c[e].textContent, a, n.original.value, t));
		}
		if (n.noMeta || a.add(n.resource, mr.rdf("type"), mr.link("WebPage"), e.appNode), !n.noRDFa && or) try {
			or(this.dom, a, n.original.value);
		} catch (t) {
...
_r.pattern = /* @__PURE__ */ RegExp("application/xhtml");
var vr = class e extends hr {
	static toString() {
		return "XMLHandler";
	}
	static register(e) {
...
vr.pattern = /* @__PURE__ */ RegExp("(text|application)/(.*)xml");
var yr = class extends hr {
	...
	parse(e, t, n) {
		let r = e.store;
		if (Tr(t)) return e.addStatus(n.req, "Has an XML declaration. We'll assume it's XHTML as the content-type was text/html.\n"), new _r(this.response).parse(e, t, n);
		if (wr(t)) return e.addStatus(n.req, "Has XHTML DOCTYPE. Switching to XHTMLHandler.\n"), new _r(this.response).parse(e, t, n);
		if (Er(t)) return e.addStatus(n.req, "Has default namespace for XHTML, so switching to XHTMLHandler.\n"), new _r(this.response).parse(e, t, n);
		let i = (/* @__PURE__ */ RegExp("<title>([\\s\\S]+?)</title>", "im")).exec(t);
		return i && r.add(n.resource, mr.dc("title"), r.rdfFactory.literal(i[1]), n.resource), r.add(n.resource, mr.rdf("type"), mr.link("WebPage"), e.appNode), e.addStatus(n.req, "non-XML HTML document, not parsed for data."), e.doneFetch(n, this.response);
	}
};
yr.pattern = /* @__PURE__ */ RegExp("text/html");
var br = class extends hr {
	...
	async parse(e, t, n, r) {
		let i = e.store;
		try {
			return await Sn(t, i, n.original.value), e.store.add(n.original, mr.rdf("type"), mr.link("RDFDocument"), e.appNode), e.doneFetch(n, r);
		} catch (t) {
...
br.pattern = /application\/(ld\+json|activity\+json)/;
var xr = class extends hr {
	...
	parse(e, t, n) {
		return Tr(t) ? (e.addStatus(n.req, "Warning: " + n.resource + " has an XML declaration. We'll assume it's XML but its content-type wasn't XML.\n"), new vr(this.response).parse(e, t, n)) : t.slice(0, 500).match(/xmlns:/) ? (e.addStatus(n.req, "May have an XML namespace. We'll assume it's XML but its content-type wasn't XML.\n"), new vr(this.response).parse(e, t, n)) : (e.addStatus(n.req, "Plain text document, no known RDF semantics."), e.doneFetch(n, this.response));
	}
};
xr.pattern = /* @__PURE__ */ RegExp("text/plain");
var Sr = class extends hr {
	...
	parse(e, t, n, r) {
		...
		return e.addStatus(n.req, "N3 parsed: " + a.statementCount + " triples in " + a.lines + " lines."), e.store.add(n.original, mr.rdf("type"), mr.link("RDFDocument"), e.appNode), e.doneFetch(n, this.response);
	}
};
Sr.pattern = /* @__PURE__ */ RegExp("(application|text)/(x-)?(rdf\\+)?(n3|turtle)");
var Cr = {
	RDFXMLHandler: gr,
	XHTMLHandler: _r,
	XMLHandler: vr,
...
var Dr = class e {
	constructor(e, t = {}) {
		g(this, "store", void 0), g(this, "timeout", void 0), g(this, "_fetch", void 0), g(this, "mediatypes", void 0), g(this, "appNode", void 0), g(this, "requested", void 0), g(this, "timeouts", void 0), g(this, "redirectedTo", void 0), g(this, "fetchQueue", void 0), g(this, "fetchCallbacks", void 0), g(this, "nonexistent", void 0), g(this, "lookedUp", void 0), g(this, "handlers", void 0), g(this, "ns", void 0), g(this, "fireCallbacks", void 0), this.store = e || new At(), this.ns = pr(this.store.rdfFactory), this.timeout = t.timeout || 3e4;
		let n = t.fetch || typeof global < "u" && (global.solidFetcher || global.solidFetch) || typeof window < "u" && (window.solidFetcher || window.solidFetch) || ur.default;
		...
		this._fetch = typeof window < "u" && n === window.fetch ? n.bind(window) : typeof global < "u" && n === global.fetch ? n.bind(global) : n, this.appNode = this.store.sym("chrome://TheCurrentSession"), this.store.fetcher = this, this.requested = {}, this.timeouts = {}, this.redirectedTo = {}, this.fetchQueue = {}, this.fetchCallbacks = {}, this.nonexistent = {}, this.lookedUp = {}, this.handlers = [], this.mediatypes = {
			...
			"retract",
			"done"
		]), Object.keys(t.handlers || Cr).map((e) => this.addHandler(Cr[e]));
	}
...
g(Dr, "HANDLERS", void 0), g(Dr, "CONTENT_TYPE_BY_EXT", void 0), g(Dr, "crossSiteProxyTemplate", void 0), Dr.HANDLERS = Cr, Dr.CONTENT_TYPE_BY_EXT = fr;
//#endregion
...
var jr = class {
	...
	updateLocalFile(e, t, n, r, i = {}) {
		...
		let l = Dr.CONTENT_TYPE_BY_EXT[c];
	...
	onUpdate(e, t) {
		return this.fetcher.refresh(Mr.namedNode(e));
	}
	register(e, t) {
		return this.via[e] ?? (this.via[e] = new Nr(this, e)), this.via[e].subscribe(t);
	}
}, Fr = /* @__PURE__ */ c({
	BlankNode: () => N,
	Collection: () => _e,
...
for (let e in Mr) typeof Mr[e] == "function" && (Ir[e] = Mr[e].bind(Mr));
var { fetcher: Lr, graph: Rr, lit: zr, st: Br, namedNode: Q, variable: Vr, blankNode: Hr, defaultGraph: Ur, literal: Wr, quad: Gr, triple: Kr } = Ir, qr = new xt(), Jr = (e) => qr.fromNT(e), Yr = St.fromValue, Xr = N.nextId, Zr = class extends At {
	constructor(e) {
		super(e), g(this, "fetcher", void 0), this.fetcher = new Dr(this, {});
	}
}, Qr = class extends Zr {
	constructor(e) {
		super(e), g(this, "updater", void 0), this.updater = new jr(this);
	}
}, $r = (0, f.default)(Fr), ei = (0, f.default)(Fr), $ = Q("http://www.iana.org/assignments/link-relations/acl");
function ti(e) {
	let t = ei;
	async function n(t) {
	...
	function a(e, n, r, i = {}) {
		let a = i.public || [], o = Rr(), s = Ie("http://www.w3.org/ns/auth/acl#"), c = o.sym(`${r}#a1`), l = o.sym(r), u = o.sym(e);
...
function ai() {
	...
	if (e !== void 0 && e.username) return ni("Assuming the user is " + e.username), Q(e.username);
	if (typeof document < "u" && document.location && ("" + document.location).slice(0, 16) === "http://localhost") {
		...
		return t ? (ni("Assuming user is " + t), Q(t)) : null;
	}
	return null;
}
//#endregion
...
}, si = class extends oi {}, ci = class extends oi {}, li = class extends oi {}, ui = class extends oi {}, di = class extends oi {}, fi = class extends oi {
	constructor(e, t) {
...
function gi(e) {
	return Q(e.uri.replace("/profile/", "/").replace("/public/", "/").split("/").slice(0, -1).join("/") + "/settings/prefs.ttl");
}
function _i(e, t) {
...
function bi(e, t, n, r) {
	...
	async function s(t, a) {
		...
		try {
			...
			if (n) s = n;
				...
				try {
					...
					ri(`User ${t} has no usable profile document directory for publicTypeIndex.`);
				}
				e && (s = await r.followOrCreateLinkWithContentOnCreate(t, i.solid("publicTypeIndex"), e, o, vi()));
			}
		} catch (e) {
			ri(`User ${t} has no pointer in profile to publicTypeIndex file: ${e}`);
		}
		let c = s ? [{
			label: "public",
			index: s,
		...
		if (l) {
			...
			if (a) try {
				...
				ri(`User ${t} has no usable preferences document directory for privateTypeIndex.`);
			}
			let s = null;
			try {
				let c = e.any(t, i.solid("privateTypeIndex"), void 0, o);
				c ? s = c : a && n && (s = await r.followOrCreateLinkWithContentOnCreate(t, i.solid("privateTypeIndex"), n, l, yi()));
			} catch (e) {
				ri(`User ${t} has no pointer in preference file to privateTypeIndex file: ${e}`);
			}
			u = s ? [{
				label: "private",
				index: s,
		...
		try {
			...
			ri("Problems loading type index: ", e);
		}
		return d;
	}
	async function c(e) {
	...
	async function u(t) {
		...
		try {
			...
			ri(`User ${t} has no pointer in profile to preferences file.`);
		}
		if (r) {
			...
			for (let e of n) {
				if (e.termType !== "NamedNode" || !a(e.uri)) {
					ri(`Skipping malformed community node for ${t}: ${e}`);
					continue;
				}
				try {
					o = o.concat(await c(e));
				} catch (t) {
					ri(`Skipping community type indexes for ${e.uri}: ${t}`);
				}
			}
			return o;
		}
	...
	function m(e) {
		...
		let r = t.uri;
		if (!r || !a(r)) return ni(`docDirUri: missing or non-http(s) doc uri for ${e?.uri}`), null;
		let i = r.split("#")[0], o = i.lastIndexOf("/");
		return o === -1 ? (ni(`docDirUri: no slash in doc uri ${r}`), null) : i.slice(0, o + 1);
	}
	function h(e) {
		let t = m(e);
		if (!t) throw Error(`suggestPublicTypeIndex: Cannot derive directory for ${e.uri}`);
		return Q(t + "publicTypeIndex.ttl");
	}
	function g(e) {
		let t = m(e);
		if (!t) throw Error(`suggestPrivateTypeIndex: Cannot derive directory for ${e.uri}`);
		return Q(t + "privateTypeIndex.ttl");
	}
	async function _(t, n, r) {
		let a = pi(n), o = [
			Br(a, i.rdf("type"), i.solid("TypeRegistration"), n),
			Br(a, i.solid("forClass"), r, n),
			Br(a, i.solid("instance"), t, n)
		];
		try {
...
var xi = crypto, Si = (e) => e instanceof CryptoKey, Ci = async (e, t) => {
	let n = `SHA-${e.slice(-3)}`;
	return new Uint8Array(await xi.subtle.digest(n, t));
}, wi = new TextEncoder(), Ti = new TextDecoder();
function Ei(...e) {
...
var Di = (e) => {
	let t = e;
	typeof t == "string" && (t = wi.encode(t));
	let n = 32768, r = [];
	for (let e = 0; e < t.length; e += n) r.push(String.fromCharCode.apply(null, t.subarray(e, e + n)));
	return btoa(r.join(""));
}, Oi = (e) => Di(e).replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_"), ki = (e) => {
	let t = atob(e), n = new Uint8Array(t.length);
	for (let e = 0; e < t.length; e++) n[e] = t.charCodeAt(e);
	return n;
}, Ai = (e) => {
	let t = e;
	t instanceof Uint8Array && (t = Ti.decode(t)), t = t.replace(/-/g, "+").replace(/_/g, "/").replace(/\s/g, "");
	try {
		return ki(t);
	} catch {
...
Mi.code = "ERR_JWT_CLAIM_VALIDATION_FAILED";
...
Pi.code = "ERR_JOSE_ALG_NOT_ALLOWED";
var Fi = class extends ji {
	constructor() {
		super(...arguments), this.code = "ERR_JOSE_NOT_SUPPORTED";
	}
};
Fi.code = "ERR_JOSE_NOT_SUPPORTED";
var Ii = class extends ji {
...
Li.code = "ERR_JWE_INVALID";
var Ri = class extends ji {
	constructor() {
		super(...arguments), this.code = "ERR_JWS_INVALID";
	}
};
Ri.code = "ERR_JWS_INVALID";
var zi = class extends ji {
	constructor() {
		super(...arguments), this.code = "ERR_JWT_INVALID";
	}
};
zi.code = "ERR_JWT_INVALID";
var Bi = class extends ji {
	constructor() {
		super(...arguments), this.code = "ERR_JWK_INVALID";
	}
};
Bi.code = "ERR_JWK_INVALID";
var Vi = class extends ji {
	constructor() {
...
Gi.code = "ERR_JWS_SIGNATURE_VERIFICATION_FAILED";
function Ki(e, t = "algorithm.name") {
...
function Zi(e, t, ...n) {
	switch (t) {
		...
		case "HS384":
		case "HS512": {
			if (!qi(e.algorithm, "HMAC")) throw Ki("HMAC");
			let n = parseInt(t.slice(2), 10);
			if (Ji(e.algorithm.hash) !== n) throw Ki(`SHA-${n}`, "algorithm.hash");
			break;
		}
		case "RS256":
		case "RS384":
		case "RS512": {
			if (!qi(e.algorithm, "RSASSA-PKCS1-v1_5")) throw Ki("RSASSA-PKCS1-v1_5");
			let n = parseInt(t.slice(2), 10);
			if (Ji(e.algorithm.hash) !== n) throw Ki(`SHA-${n}`, "algorithm.hash");
			break;
		}
		...
		case "PS384":
		case "PS512": {
			if (!qi(e.algorithm, "RSA-PSS")) throw Ki("RSA-PSS");
			let n = parseInt(t.slice(2), 10);
			if (Ji(e.algorithm.hash) !== n) throw Ki(`SHA-${n}`, "algorithm.hash");
			break;
		}
		case "EdDSA":
			if (e.algorithm.name !== "Ed25519" && e.algorithm.name !== "Ed448") throw Ki("Ed25519 or Ed448");
			break;
		case "Ed25519":
			if (!qi(e.algorithm, "Ed25519")) throw Ki("Ed25519");
			break;
		case "ES256":
		case "ES384":
		case "ES512": {
			if (!qi(e.algorithm, "ECDSA")) throw Ki("ECDSA");
			let n = Yi(t);
			if (e.algorithm.namedCurve !== n) throw Ki(n, "algorithm.namedCurve");
			break;
		}
...
var $i = (e, ...t) => Qi("Key must be ", e, ...t);
function ea(e, t, ...n) {
	return Qi(`Key for the ${e} algorithm must be `, t, ...n);
}
var ta = (e) => Si(e) ? !0 : e?.[Symbol.toStringTag] === "KeyObject", na = ["CryptoKey"], ra = (...e) => {
...
function aa(e) {
	if (!ia(e) || Object.prototype.toString.call(e) !== "[object Object]") return !1;
...
function sa(e) {
	return aa(e) && typeof e.kty == "string";
}
function ca(e) {
...
function ua(e) {
	return sa(e) && e.kty === "oct" && typeof e.k == "string";
}
function da(e) {
	let t, n;
	switch (e.kty) {
		case "RSA":
			switch (e.alg) {
				...
				case "RSA-OAEP-512":
					...
					break;
				default: throw new Fi("Invalid or unsupported JWK \"alg\" (Algorithm) Parameter value");
			}
			break;
		case "EC":
			switch (e.alg) {
		...
		case "OKP":
			...
			break;
		default: throw new Fi("Invalid or unsupported JWK \"kty\" (Key Type) Parameter value");
	}
	return {
		algorithm: t,
		keyUsages: n
...
var fa = async (e) => {
	...
	let { algorithm: t, keyUsages: n } = da(e), r = [
		t,
		e.ext ?? !1,
		e.key_ops ?? n
	], i = { ...e };
	return delete i.alg, delete i.use, xi.subtle.importKey("jwk", i, ...r);
}, pa = (e) => Ai(e), ma, ha, ga = (e) => e?.[Symbol.toStringTag] === "KeyObject", _a = async (e, t, n, r, i = !1) => {
	let a = e.get(t);
	if (a?.[r]) return a[r];
	let o = await fa({
		...n,
	...
	normalizePublicKey: (e, t) => {
		if (ga(e)) {
			...
			return delete n.d, delete n.dp, delete n.dq, delete n.p, delete n.q, delete n.qi, n.k ? pa(n.k) : (ha ||= /* @__PURE__ */ new WeakMap(), _a(ha, e, n, t));
		}
		return sa(e) ? e.k ? Ai(e.k) : (ha ||= /* @__PURE__ */ new WeakMap(), _a(ha, e, e, t, !0)) : e;
	},
	normalizePrivateKey: (e, t) => {
		if (ga(e)) {
			let n = e.export({ format: "jwk" });
			return n.k ? pa(n.k) : (ma ||= /* @__PURE__ */ new WeakMap(), _a(ma, e, n, t));
		}
		return sa(e) ? e.k ? Ai(e.k) : (ma ||= /* @__PURE__ */ new WeakMap(), _a(ma, e, e, t, !0)) : e;
	}
};
async function ya(e, t) {
	if (!aa(e)) throw TypeError("JWK must be an object");
	switch (t ||= e.alg, e.kty) {
		case "oct":
			if (typeof e.k != "string" || !e.k) throw TypeError("missing \"k\" (Key Value) Parameter value");
			return Ai(e.k);
		case "RSA": if ("oth" in e && e.oth !== void 0) throw new Fi("RSA JWK \"oth\" (Other Primes Info) Parameter value is not supported");
		case "EC":
		case "OKP": return fa({
			...e,
			alg: t
		});
		default: throw new Fi("Unsupported \"kty\" (Key Type) Parameter value");
	}
}
var ba = (e) => e?.[Symbol.toStringTag], xa = (e, t, n) => {
	...
	if (!(t instanceof Uint8Array)) {
		...
		if (!ta(t)) throw TypeError(ea(e, t, ...na, "Uint8Array", r ? "JSON Web Key" : null));
		if (t.type !== "secret") throw TypeError(`${ba(t)} instances for symmetric algorithms must be of type "secret"`);
	}
}, Ca = (e, t, n, r) => {
	if (r && sa(t)) switch (n) {
		case "sign":
			if (ca(t) && xa(e, t, n)) return;
			throw TypeError("JSON Web Key for this operation be a private JWK");
		case "verify":
			if (la(t) && xa(e, t, n)) return;
			throw TypeError("JSON Web Key for this operation be a public JWK");
	}
	if (!ta(t)) throw TypeError(ea(e, t, ...na, r ? "JSON Web Key" : null));
	if (t.type === "secret") throw TypeError(`${ba(t)} instances for asymmetric algorithms must not be of type "secret"`);
	if (n === "sign" && t.type === "public") throw TypeError(`${ba(t)} instances for asymmetric algorithm signing must be of type "private"`);
	if (n === "decrypt" && t.type === "public") throw TypeError(`${ba(t)} instances for asymmetric algorithm decryption must be of type "private"`);
	if (t.algorithm && n === "verify" && t.type === "private") throw TypeError(`${ba(t)} instances for asymmetric algorithm verifying must be of type "public"`);
	if (t.algorithm && n === "encrypt" && t.type === "private") throw TypeError(`${ba(t)} instances for asymmetric algorithm encryption must be of type "public"`);
};
function wa(e, t, n, r) {
	t.startsWith("HS") || t === "dir" || t.startsWith("PBES2") || /^A\d{3}(?:GCM)?KW$/.test(t) ? Sa(t, n, r, e) : Ca(t, n, r, e);
}
wa.bind(void 0, !1);
var Ta = wa.bind(void 0, !0);
function Ea(e, t, n, r, i) {
	...
	for (let t of r.crit) {
		if (!a.has(t)) throw new Fi(`Extension Header Parameter "${t}" is not recognized`);
...
var Da = (e, t) => {
	...
	if (!Si(e)) throw TypeError($i(e, ...na, "Uint8Array"));
	if (!e.extractable) throw TypeError("non-extractable CryptoKey cannot be exported as a JWK");
	let { ext: t, key_ops: n, alg: r, use: i, ...a } = await xi.subtle.exportKey("jwk", e);
	return a;
};
async function ka(e) {
	return Oa(e);
...
function Aa(e, t) {
	...
	switch (e) {
		...
		default: throw new Fi(`alg ${e} is not supported either by JOSE or your javascript runtime`);
	}
}
async function ja(e, t, n) {
	if (n === "sign" && (t = await va.normalizePrivateKey(t, e)), n === "verify" && (t = await va.normalizePublicKey(t, e)), Si(t)) return Zi(t, e, n), t;
	if (t instanceof Uint8Array) {
		if (!e.startsWith("HS")) throw TypeError($i(t, ...na));
		return xi.subtle.importKey("raw", t, {
			hash: `SHA-${e.slice(-3)}`,
			name: "HMAC"
		}, !1, [n]);
	}
	throw TypeError($i(t, ...na, "Uint8Array", "JSON Web Key"));
}
var Ma = async (e, t, n, r) => {
	let i = await ja(e, t, "verify");
	oa(e, i);
	let a = Aa(e, i.algorithm);
	try {
		return await xi.subtle.verify(a, i, n, r);
	} catch {
		return !1;
	}
};
async function Na(e, t, n) {
	if (!aa(e)) throw new Ri("Flattened JWS must be an object");
	if (e.protected === void 0 && e.header === void 0) throw new Ri("Flattened JWS must have either of the \"protected\" or \"header\" members");
	if (e.protected !== void 0 && typeof e.protected != "string") throw new Ri("JWS Protected Header incorrect type");
	if (e.payload === void 0) throw new Ri("JWS Payload missing");
	if (typeof e.signature != "string") throw new Ri("JWS Signature missing or incorrect type");
	if (e.header !== void 0 && !aa(e.header)) throw new Ri("JWS Unprotected Header incorrect type");
	let r = {};
	if (e.protected) try {
		let t = Ai(e.protected);
		r = JSON.parse(Ti.decode(t));
	} catch {
		throw new Ri("JWS Protected Header is invalid");
	}
	if (!ra(r, e.header)) throw new Ri("JWS Protected and JWS Unprotected Header Parameter names must be disjoint");
	let i = {
		...r,
		...e.header
	}, a = Ea(Ri, /* @__PURE__ */ new Map([["b64", !0]]), n?.crit, r, i), o = !0;
	if (a.has("b64") && (o = r.b64, typeof o != "boolean")) throw new Ri("The \"b64\" (base64url-encode payload) Header Parameter must be a boolean");
	let { alg: s } = i;
	if (typeof s != "string" || !s) throw new Ri("JWS \"alg\" (Algorithm) Header Parameter missing or invalid");
	let c = n && Da("algorithms", n.algorithms);
	if (c && !c.has(s)) throw new Pi("\"alg\" (Algorithm) Header Parameter value not allowed");
	if (o) {
		if (typeof e.payload != "string") throw new Ri("JWS Payload must be a string");
	} else if (typeof e.payload != "string" && !(e.payload instanceof Uint8Array)) throw new Ri("JWS Payload must be a string or an Uint8Array instance");
	let l = !1;
	typeof t == "function" ? (t = await t(r, e), l = !0, Ta(s, t, "verify"), sa(t) && (t = await ya(t, s))) : Ta(s, t, "verify");
	let u = Ei(wi.encode(e.protected ?? ""), wi.encode("."), typeof e.payload == "string" ? wi.encode(e.payload) : e.payload), d;
	try {
		d = Ai(e.signature);
	} catch {
		throw new Ri("Failed to base64url decode the signature");
	}
	if (!await Ma(s, t, d, u)) throw new Gi();
	let f;
	if (o) try {
		f = Ai(e.payload);
	} catch {
	...
	else f = typeof e.payload == "string" ? wi.encode(e.payload) : e.payload;
	let p = { payload: f };
...
async function Pa(e, t, n) {
	if (e instanceof Uint8Array && (e = Ti.decode(e)), typeof e != "string") throw new Ri("Compact JWS must be a string or Uint8Array");
	let { 0: r, 1: i, 2: a, length: o } = e.split(".");
	if (o !== 3) throw new Ri("Invalid Compact JWS");
	let s = await Na({
		payload: i,
		protected: r,
		signature: a
...
var Fa = (e) => Math.floor(e.getTime() / 1e3), Ia = 60, La = 3600, Ra = La * 24, za = Ra * 7, Ba = Ra * 365.25, Va = /^(\+|\-)? ?(\d+|\d+\.\d+) ?(seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|years?|yrs?|y)(?: (ago|from now))?$/i, Ha = (e) => {
	let t = Va.exec(e);
	...
	switch (r) {
		...
		case "w":
			i = Math.round(n * za);
			break;
		default: i = Math.round(n * Ba);
	}
	return t[1] === "-" || t[4] === "ago" ? -i : i;
}, Ua = (e) => e.toLowerCase().replace(/^application\//, ""), Wa = (e, t) => typeof e == "string" ? t.includes(e) : Array.isArray(e) ? t.some(Set.prototype.has.bind(new Set(e))) : !1, Ga = (e, t, n = {}) => {
	let r;
	try {
		r = JSON.parse(Ti.decode(t));
	} catch {}
	if (!aa(r)) throw new zi("JWT Claims Set must be a top-level JSON object");
	let { typ: i } = n;
	if (i && (typeof e.typ != "string" || Ua(e.typ) !== Ua(i))) throw new Mi("unexpected \"typ\" JWT header value", r, "typ", "check_failed");
	...
	for (let e of new Set(u.reverse())) if (!(e in r)) throw new Mi(`missing required "${e}" claim`, r, e, "missing");
	if (o && !(Array.isArray(o) ? o : [o]).includes(r.iss)) throw new Mi("unexpected \"iss\" claim value", r, "iss", "check_failed");
	if (s && r.sub !== s) throw new Mi("unexpected \"sub\" claim value", r, "sub", "check_failed");
	if (c && !Wa(r.aud, typeof c == "string" ? [c] : c)) throw new Mi("unexpected \"aud\" claim value", r, "aud", "check_failed");
	let d;
	switch (typeof n.clockTolerance) {
		case "string":
			d = Ha(n.clockTolerance);
			break;
		case "number":
			d = n.clockTolerance;
			break;
	...
	let { currentDate: f } = n, p = Fa(f || /* @__PURE__ */ new Date());
	if ((r.iat !== void 0 || l) && typeof r.iat != "number") throw new Mi("\"iat\" claim must be a number", r, "iat", "invalid");
	if (r.nbf !== void 0) {
		if (typeof r.nbf != "number") throw new Mi("\"nbf\" claim must be a number", r, "nbf", "invalid");
		if (r.nbf > p + d) throw new Mi("\"nbf\" claim timestamp check failed", r, "nbf", "check_failed");
	}
	if (r.exp !== void 0) {
		if (typeof r.exp != "number") throw new Mi("\"exp\" claim must be a number", r, "exp", "invalid");
		if (r.exp <= p - d) throw new Ni("\"exp\" claim timestamp check failed", r, "exp", "check_failed");
	}
	if (l) {
		let e = p - r.iat, t = typeof l == "number" ? l : Ha(l);
		if (e - d > t) throw new Ni("\"iat\" claim timestamp check failed (too far in the past)", r, "iat", "check_failed");
		if (e < 0 - d) throw new Mi("\"iat\" claim timestamp check failed (it should be in the past)", r, "iat", "check_failed");
	}
	return r;
};
async function Ka(e, t, n) {
	let r = await Pa(e, t, n);
	if (r.protectedHeader.crit?.includes("b64") && r.protectedHeader.b64 === !1) throw new zi("JWTs MUST NOT use unencoded payload");
	let i = {
		payload: Ga(r.protectedHeader, r.payload, n),
...
var qa = async (e, t, n) => {
	let r = await ja(e, t, "sign");
	oa(e, r);
	let i = await xi.subtle.sign(Aa(e, r.algorithm), r, n);
	...
	async sign(e, t) {
		if (!this._protectedHeader && !this._unprotectedHeader) throw new Ri("either setProtectedHeader or setUnprotectedHeader must be called before #sign()");
		if (!ra(this._protectedHeader, this._unprotectedHeader)) throw new Ri("JWS Protected and JWS Unprotected Header Parameter names must be disjoint");
		let n = {
			...this._protectedHeader,
			...this._unprotectedHeader
		}, r = Ea(Ri, /* @__PURE__ */ new Map([["b64", !0]]), t?.crit, this._protectedHeader, n), i = !0;
		if (r.has("b64") && (i = this._protectedHeader.b64, typeof i != "boolean")) throw new Ri("The \"b64\" (base64url-encode payload) Header Parameter must be a boolean");
		let { alg: a } = n;
		if (typeof a != "string" || !a) throw new Ri("JWS \"alg\" (Algorithm) Header Parameter missing or invalid");
		Ta(a, e, "sign");
		let o = this._payload;
		i && (o = wi.encode(Oi(o)));
		let s;
		s = this._protectedHeader ? wi.encode(Oi(JSON.stringify(this._protectedHeader))) : wi.encode("");
		let c = {
			signature: Oi(await qa(a, e, Ei(s, wi.encode("."), o))),
			payload: ""
		};
		return i && (c.payload = Ti.decode(o)), this._unprotectedHeader && (c.header = this._unprotectedHeader), this._protectedHeader && (c.protected = Ti.decode(s)), c;
	}
}, Ya = class {
	constructor(e) {
		this._flattened = new Ja(e);
	}
	setProtectedHeader(e) {
...
var Za = class {
	constructor(e = {}) {
		if (!aa(e)) throw TypeError("JWT Claims Set MUST be an object");
		this._payload = e;
	}
	setIssuer(e) {
	...
	setNotBefore(e) {
		return this._payload = typeof e == "number" ? {
			...this._payload,
			nbf: Xa("setNotBefore", e)
		} : e instanceof Date ? {
			...this._payload,
			nbf: Xa("setNotBefore", Fa(e))
		} : {
			...this._payload,
			nbf: Fa(/* @__PURE__ */ new Date()) + Ha(e)
		}, this;
	}
	setExpirationTime(e) {
		return this._payload = typeof e == "number" ? {
			...this._payload,
			exp: Xa("setExpirationTime", e)
		} : e instanceof Date ? {
			...this._payload,
			exp: Xa("setExpirationTime", Fa(e))
		} : {
			...this._payload,
			exp: Fa(/* @__PURE__ */ new Date()) + Ha(e)
		}, this;
	}
	setIssuedAt(e) {
		return this._payload = e === void 0 ? {
			...this._payload,
			iat: Fa(/* @__PURE__ */ new Date())
		} : e instanceof Date ? {
			...this._payload,
			iat: Xa("setIssuedAt", Fa(e))
		} : typeof e == "string" ? {
			...this._payload,
			iat: Xa("setIssuedAt", Fa(/* @__PURE__ */ new Date()) + Ha(e))
		} : {
			...this._payload,
			iat: Xa("setIssuedAt", e)
		}, this;
	}
}, Qa = class extends Za {
	setProtectedHeader(e) {
	...
	async sign(e, t) {
		let n = new Ya(wi.encode(JSON.stringify(this._payload)));
		if (n.setProtectedHeader(this._protectedHeader), Array.isArray(this._protectedHeader?.crit) && this._protectedHeader.crit.includes("b64") && this._protectedHeader.b64 === !1) throw new zi("JWTs MUST NOT use unencoded payload");
		return n.sign(e, t);
	}
}, $a = (e, t) => {
	if (typeof e != "string" || !e) throw new Bi(`${t} missing or invalid`);
};
async function eo(e, t) {
	...
	switch (e.kty) {
		case "EC":
			$a(e.crv, "\"crv\" (Curve) Parameter"), $a(e.x, "\"x\" (X Coordinate) Parameter"), $a(e.y, "\"y\" (Y Coordinate) Parameter"), n = {
				crv: e.crv,
				kty: e.kty,
				x: e.x,
				y: e.y
			...
			break;
		case "OKP":
			$a(e.crv, "\"crv\" (Subtype of Key Pair) Parameter"), $a(e.x, "\"x\" (Public Key) Parameter"), n = {
				crv: e.crv,
				kty: e.kty,
				x: e.x
			};
			break;
		case "RSA":
			$a(e.e, "\"e\" (Exponent) Parameter"), $a(e.n, "\"n\" (Modulus) Parameter"), n = {
				e: e.e,
				kty: e.kty,
				n: e.n
			};
			break;
		case "oct":
			$a(e.k, "\"k\" (Key Value) Parameter"), n = {
				k: e.k,
				kty: e.kty
			};
			break;
		default: throw new Fi("\"kty\" (Key Type) Parameter missing or unsupported");
	}
	let r = wi.encode(JSON.stringify(n));
	return Oi(await Ci(t, r));
}
function to(e) {
	switch (typeof e == "string" && e.slice(0, 2)) {
		...
		case "ES": return "EC";
		case "Ed": return "OKP";
		default: throw new Fi("Unsupported \"alg\" value for a JSON Web Key Set");
	}
}
function no(e) {
	return e && typeof e == "object" && Array.isArray(e.keys) && e.keys.every(ro);
}
function ro(e) {
	return aa(e);
}
...
var ao = class {
	constructor(e) {
		if (this._cached = /* @__PURE__ */ new WeakMap(), !no(e)) throw new Vi("JSON Web Key Set malformed");
		this._jwks = io(e);
	}
	async getKey(e, t) {
		let { alg: n, kid: r } = {
			...e,
			...t?.header
		}, i = to(n), a = this._jwks.keys.filter((e) => {
			let t = i === e.kty;
		...
		if (s === 0) throw new Hi();
		if (s !== 1) {
			let e = new Ui(), { _cached: t } = this;
			throw e[Symbol.asyncIterator] = async function* () {
				for (let e of a) try {
					yield await oo(t, e, n);
				} catch {}
			}, e;
		}
		return oo(this._cached, o, n);
	}
};
async function oo(e, t, n) {
	...
	if (r[n] === void 0) {
		...
		if (e instanceof Uint8Array || e.type !== "public") throw new Vi("JSON Web Key Set members must be public keys");
		r[n] = e;
	}
	return r[n];
}
function so(e) {
	let t = new ao(e), n = async (e, n) => t.getKey(e, n);
...
var co = async (e, t, n) => {
	...
	if (i !== void 0 && clearTimeout(i), o.status !== 200) throw new ji("Expected 200 OK from the JSON Web Key Set HTTP response");
	try {
		return await o.json();
	} catch {
		throw new ji("Failed to parse the JSON Web Key Set HTTP response as JSON");
	}
};
function lo() {
...
var uo;
(typeof navigator > "u" || !navigator.userAgent?.startsWith?.("Mozilla/5.0 ")) && (uo = "jose/v5.10.0");
var fo = Symbol();
function po(e, t) {
	return !(typeof e != "object" || !e || !("uat" in e) || typeof e.uat != "number" || Date.now() - e.uat >= t || !("jwks" in e) || !aa(e.jwks) || !Array.isArray(e.jwks.keys) || !Array.prototype.every.call(e.jwks.keys, aa));
}
var mo = class {
	constructor(e, t) {
		...
		this._url = new URL(e.href), this._options = {
			agent: t?.agent,
			headers: t?.headers
		}, this._timeoutDuration = typeof t?.timeoutDuration == "number" ? t?.timeoutDuration : 5e3, this._cooldownDuration = typeof t?.cooldownDuration == "number" ? t?.cooldownDuration : 3e4, this._cacheMaxAge = typeof t?.cacheMaxAge == "number" ? t?.cacheMaxAge : 6e5, t?.[fo] !== void 0 && (this._cache = t?.[fo], po(t?.[fo], this._cacheMaxAge) && (this._jwksTimestamp = this._cache.uat, this._local = so(this._cache.jwks)));
	}
	coolingDown() {
	...
	async getKey(e, t) {
		...
		try {
			...
			if (n instanceof Hi && this.coolingDown() === !1) return await this.reload(), this._local(e, t);
			throw n;
		}
	}
	async reload() {
		...
		uo && !e.has("User-Agent") && (e.set("User-Agent", uo), this._options.headers = Object.fromEntries(e.entries())), this._pendingFetch ||= co(this._url, this._timeoutDuration, this._options).then((e) => {
			this._local = so(e), this._cache && (this._cache.uat = Date.now(), this._cache.jwks = e), this._jwksTimestamp = Date.now(), this._pendingFetch = void 0;
		}).catch((e) => {
...
function ho(e, t) {
	let n = new mo(e, t), r = async (e, t) => n.getKey(e, t);
...
var go = Ai;
function _o(e) {
	if (typeof e != "string") throw new zi("JWTs must use Compact JWS serialization, JWT must be a string");
	let { 1: t, length: n } = e.split(".");
	if (n === 5) throw new zi("Only JWTs using Compact JWS serialization can be decoded");
	if (n !== 3) throw new zi("Invalid JWT");
	if (!t) throw new zi("JWTs must contain a payload");
	let r;
	try {
		r = go(t);
	} catch {
		throw new zi("Failed to base64url decode the payload");
	}
	let i;
	try {
		i = JSON.parse(Ti.decode(r));
	} catch {
		throw new zi("Failed to parse the decoded payload as JSON");
	}
	if (!aa(i)) throw new zi("Invalid JWT Claims Set");
	return i;
}
function vo(e) {
	let t = e?.modulusLength ?? 2048;
	if (typeof t != "number" || t < 2048) throw new Fi("Invalid or unsupported modulusLength option provided, 2048 bits or larger keys must be used");
	return t;
}
		...
		case "EdDSA": {
			...
			switch (e) {
				...
				case "Ed448":
					n = { name: e };
					break;
				default: throw new Fi("Invalid or unsupported crv option provided");
			}
			break;
		}
		case "ECDH-ES":
		...
		case "ECDH-ES+A256KW": {
			...
			switch (e) {
				...
				case "X448":
					n = { name: e };
					break;
				default: throw new Fi("Invalid or unsupported crv option provided, supported values are P-256, P-384, P-521, X25519, and X448");
			}
			break;
		}
		default: throw new Fi("Invalid or unsupported JWK \"alg\" (Algorithm) Parameter value");
	}
	return xi.subtle.generateKey(n, t?.extractable ?? !1, r);
}
...
var xo = async (e, t) => {
	...
	if (!l) {
		...
		l = (await xo(e, n ?? { redirect_uris: [i] }).then((e) => {
			if (!e.ok) throw Error(`HTTP error! Status: ${e.status}`);
			return e.json();
		})).client_id;
	}
	...
	let { pkce_code_verifier: u, pkce_code_challenge: d } = await Co();
	...
	let c = await bo("ES256"), l = await To(r, a, n.toString(), o, s, c).then((e) => {
		if (!e.ok) throw Error(`HTTP error! Status: ${e.status}`);
		return e.json();
	...
	let { payload: f } = await Ka(u, ho(new URL(d)), {
		issuer: i,
		audience: "solid"
	}), p = await eo(await ka(c.publicKey));
...
}, To = async (e, t, n, r, i, a) => {
	let o = await ka(a.publicKey);
	o.alg = "ES256";
	let s = await new Qa({
		htu: i,
		htm: "POST"
	...
	try {
		...
		let { payload: l } = await Ka(o, ho(new URL(c)), {
			issuer: s,
			audience: "solid"
		}), u = await eo(await ka(r.publicKey));
...
var ko = class extends EventTarget {
	...
	rejectRefresh;
	constructor(e, t) {
		super(), this.authFetch = this.authFetch.bind(this), this.information = { clientDetails: e }, this.database = t?.database, t?.onSessionStateChange && this.addEventListener(Oo.STATE_CHANGE, (e) => t.onSessionStateChange?.(e)), t?.onSessionExpirationWarning && this.addEventListener(Oo.EXPIRATION_WARNING, (e) => t?.onSessionExpirationWarning?.(e)), t?.onSessionExpiration && this.addEventListener(Oo.EXPIRATION, (e) => t?.onSessionExpiration?.(e));
	}
	async login(e, t) {
		await So(e, t, this.information.clientDetails);
	}
	async handleRedirectFromLogin() {
		let e = await wo(this.information.clientDetails, this.database);
	...
	async restore() {
		...
		let e = this.isActive;
		return Eo(this.database).then((e) => this.setTokenDetails(e)).then(() => this.resolveRefresh()).catch((e) => {
	...
	async _createSignedDPoPToken(e) {
		...
		let t = await ka(this.information.tokenDetails.dpop_key_pair.publicKey);
		return new Qa(e).setIssuedAt().setJti(window.crypto.randomUUID()).setProtectedHeader({
			alg: "ES256",
			typ: "dpop+jwt",
			jwk: t
...
var Mo = class {
	...
	constructor(e, t) {
		...
		this.worker = new SharedWorker(i, { type: "module" }), this.worker.port.onmessage = (e) => {
			...
			this.worker.port.postMessage({ type: jo.DISCONNECT });
		});
	}
	async handleWorkerMessage(e) {
		let { type: t, payload: n, error: r } = e;
		switch (t) {
			case jo.TOKEN_DETAILS:
				let e = this.isActive;
			...
			case jo.ERROR_ON_REFRESH:
				...
				break;
			case jo.EXPIRED: this.isActive && (this.dispatchExpirationEvent(), await this.logout()), this.refreshPromise && this.rejectRefresh && (this.rejectRefresh(Error(r || "Token refresh failed")), this.clearRefreshPromise());
		}
	}
	async handleRedirectFromLogin() {
	...
	async restore() {
		return this.refreshPromise ? this.refreshPromise : (this.refreshPromise = new Promise((e, t) => {
			this.resolveRefresh = e, this.rejectRefresh = t;
		}), this.worker.port.postMessage({ type: jo.REFRESH }), this.refreshPromise);
	}
	async logout() {
		this.worker.port.postMessage({ type: jo.STOP }), await super.logout();
	}
}, Po = /* @__PURE__ */ c({
	SessionCore: () => Hc,
	SessionEvents: () => Vc
}), Fo = crypto, Io = (e) => e instanceof CryptoKey, Lo = async (e, t) => {
	let n = `SHA-${e.slice(-3)}`;
	return new Uint8Array(await Fo.subtle.digest(n, t));
}, Ro = new TextEncoder(), zo = new TextDecoder();
function Bo(...e) {
	let t = e.reduce((e, { length: t }) => e + t, 0), n = new Uint8Array(t), r = 0;
	for (let t of e) n.set(t, r), r += t.length;
	return n;
...
var Vo = (e) => {
	let t = e;
	typeof t == "string" && (t = Ro.encode(t));
	let n = 32768, r = [];
	for (let e = 0; e < t.length; e += n) r.push(String.fromCharCode.apply(null, t.subarray(e, e + n)));
	return btoa(r.join(""));
}, Ho = (e) => Vo(e).replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_"), Uo = (e) => {
	let t = atob(e), n = new Uint8Array(t.length);
	for (let e = 0; e < t.length; e++) n[e] = t.charCodeAt(e);
	return n;
}, Wo = (e) => {
...
Jo.code = "ERR_JOSE_ALG_NOT_ALLOWED";
var Yo = class extends Go {
	constructor() {
		super(...arguments), this.code = "ERR_JOSE_NOT_SUPPORTED";
	}
};
Yo.code = "ERR_JOSE_NOT_SUPPORTED";
var Xo = class extends Go {
	constructor(e = "decryption operation failed", t) {
		super(e, t), this.code = "ERR_JWE_DECRYPTION_FAILED";
	}
};
Xo.code = "ERR_JWE_DECRYPTION_FAILED";
...
var es = class extends Go {
	constructor() {
		super(...arguments), this.code = "ERR_JWK_INVALID";
	}
};
es.code = "ERR_JWK_INVALID";
var ts = class extends Go {
	constructor() {
		super(...arguments), this.code = "ERR_JWKS_INVALID";
	}
};
ts.code = "ERR_JWKS_INVALID";
...
rs.code = "ERR_JWKS_MULTIPLE_MATCHING_KEYS";
var is = class extends Go {
	constructor(e = "request timed out", t) {
		super(e, t), this.code = "ERR_JWKS_TIMEOUT";
	}
};
is.code = "ERR_JWKS_TIMEOUT";
var as = class extends Go {
	constructor(e = "signature verification failed", t) {
		super(e, t), this.code = "ERR_JWS_SIGNATURE_VERIFICATION_FAILED";
	}
};
as.code = "ERR_JWS_SIGNATURE_VERIFICATION_FAILED";
function os(e, t = "algorithm.name") {
	return /* @__PURE__ */ TypeError(`CryptoKey does not support this operation, its ${t} must be ${e}`);
}
function ss(e, t) {
	return e.name === t;
...
function ds(e, t, ...n) {
	switch (t) {
		...
		case "HS384":
		case "HS512": {
			if (!ss(e.algorithm, "HMAC")) throw os("HMAC");
			let n = parseInt(t.slice(2), 10);
			if (cs(e.algorithm.hash) !== n) throw os(`SHA-${n}`, "algorithm.hash");
			break;
		}
		case "RS256":
		case "RS384":
		case "RS512": {
			if (!ss(e.algorithm, "RSASSA-PKCS1-v1_5")) throw os("RSASSA-PKCS1-v1_5");
			let n = parseInt(t.slice(2), 10);
			if (cs(e.algorithm.hash) !== n) throw os(`SHA-${n}`, "algorithm.hash");
			break;
		}
		...
		case "PS384":
		case "PS512": {
			if (!ss(e.algorithm, "RSA-PSS")) throw os("RSA-PSS");
			let n = parseInt(t.slice(2), 10);
			if (cs(e.algorithm.hash) !== n) throw os(`SHA-${n}`, "algorithm.hash");
			break;
		}
		case "EdDSA":
			if (e.algorithm.name !== "Ed25519" && e.algorithm.name !== "Ed448") throw os("Ed25519 or Ed448");
			break;
		case "Ed25519":
			if (!ss(e.algorithm, "Ed25519")) throw os("Ed25519");
			break;
		case "ES256":
		case "ES384":
		case "ES512": {
			if (!ss(e.algorithm, "ECDSA")) throw os("ECDSA");
			let n = ls(t);
			if (e.algorithm.namedCurve !== n) throw os(n, "algorithm.namedCurve");
			break;
		}
		default: throw TypeError("CryptoKey does not support this operation");
	}
...
var ps = (e, ...t) => fs("Key must be ", e, ...t);
function ms(e, t, ...n) {
	return fs(`Key for the ${e} algorithm must be `, t, ...n);
}
var hs = (e) => Io(e) ? !0 : e?.[Symbol.toStringTag] === "KeyObject", gs = ["CryptoKey"], _s = (...e) => {
	let t = e.filter(Boolean);
	if (t.length === 0 || t.length === 1) return !0;
	let n;
	for (let e of t) {
...
function ys(e) {
	if (!vs(e) || Object.prototype.toString.call(e) !== "[object Object]") return !1;
	if (Object.getPrototypeOf(e) === null) return !0;
	let t = e;
	for (; Object.getPrototypeOf(t) !== null;) t = Object.getPrototypeOf(t);
	return Object.getPrototypeOf(e) === t;
...
function xs(e) {
	return ys(e) && typeof e.kty == "string";
}
function Ss(e) {
	return e.kty !== "oct" && typeof e.d == "string";
}
...
function ws(e) {
	return xs(e) && e.kty === "oct" && typeof e.k == "string";
}
function Ts(e) {
	let t, n;
	switch (e.kty) {
		case "RSA":
			switch (e.alg) {
				...
				case "RSA-OAEP-512":
					...
					break;
				default: throw new Yo("Invalid or unsupported JWK \"alg\" (Algorithm) Parameter value");
			}
			break;
		case "EC":
			switch (e.alg) {
		...
		case "OKP":
			...
			break;
		default: throw new Yo("Invalid or unsupported JWK \"kty\" (Key Type) Parameter value");
	}
	return {
		algorithm: t,
		keyUsages: n
...
var Es = async (e) => {
	if (!e.alg) throw TypeError("\"alg\" argument is required when \"jwk.alg\" is not present");
	let { algorithm: t, keyUsages: n } = Ts(e), r = [
		t,
		e.ext ?? !1,
		e.key_ops ?? n
	], i = { ...e };
	return delete i.alg, delete i.use, Fo.subtle.importKey("jwk", i, ...r);
}, Ds = (e) => Wo(e), Os, ks, As = (e) => e?.[Symbol.toStringTag] === "KeyObject", js = async (e, t, n, r, i = !1) => {
	let a = e.get(t);
	if (a?.[r]) return a[r];
	let o = await Es({
		...n,
	...
	normalizePublicKey: (e, t) => {
		if (As(e)) {
			let n = e.export({ format: "jwk" });
			return delete n.d, delete n.dp, delete n.dq, delete n.p, delete n.q, delete n.qi, n.k ? Ds(n.k) : (ks ||= /* @__PURE__ */ new WeakMap(), js(ks, e, n, t));
		}
		return xs(e) ? e.k ? Wo(e.k) : (ks ||= /* @__PURE__ */ new WeakMap(), js(ks, e, e, t, !0)) : e;
	},
	normalizePrivateKey: (e, t) => {
		if (As(e)) {
			let n = e.export({ format: "jwk" });
			return n.k ? Ds(n.k) : (Os ||= /* @__PURE__ */ new WeakMap(), js(Os, e, n, t));
		}
		return xs(e) ? e.k ? Wo(e.k) : (Os ||= /* @__PURE__ */ new WeakMap(), js(Os, e, e, t, !0)) : e;
	}
};
async function Ns(e, t) {
	if (!ys(e)) throw TypeError("JWK must be an object");
	switch (t ||= e.alg, e.kty) {
		case "oct":
			if (typeof e.k != "string" || !e.k) throw TypeError("missing \"k\" (Key Value) Parameter value");
			return Wo(e.k);
		case "RSA": if ("oth" in e && e.oth !== void 0) throw new Yo("RSA JWK \"oth\" (Other Primes Info) Parameter value is not supported");
		case "EC":
		case "OKP": return Es({
			...e,
			alg: t
		});
		default: throw new Yo("Unsupported \"kty\" (Key Type) Parameter value");
	}
}
var Ps = (e) => e?.[Symbol.toStringTag], Fs = (e, t, n) => {
	if (t.use !== void 0 && t.use !== "sig") throw TypeError("Invalid key for this operation, when present its use must be sig");
	if (t.key_ops !== void 0 && t.key_ops.includes?.(n) !== !0) throw TypeError(`Invalid key for this operation, when present its key_ops must include ${n}`);
	if (t.alg !== void 0 && t.alg !== e) throw TypeError(`Invalid key for this operation, when present its alg must be ${e}`);
	return !0;
	...
	if (!(t instanceof Uint8Array)) {
		if (r && xs(t)) {
			if (ws(t) && Fs(e, t, n)) return;
			throw TypeError("JSON Web Key for symmetric algorithms must have JWK \"kty\" (Key Type) equal to \"oct\" and the JWK \"k\" (Key Value) present");
		}
		if (!hs(t)) throw TypeError(ms(e, t, ...gs, "Uint8Array", r ? "JSON Web Key" : null));
		if (t.type !== "secret") throw TypeError(`${Ps(t)} instances for symmetric algorithms must be of type "secret"`);
	}
}, Ls = (e, t, n, r) => {
	if (r && xs(t)) switch (n) {
		case "sign":
			if (Ss(t) && Fs(e, t, n)) return;
			throw TypeError("JSON Web Key for this operation be a private JWK");
		case "verify":
			if (Cs(t) && Fs(e, t, n)) return;
			throw TypeError("JSON Web Key for this operation be a public JWK");
	}
	if (!hs(t)) throw TypeError(ms(e, t, ...gs, r ? "JSON Web Key" : null));
	if (t.type === "secret") throw TypeError(`${Ps(t)} instances for asymmetric algorithms must not be of type "secret"`);
	if (n === "sign" && t.type === "public") throw TypeError(`${Ps(t)} instances for asymmetric algorithm signing must be of type "private"`);
	if (n === "decrypt" && t.type === "public") throw TypeError(`${Ps(t)} instances for asymmetric algorithm decryption must be of type "private"`);
	if (t.algorithm && n === "verify" && t.type === "private") throw TypeError(`${Ps(t)} instances for asymmetric algorithm verifying must be of type "public"`);
	if (t.algorithm && n === "encrypt" && t.type === "private") throw TypeError(`${Ps(t)} instances for asymmetric algorithm encryption must be of type "public"`);
};
function Rs(e, t, n, r) {
	t.startsWith("HS") || t === "dir" || t.startsWith("PBES2") || /^A\d{3}(?:GCM)?KW$/.test(t) ? Is(t, n, r, e) : Ls(t, n, r, e);
}
Rs.bind(void 0, !1);
var zs = Rs.bind(void 0, !0);
function Bs(e, t, n, r, i) {
	...
	a = n === void 0 ? t : new Map([...Object.entries(n), ...t.entries()]);
	for (let t of r.crit) {
		if (!a.has(t)) throw new Yo(`Extension Header Parameter "${t}" is not recognized`);
		if (i[t] === void 0) throw new e(`Extension Header Parameter "${t}" is missing`);
		if (a.get(t) && r[t] === void 0) throw new e(`Extension Header Parameter "${t}" MUST be integrity protected`);
	}
	return new Set(r.crit);
...
var Vs = (e, t) => {
	...
	if (!Io(e)) throw TypeError(ps(e, ...gs, "Uint8Array"));
	if (!e.extractable) throw TypeError("non-extractable CryptoKey cannot be exported as a JWK");
	let { ext: t, key_ops: n, alg: r, use: i, ...a } = await Fo.subtle.exportKey("jwk", e);
	return a;
};
async function Us(e) {
	return Hs(e);
...
function Ws(e, t) {
	...
	switch (e) {
		...
		case "Ed25519": return { name: "Ed25519" };
		case "EdDSA": return { name: t.name };
		default: throw new Yo(`alg ${e} is not supported either by JOSE or your javascript runtime`);
	}
}
async function Gs(e, t, n) {
	if (n === "sign" && (t = await Ms.normalizePrivateKey(t, e)), n === "verify" && (t = await Ms.normalizePublicKey(t, e)), Io(t)) return ds(t, e, n), t;
	if (t instanceof Uint8Array) {
		if (!e.startsWith("HS")) throw TypeError(ps(t, ...gs));
		return Fo.subtle.importKey("raw", t, {
			hash: `SHA-${e.slice(-3)}`,
			name: "HMAC"
		}, !1, [n]);
	}
	throw TypeError(ps(t, ...gs, "Uint8Array", "JSON Web Key"));
}
var Ks = async (e, t, n, r) => {
	let i = await Gs(e, t, "verify");
	bs(e, i);
	let a = Ws(e, i.algorithm);
	try {
		return await Fo.subtle.verify(a, i, n, r);
	} catch {
		return !1;
	}
};
async function qs(e, t, n) {
	if (!ys(e)) throw new Qo("Flattened JWS must be an object");
	if (e.protected === void 0 && e.header === void 0) throw new Qo("Flattened JWS must have either of the \"protected\" or \"header\" members");
	if (e.protected !== void 0 && typeof e.protected != "string") throw new Qo("JWS Protected Header incorrect type");
	if (e.payload === void 0) throw new Qo("JWS Payload missing");
	if (typeof e.signature != "string") throw new Qo("JWS Signature missing or incorrect type");
	if (e.header !== void 0 && !ys(e.header)) throw new Qo("JWS Unprotected Header incorrect type");
	let r = {};
	if (e.protected) try {
		let t = Wo(e.protected);
		r = JSON.parse(zo.decode(t));
	} catch {
		throw new Qo("JWS Protected Header is invalid");
	}
	if (!_s(r, e.header)) throw new Qo("JWS Protected and JWS Unprotected Header Parameter names must be disjoint");
	let i = {
		...r,
		...e.header
	}, a = Bs(Qo, /* @__PURE__ */ new Map([["b64", !0]]), n?.crit, r, i), o = !0;
	if (a.has("b64") && (o = r.b64, typeof o != "boolean")) throw new Qo("The \"b64\" (base64url-encode payload) Header Parameter must be a boolean");
	let { alg: s } = i;
	if (typeof s != "string" || !s) throw new Qo("JWS \"alg\" (Algorithm) Header Parameter missing or invalid");
	let c = n && Vs("algorithms", n.algorithms);
	if (c && !c.has(s)) throw new Jo("\"alg\" (Algorithm) Header Parameter value not allowed");
	if (o) {
		if (typeof e.payload != "string") throw new Qo("JWS Payload must be a string");
	} else if (typeof e.payload != "string" && !(e.payload instanceof Uint8Array)) throw new Qo("JWS Payload must be a string or an Uint8Array instance");
	let l = !1;
	typeof t == "function" ? (t = await t(r, e), l = !0, zs(s, t, "verify"), xs(t) && (t = await Ns(t, s))) : zs(s, t, "verify");
	let u = Bo(Ro.encode(e.protected ?? ""), Ro.encode("."), typeof e.payload == "string" ? Ro.encode(e.payload) : e.payload), d;
	try {
		d = Wo(e.signature);
	} catch {
		throw new Qo("Failed to base64url decode the signature");
	}
	if (!await Ks(s, t, d, u)) throw new as();
	let f;
	if (o) try {
		f = Wo(e.payload);
	} catch {
		throw new Qo("Failed to base64url decode the payload");
	}
	else f = typeof e.payload == "string" ? Ro.encode(e.payload) : e.payload;
	let p = { payload: f };
	return e.protected !== void 0 && (p.protectedHeader = r), e.header !== void 0 && (p.unprotectedHeader = e.header), l ? {
		...p,
		key: t
...
async function Js(e, t, n) {
	if (e instanceof Uint8Array && (e = zo.decode(e)), typeof e != "string") throw new Qo("Compact JWS must be a string or Uint8Array");
	let { 0: r, 1: i, 2: a, length: o } = e.split(".");
	if (o !== 3) throw new Qo("Invalid Compact JWS");
	let s = await qs({
		payload: i,
		protected: r,
		signature: a
...
var Ys = (e) => Math.floor(e.getTime() / 1e3), Xs = 60, Zs = 3600, Qs = Zs * 24, $s = Qs * 7, ec = Qs * 365.25, tc = /^(\+|\-)? ?(\d+|\d+\.\d+) ?(seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|years?|yrs?|y)(?: (ago|from now))?$/i, nc = (e) => {
	let t = tc.exec(e);
	if (!t || t[4] && t[1]) throw TypeError("Invalid time period format");
	let n = parseFloat(t[2]), r = t[3].toLowerCase(), i;
	switch (r) {
		...
		case "w":
			i = Math.round(n * $s);
			break;
		default: i = Math.round(n * ec);
	}
	return t[1] === "-" || t[4] === "ago" ? -i : i;
}, rc = (e) => e.toLowerCase().replace(/^application\//, ""), ic = (e, t) => typeof e == "string" ? t.includes(e) : Array.isArray(e) ? t.some(Set.prototype.has.bind(new Set(e))) : !1, ac = (e, t, n = {}) => {
	let r;
	try {
		r = JSON.parse(zo.decode(t));
	} catch {}
	if (!ys(r)) throw new $o("JWT Claims Set must be a top-level JSON object");
	let { typ: i } = n;
	if (i && (typeof e.typ != "string" || rc(e.typ) !== rc(i))) throw new Ko("unexpected \"typ\" JWT header value", r, "typ", "check_failed");
	let { requiredClaims: a = [], issuer: o, subject: s, audience: c, maxTokenAge: l } = n, u = [...a];
	l !== void 0 && u.push("iat"), c !== void 0 && u.push("aud"), s !== void 0 && u.push("sub"), o !== void 0 && u.push("iss");
	for (let e of new Set(u.reverse())) if (!(e in r)) throw new Ko(`missing required "${e}" claim`, r, e, "missing");
	if (o && !(Array.isArray(o) ? o : [o]).includes(r.iss)) throw new Ko("unexpected \"iss\" claim value", r, "iss", "check_failed");
	if (s && r.sub !== s) throw new Ko("unexpected \"sub\" claim value", r, "sub", "check_failed");
	if (c && !ic(r.aud, typeof c == "string" ? [c] : c)) throw new Ko("unexpected \"aud\" claim value", r, "aud", "check_failed");
	let d;
	switch (typeof n.clockTolerance) {
		case "string":
			d = nc(n.clockTolerance);
			break;
		case "number":
			d = n.clockTolerance;
			break;
	...
	let { currentDate: f } = n, p = Ys(f || /* @__PURE__ */ new Date());
	if ((r.iat !== void 0 || l) && typeof r.iat != "number") throw new Ko("\"iat\" claim must be a number", r, "iat", "invalid");
	if (r.nbf !== void 0) {
		if (typeof r.nbf != "number") throw new Ko("\"nbf\" claim must be a number", r, "nbf", "invalid");
		if (r.nbf > p + d) throw new Ko("\"nbf\" claim timestamp check failed", r, "nbf", "check_failed");
	}
	if (r.exp !== void 0) {
		if (typeof r.exp != "number") throw new Ko("\"exp\" claim must be a number", r, "exp", "invalid");
		if (r.exp <= p - d) throw new qo("\"exp\" claim timestamp check failed", r, "exp", "check_failed");
	}
	if (l) {
		let e = p - r.iat, t = typeof l == "number" ? l : nc(l);
		if (e - d > t) throw new qo("\"iat\" claim timestamp check failed (too far in the past)", r, "iat", "check_failed");
		if (e < 0 - d) throw new Ko("\"iat\" claim timestamp check failed (it should be in the past)", r, "iat", "check_failed");
	}
	return r;
};
async function oc(e, t, n) {
	let r = await Js(e, t, n);
	if (r.protectedHeader.crit?.includes("b64") && r.protectedHeader.b64 === !1) throw new $o("JWTs MUST NOT use unencoded payload");
	let i = {
		payload: ac(r.protectedHeader, r.payload, n),
		protectedHeader: r.protectedHeader
	};
	return typeof t == "function" ? {
		...i,
...
var sc = async (e, t, n) => {
	let r = await Gs(e, t, "sign");
	bs(e, r);
	let i = await Fo.subtle.sign(Ws(e, r.algorithm), r, n);
	return new Uint8Array(i);
}, cc = class {
	constructor(e) {
		if (!(e instanceof Uint8Array)) throw TypeError("payload must be an instance of Uint8Array");
	...
	async sign(e, t) {
		if (!this._protectedHeader && !this._unprotectedHeader) throw new Qo("either setProtectedHeader or setUnprotectedHeader must be called before #sign()");
		if (!_s(this._protectedHeader, this._unprotectedHeader)) throw new Qo("JWS Protected and JWS Unprotected Header Parameter names must be disjoint");
		let n = {
			...this._protectedHeader,
			...this._unprotectedHeader
		}, r = Bs(Qo, /* @__PURE__ */ new Map([["b64", !0]]), t?.crit, this._protectedHeader, n), i = !0;
		if (r.has("b64") && (i = this._protectedHeader.b64, typeof i != "boolean")) throw new Qo("The \"b64\" (base64url-encode payload) Header Parameter must be a boolean");
		let { alg: a } = n;
		if (typeof a != "string" || !a) throw new Qo("JWS \"alg\" (Algorithm) Header Parameter missing or invalid");
		zs(a, e, "sign");
		let o = this._payload;
		i && (o = Ro.encode(Ho(o)));
		let s;
		s = this._protectedHeader ? Ro.encode(Ho(JSON.stringify(this._protectedHeader))) : Ro.encode("");
		let c = {
			signature: Ho(await sc(a, e, Bo(s, Ro.encode("."), o))),
			payload: ""
		};
		return i && (c.payload = zo.decode(o)), this._unprotectedHeader && (c.header = this._unprotectedHeader), this._protectedHeader && (c.protected = zo.decode(s)), c;
	}
}, lc = class {
	constructor(e) {
		this._flattened = new cc(e);
	}
	setProtectedHeader(e) {
		return this._flattened.setProtectedHeader(e), this;
	}
...
var dc = class {
	constructor(e = {}) {
		if (!ys(e)) throw TypeError("JWT Claims Set MUST be an object");
		this._payload = e;
	}
	setIssuer(e) {
		return this._payload = {
	...
	setNotBefore(e) {
		return this._payload = typeof e == "number" ? {
			...this._payload,
			nbf: uc("setNotBefore", e)
		} : e instanceof Date ? {
			...this._payload,
			nbf: uc("setNotBefore", Ys(e))
		} : {
			...this._payload,
			nbf: Ys(/* @__PURE__ */ new Date()) + nc(e)
		}, this;
	}
	setExpirationTime(e) {
		return this._payload = typeof e == "number" ? {
			...this._payload,
			exp: uc("setExpirationTime", e)
		} : e instanceof Date ? {
			...this._payload,
			exp: uc("setExpirationTime", Ys(e))
		} : {
			...this._payload,
			exp: Ys(/* @__PURE__ */ new Date()) + nc(e)
		}, this;
	}
	setIssuedAt(e) {
		return this._payload = e === void 0 ? {
			...this._payload,
			iat: Ys(/* @__PURE__ */ new Date())
		} : e instanceof Date ? {
			...this._payload,
			iat: uc("setIssuedAt", Ys(e))
		} : typeof e == "string" ? {
			...this._payload,
			iat: uc("setIssuedAt", Ys(/* @__PURE__ */ new Date()) + nc(e))
		} : {
			...this._payload,
			iat: uc("setIssuedAt", e)
		}, this;
	}
}, fc = class extends dc {
	setProtectedHeader(e) {
	...
	async sign(e, t) {
		let n = new lc(Ro.encode(JSON.stringify(this._payload)));
		if (n.setProtectedHeader(this._protectedHeader), Array.isArray(this._protectedHeader?.crit) && this._protectedHeader.crit.includes("b64") && this._protectedHeader.b64 === !1) throw new $o("JWTs MUST NOT use unencoded payload");
		return n.sign(e, t);
	}
}, pc = (e, t) => {
...
async function mc(e, t) {
	...
	switch (e.kty) {
		case "EC":
			pc(e.crv, "\"crv\" (Curve) Parameter"), pc(e.x, "\"x\" (X Coordinate) Parameter"), pc(e.y, "\"y\" (Y Coordinate) Parameter"), n = {
				crv: e.crv,
				kty: e.kty,
				x: e.x,
				y: e.y
			...
			break;
		case "OKP":
			pc(e.crv, "\"crv\" (Subtype of Key Pair) Parameter"), pc(e.x, "\"x\" (Public Key) Parameter"), n = {
				crv: e.crv,
				kty: e.kty,
				x: e.x
			};
			break;
		case "RSA":
			pc(e.e, "\"e\" (Exponent) Parameter"), pc(e.n, "\"n\" (Modulus) Parameter"), n = {
				e: e.e,
				kty: e.kty,
				n: e.n
			};
			break;
		case "oct":
			pc(e.k, "\"k\" (Key Value) Parameter"), n = {
				k: e.k,
				kty: e.kty
			};
			break;
		default: throw new Yo("\"kty\" (Key Type) Parameter missing or unsupported");
	}
	let r = Ro.encode(JSON.stringify(n));
	return Ho(await Lo(t, r));
}
function hc(e) {
	switch (typeof e == "string" && e.slice(0, 2)) {
		case "RS":
		...
		case "ES": return "EC";
		case "Ed": return "OKP";
		default: throw new Yo("Unsupported \"alg\" value for a JSON Web Key Set");
	}
}
function gc(e) {
	return e && typeof e == "object" && Array.isArray(e.keys) && e.keys.every(_c);
}
function _c(e) {
	return ys(e);
}
...
var yc = class {
	constructor(e) {
		if (this._cached = /* @__PURE__ */ new WeakMap(), !gc(e)) throw new ts("JSON Web Key Set malformed");
		this._jwks = vc(e);
	}
	async getKey(e, t) {
		let { alg: n, kid: r } = {
			...e,
			...t?.header
		}, i = hc(n), a = this._jwks.keys.filter((e) => {
			let t = i === e.kty;
			if (t && typeof r == "string" && (t = r === e.kid), t && typeof e.alg == "string" && (t = n === e.alg), t && typeof e.use == "string" && (t = e.use === "sig"), t && Array.isArray(e.key_ops) && (t = e.key_ops.includes("verify")), t) switch (n) {
				case "ES256":
					t = e.crv === "P-256";
		...
		if (s === 0) throw new ns();
		if (s !== 1) {
			let e = new rs(), { _cached: t } = this;
			throw e[Symbol.asyncIterator] = async function* () {
				for (let e of a) try {
					yield await bc(t, e, n);
				} catch {}
			}, e;
		}
		return bc(this._cached, o, n);
	}
};
async function bc(e, t, n) {
	...
	if (r[n] === void 0) {
		...
		if (e instanceof Uint8Array || e.type !== "public") throw new ts("JSON Web Key Set members must be public keys");
		r[n] = e;
	}
	return r[n];
}
function xc(e) {
	let t = new yc(e), n = async (e, n) => t.getKey(e, n);
	return Object.defineProperties(n, { jwks: {
		value: () => vc(t._jwks),
		enumerable: !0,
		configurable: !1,
...
var Sc = async (e, t, n) => {
	...
	if (i !== void 0 && clearTimeout(i), o.status !== 200) throw new Go("Expected 200 OK from the JSON Web Key Set HTTP response");
	try {
		return await o.json();
	} catch {
		throw new Go("Failed to parse the JSON Web Key Set HTTP response as JSON");
	}
};
function Cc() {
	return typeof WebSocketPair < "u" || typeof navigator < "u" && navigator.userAgent === "Cloudflare-Workers" || typeof EdgeRuntime < "u" && EdgeRuntime === "vercel";
...
var wc;
(typeof navigator > "u" || !navigator.userAgent?.startsWith?.("Mozilla/5.0 ")) && (wc = "jose/v5.10.0");
var Tc = Symbol();
function Ec(e, t) {
	return !(typeof e != "object" || !e || !("uat" in e) || typeof e.uat != "number" || Date.now() - e.uat >= t || !("jwks" in e) || !ys(e.jwks) || !Array.isArray(e.jwks.keys) || !Array.prototype.every.call(e.jwks.keys, ys));
}
var Dc = class {
	constructor(e, t) {
		if (!(e instanceof URL)) throw TypeError("url must be an instance of URL");
		this._url = new URL(e.href), this._options = {
			agent: t?.agent,
			headers: t?.headers
		}, this._timeoutDuration = typeof t?.timeoutDuration == "number" ? t?.timeoutDuration : 5e3, this._cooldownDuration = typeof t?.cooldownDuration == "number" ? t?.cooldownDuration : 3e4, this._cacheMaxAge = typeof t?.cacheMaxAge == "number" ? t?.cacheMaxAge : 6e5, t?.[Tc] !== void 0 && (this._cache = t?.[Tc], Ec(t?.[Tc], this._cacheMaxAge) && (this._jwksTimestamp = this._cache.uat, this._local = xc(this._cache.jwks)));
	}
	coolingDown() {
		return typeof this._jwksTimestamp == "number" && Date.now() < this._jwksTimestamp + this._cooldownDuration;
	}
	...
	async getKey(e, t) {
		...
		try {
			...
			if (n instanceof ns && this.coolingDown() === !1) return await this.reload(), this._local(e, t);
			throw n;
		}
	}
	async reload() {
		this._pendingFetch && Cc() && (this._pendingFetch = void 0);
		let e = new Headers(this._options.headers);
		wc && !e.has("User-Agent") && (e.set("User-Agent", wc), this._options.headers = Object.fromEntries(e.entries())), this._pendingFetch ||= Sc(this._url, this._timeoutDuration, this._options).then((e) => {
			this._local = xc(e), this._cache && (this._cache.uat = Date.now(), this._cache.jwks = e), this._jwksTimestamp = Date.now(), this._pendingFetch = void 0;
		}).catch((e) => {
			throw this._pendingFetch = void 0, e;
		}), await this._pendingFetch;
	}
...
function Oc(e, t) {
	let n = new Dc(e, t), r = async (e, t) => n.getKey(e, t);
	return Object.defineProperties(r, {
		coolingDown: {
			get: () => n.coolingDown(),
			enumerable: !0,
...
var kc = Wo;
function Ac(e) {
	if (typeof e != "string") throw new $o("JWTs must use Compact JWS serialization, JWT must be a string");
	let { 1: t, length: n } = e.split(".");
	...
	let i;
	try {
		i = JSON.parse(zo.decode(r));
	} catch {
		throw new $o("Failed to parse the decoded payload as JSON");
	}
	if (!ys(i)) throw new $o("Invalid JWT Claims Set");
	return i;
}
function jc(e) {
	let t = e?.modulusLength ?? 2048;
	if (typeof t != "number" || t < 2048) throw new Yo("Invalid or unsupported modulusLength option provided, 2048 bits or larger keys must be used");
	return t;
}
async function Mc(e, t) {
	let n, r;
		...
		case "EdDSA": {
			...
			switch (e) {
				...
				case "Ed448":
					n = { name: e };
					break;
				default: throw new Yo("Invalid or unsupported crv option provided");
			}
			break;
		}
		case "ECDH-ES":
		...
		case "ECDH-ES+A256KW": {
			...
			switch (e) {
				...
				case "X448":
					n = { name: e };
					break;
				default: throw new Yo("Invalid or unsupported crv option provided, supported values are P-256, P-384, P-521, X25519, and X448");
			}
			break;
		}
		default: throw new Yo("Invalid or unsupported JWK \"alg\" (Algorithm) Parameter value");
	}
	return Fo.subtle.generateKey(n, t?.extractable ?? !1, r);
}
...
var Pc = async (e, t) => {
	...
	if (!l) {
		let e = o.registration_endpoint;
		l = (await Pc(e, n ?? { redirect_uris: [i] }).then((e) => {
			if (!e.ok) throw Error(`HTTP error! Status: ${e.status}`);
			return e.json();
		})).client_id;
	}
	...
	let { pkce_code_verifier: u, pkce_code_challenge: d } = await Ic();
	sessionStorage.setItem("pkce_code_verifier", u);
	let f = window.crypto.randomUUID();
	sessionStorage.setItem("csrf_token", f);
	let p = o.authorization_endpoint + `?response_type=code&redirect_uri=${encodeURIComponent(i)}&scope=openid offline_access webid&client_id=${encodeURIComponent(l)}&code_challenge_method=S256&code_challenge=${d}&state=${f}&prompt=consent`;
	...
	let s = sessionStorage.getItem("token_endpoint");
	if (s === null) throw Error("Access Token Request preparation - Could not find in sessionStorage: token_endpoint");
	let c = await Nc("ES256"), l = await Rc(r, a, n.toString(), o, s, c).then((e) => {
		if (!e.ok) throw Error(`HTTP error! Status: ${e.status}`);
		return e.json();
	}), u = l.access_token, d = sessionStorage.getItem("jwks_uri");
	if (d === null) throw Error("Access Token validation preparation - Could not find in sessionStorage: jwks_uri");
	let { payload: f } = await oc(u, Oc(new URL(d)), {
		issuer: i,
		audience: "solid"
	}), p = await mc(await Us(c.publicKey));
	if (f.cnf.jkt !== p) throw Error("Access Token validation failed on `jkt`: jkt !== DPoP thumbprint - " + f.cnf.jkt + " !== " + p);
	if (f.client_id !== o) throw Error("Access Token validation failed on `client_id`: JWT payload !== client_id - " + f.client_id + " !== " + o);
	let m = {
		...l,
...
}, Rc = async (e, t, n, r, i, a) => {
	let o = await Us(a.publicKey);
	o.alg = "ES256";
	let s = await new fc({
		htu: i,
		htm: "POST"
	...
	try {
		...
		let c = await e.getItem("jwks_uri");
		if (c === null) throw Error("Access Token validation preparation - Could not find in sessionDatabase: jwks_uri");
		let { payload: l } = await oc(o, Oc(new URL(c)), {
			issuer: s,
			audience: "solid"
		}), u = await mc(await Us(r.publicKey));
		if (l.cnf.jkt !== u) throw Error("Access Token validation failed on `jkt`: jkt !== DPoP thumbprint - " + l.cnf.jkt + " !== " + u);
		if (l.client_id !== t) throw Error("Access Token validation failed on `client_id`: JWT payload !== client_id - " + l.client_id + " !== " + t);
		return await e.setItem("refresh_token", a.refresh_token), {
			...a,
...
var Hc = class extends EventTarget {
	isActive_ = !1;
	exp_;
	webId_ = void 0;
	currentAth_ = void 0;
	...
	rejectRefresh;
	constructor(e, t) {
		super(), this.authFetch = this.authFetch.bind(this), this.information = { clientDetails: e }, this.database = t?.database, t?.onSessionStateChange && this.addEventListener(Vc.STATE_CHANGE, (e) => t.onSessionStateChange?.(e)), t?.onSessionExpirationWarning && this.addEventListener(Vc.EXPIRATION_WARNING, (e) => t?.onSessionExpirationWarning?.(e)), t?.onSessionExpiration && this.addEventListener(Vc.EXPIRATION, (e) => t?.onSessionExpiration?.(e));
	}
	async login(e, t) {
		await Fc(e, t, this.information.clientDetails);
	}
	async handleRedirectFromLogin() {
		let e = await Lc(this.information.clientDetails, this.database);
		e.tokenDetails && (this.information.clientDetails = e.clientDetails, this.information.idpDetails = e.idpDetails, await this.setTokenDetails(e.tokenDetails), this.dispatchStateChangeEvent());
	}
	async restore() {
		if (!this.database) throw Error("Could not refresh tokens: missing database. Provide database in sessionOption.");
		...
		let e = this.isActive;
		return zc(this.database).then((e) => this.setTokenDetails(e)).then(() => this.resolveRefresh()).catch((e) => {
			this.isActive ? (this.rejectRefresh(Error(e || "Token refresh failed")), this.isExpired() ? this.dispatchExpirationEvent() : this.dispatchExpirationWarningEvent()) : this.rejectRefresh(/* @__PURE__ */ Error("No session to restore."));
		}).finally(() => {
			this.clearRefreshPromise(), e !== this.isActive && this.dispatchStateChangeEvent();
		}), this.refreshPromise;
	...
	async _createSignedDPoPToken(e) {
		if (!this.information.tokenDetails || !this.currentAth_) throw Error("Session not established.");
		e.ath = this.currentAth_;
		let t = await Us(this.information.tokenDetails.dpop_key_pair.publicKey);
		return new fc(e).setIssuedAt().setJti(window.crypto.randomUUID()).setProtectedHeader({
			alg: "ES256",
			typ: "dpop+jwt",
			jwk: t
		}).sign(this.information.tokenDetails.dpop_key_pair.privateKey);
	...
	dispatchStateChangeEvent() {
		this.dispatchEvent(new CustomEvent(Vc.STATE_CHANGE, { detail: {
			isActive: this.isActive,
			webId: this.webId
		} }));
	}
	dispatchExpirationWarningEvent() {
		this.dispatchEvent(new CustomEvent(Vc.EXPIRATION_WARNING, { detail: { expires_in: this.getExpiresIn() } }));
	}
	dispatchExpirationEvent() {
		this.dispatchEvent(new CustomEvent(Vc.EXPIRATION));
	}
}, Uc = class {
	constructor() {
...
function Gc() {
	let e = Po, t = e.SessionCore ?? e.default?.SessionCore ?? e.default;
...
function qc() {
	...
	})()) return Kc ? new Kc(void 0, { database: new Wc() }) : new No();
	try {
		return new No();
	} catch (e) {
		...
		try {
			...
			return console.warn("solid-logic: IndexedDB unavailable, using in-memory session database:", e), Kc ? new Kc(void 0, { database: new Uc() }) : new No();
		}
	}
}
var Jc = qc();
...
}, Qc = Jc, $c = typeof Qc.login == "function" ? Qc.login.bind(Jc) : void 0;
$c && (Qc.login = async (e, t) => {
	if (e && typeof e == "object" && !Array.isArray(e)) {
		let t = e.oidcIssuer ?? e.idp ?? e.issuer, n = e.redirectUrl ?? e.redirect_uri ?? e.redirectUri;
		if (typeof t == "string" && typeof n == "string") return $c(await Xc(t), n);
	}
	return $c(typeof e == "string" ? await Xc(e) : e, t);
});
var el = new Zc(), tl = Jc.isActive ?? !!Jc.webId;
typeof Jc.addEventListener == "function" && Jc.addEventListener("sessionStateChange", () => {
	let e = Jc.isActive ?? !!Jc.webId;
	tl && !e && el.emit("logout"), tl = e;
});
var nl = Object.assign(Jc, { events: el }), rl = 5e3;
async function il(e) {
	if (e === null) return;
	let t, n = new Promise((e) => {
		t = setTimeout(() => e(void 0), rl);
	});
	try {
...
var al = class {
	...
	currentUser() {
		let e = ii();
		if (e.viewingNoAuthPage) return Q(e.webId);
		...
		return this && this.session && i && s ? Q(i) : ai();
	}
	async checkUser(e) {
		...
		!this.sessionRestoreHookAttached && typeof n?.events?.on == "function" && (n.events.on("sessionRestore", (e) => {
			ni(`Session restored to ${e}`), document.location.toString() !== e && history.replaceState(null, "", e);
	...
	async resolveCurrentUser() {
		...
		if (t.hash = "", typeof e?.handleIncomingRedirect == "function") await e.handleIncomingRedirect({
			...
			if (typeof e?.restore == "function") {
				try {
					...
					ni(`Session restore failed, continuing logged-out: ${n}`);
				}
		...
		return i ||= await this.probeNssCookieBackedWebId(), this.fallbackWebId = i || null, i && (r = this.saveUser(i)), r && ni(`(Logged in as ${r} by authentication)`), r;
	}
...
function sl(e, t) {
	...
	async function i(t, r) {
		...
		let a = pi(i), o = [
			Br(a, n.rdf("type"), n.solid("TypeRegistration"), i.doc()),
			Br(a, n.solid("forClass"), n.meeting("LongChat"), i.doc()),
			Br(a, n.solid("instance"), t, i.doc())
		];
	...
	async function a(n) {
		let r = await t.loadMe(), i = _i(n, await t.getPodRoot(r)), a = !0;
		try {
	...
	function s(t) {
		...
		let a = t.newInstance = t.newInstance || r.sym(t.newBase + ol), o = a.doc();
		return r.add(a, n.rdf("type"), n.meeting("LongChat"), o), r.add(a, n.dc("title"), "Chat channel", o), r.add(a, n.dc("created"), Yr(new Date(Date.now())), o), t.me && r.add(a, n.dc("author"), t.me, o), new Promise(function(e, n) {
...
function dl(e, t, n = {}) {
	if (e.ok || n.allowNotFound && ul(e.status)) return;
...
function ml(e) {
	...
	let t = pl(e), n = t.get("user") ?? /* @__PURE__ */ new Set(), r = t.get("public") ?? /* @__PURE__ */ new Set();
	return {
		canEdit: n.has("write"),
...
function hl(e, t, n, r) {
	...
	function s(t, n) {
		...
		if (n.headers) {
			...
			let c = ml(n.headers.get("wac-allow"));
			i = c.canEdit, a = c.isPublic, o = n.headers.get("etag") ?? void 0, s = e.anyValue(t, ei.dct("modified")) || e.anyValue(t, ei.dc("modified")) || void 0;
		}
		(!n.headers || !n.headers.get("content-type")) && (!n.headers || !n.headers.get("wac-allow") || !n.headers.get("etag")) && e.each(null, e.sym("http://www.w3.org/2007/ont/link#requestedURI"), t).map((n) => {
			...
			if (c && c.termType === "NamedNode") {
				let n = e.anyValue(c, ei.httph("content-type")) || void 0;
				r === void 0 && (r = n);
				let l = ml(e.anyValue(c, ei.httph("wac-allow")) || e.anyValue(c, ei.httph("WAC-Allow")));
				i ||= l.canEdit, a ||= l.isPublic;
				let u = e.anyValue(c, ei.httph("etag")) || void 0;
				o === void 0 && (o = u), s = e.anyValue(t, ei.dct("modified")) || e.anyValue(t, ei.dc("modified")) || void 0;
			}
		});
		let c = e.any(t, $)?.value || void 0;
		return {
			contentType: r,
			access: {
				canEdit: i,
	...
	async function c(t) {
		...
		return dl(n, "HEAD"), s(t, n);
	}
	async function l(t) {
		let n = e.any(t, ei.ldp("contains")) || t.dir();
	...
	async function m(t, r = {}) {
		...
		if (a(t)) {
			...
			await Promise.all(e.map((e) => m(Q(e), r)));
		}
		let o;
		try {
			o = await e.fetcher._fetch(t.value, { method: "DELETE" }), dl(o, "DELETE", { allowNotFound: !0 });
		} catch (e) {
			if (!fl(e)) throw e;
		}
		return r.deleteTypeIndexes && await p(t, r.user), i && e.removeMatches(i, ei.ldp("contains"), t, i.doc()), e.removeDocument(t), e.fetcher.unload(t.doc()), o;
	}
...
function gl(e) {
	function t(t) {
		return e.statementsMatching(t, Q("http://www.w3.org/ns/ldp#contains"), void 0).map((e) => e.object);
	}
	function n(t) {
		let n = e.findTypeURIs(t);
		return !!(t.value.charAt(t.value.length - 1) === "/" || n[ei.ldp("Container").uri] || n[ei.ldp("BasicContainer").uri]);
	}
	function r(e) {
		return t(e).length;
	}
	async function i(t) {
		if (!n(Q(t))) throw Error(`Not a container URL ${t}`);
		let r = await e.fetcher._fetch(t, {
			method: "PUT",
			headers: {
...
function bl(e, t, n) {
	let r = ei, i = gl(e), a = /* @__PURE__ */ new Map(), o = /* @__PURE__ */ new Map();
	function s(e) {
		return !!e && (e.startsWith("https://") || e.startsWith("http://"));
	}
	function c(e) {
	...
	async function f(t, n) {
		...
		let i = Q(r), a;
		try {
			await e.fetcher.load(i), a = e.any(i, $)?.value;
		} catch (e) {
			if (!u(e)) throw e;
		}
		a ||= `${r}.acl`;
	...
	async function p(t, r, i = !1) {
		if (!await n.loadOrCreateWithContentOnCreate(r, vi()) && !i) return;
		let a;
		try {
			await e.fetcher.load(r), a = e.any(r, $)?.value;
		} catch (e) {
			if (!u(e)) throw e;
		}
		a ||= `${r.uri}.acl`;
	...
	async function m(e) {
		await n.loadOrCreateWithContentOnCreate(e, yi());
	}
	async function h(t, i) {
		...
		f && await n.followOrCreateLinkWithContentOnCreate(t, r.solid("publicTypeIndex"), u, o, vi());
		let h = [];
		e.holds(a, r.rdf("type"), r.space("ConfigurationFile"), a) || h.push(Br(a, r.rdf("type"), r.space("ConfigurationFile"), a)), e.holds(a, r.dct("title"), void 0, a) || h.push(Br(a, r.dct("title"), Wr("Preferences file"), a)), e.holds(t, r.solid("publicTypeIndex"), u, a) || h.push(Br(t, r.solid("publicTypeIndex"), u, a)), e.holds(t, r.solid("privateTypeIndex"), d, a) || h.push(Br(t, r.solid("privateTypeIndex"), d, a)), h.length > 0 && (await e.updater.update([], h), await e.fetcher.load(a)), await p(t, u, f), await m(d);
	}
	async function g(e) {
		try {
			return !!await n.loadOrCreateWithContentOnCreate(e, vl());
		} catch (t) {
			throw t.response?.status === 401 ? new si() : t.response?.status === 403 ? hi(e) ? new ci() : new li() : t;
		}
	}
	async function _(e) {
		try {
	...
	async function v(t) {
		...
		let c = (async () => {
			...
			try {
				...
				throw ri(`User ${t} has no pointer in profile to preferences file.`), e instanceof ui || e instanceof di || e instanceof si || e instanceof ci || e instanceof li || e instanceof fi, e;
			}
			try {
				await e.fetcher.load(a);
			} catch (n) {
				...
				throw ri(r), n.response.status === 401 ? new si() : n.response.status === 403 ? hi(a) ? new ci() : new li() : Error(r);
			}
...
function xl(e, t, n) {
		...
		try {
			...
			if (n?.response?.status === 404) {
				try {
					...
					if ((e?.response?.status ?? e?.status) !== 412) throw new di(`createIfNotExists: PUT FAILED: ${t}: ${e}`);
				}
				return await e.fetcher.load(t);
			}
			if (n?.response?.status === 401) throw new si();
			if (n?.response?.status === 403) throw hi(t) ? new ci() : new li();
			let r = `createIfNotExists doc load error: ${t}: ${n}`;
			throw new fi(n?.status, `${n?.message || ""}${r}`);
		}
	}
	...
	async function a(t, n, i, a) {
		...
		try {
			await e.updater.update([], [Br(t, n, i, a)]);
		} catch (e) {
			throw ri(`followOrCreateLink: Error making link in ${a} to ${i}: ${e}`), new di(e);
		}
		try {
			await r(i);
		} catch (e) {
			throw ri(`followOrCreateLink: Error loading or saving new linked document: ${i}: ${e}`), e;
		}
		return i;
	}
	async function o(t, n, r, a, o) {
		...
		try {
			await e.updater.update([], [Br(t, n, r, a)]);
		} catch (e) {
		...
		try {
			...
			throw ri(`followOrCreateLinkWithContentOnCreate: Error loading or saving new linked document: ${r}: ${e}`), e;
		}
		return r;
	}
	async function s(n) {
		...
		let i = await t.findAclDocUrl(Q(n.target));
...
function Sl(e, t) {
	ni("SolidLogic: Unique instance created.  There should only be one of these.");
	let n = Rr();
	Lr(n, { fetch: e.fetch }), n.updater = new jr(n), n.features = [];
	let r = new al(t), i = ti(n), a = gl(n), o = xl(n, i, a), s = bl(n, r, o), c = sl(n, s), l = cl(n, s, o, a, i), u = bi(n, r, s, o), d = hl(n, i, a, u);
	ni("SolidAuthnLogic initialized");
	function f(e) {
...
var Cl = async (e, t) => {
	let n = t && t.credentials && t.credentials == "omit", r = nl;
...
}, wl = Symbol.for("solid-logic-singleton"), Tl = typeof window < "u" ? window : global;
function El() {
	return Tl[wl] ? ni("SolidLogic: Using existing global singleton instance.") : (ni("SolidLogic: Creating new global singleton instance."), Tl[wl] = Sl({ fetch: Cl }, nl), ni("Unique quadstore initialized.")), Tl[wl];
}
var Dl = El(), Ol = Dl.authn, kl = Dl.authn.authSession, Al = Dl.store;
//#endregion
export { gt as A, o as B, Yr as C, lr as D, Dr as E, _e as F, d as H, he as I, H as L, Ie as M, ke as N, wt as O, Ce as P, N as R, Br as S, jr as T, u as U, c as V, Jr as _, bi as a, Wr as b, di as c, si as d, ai as f, Lr as g, Fr as h, Dl as i, Xe as j, St as k, li as l, $r as m, kl as n, ui as o, $ as p, Ol as r, ci as s, Al as t, fi as u, Rr as v, Vr as w, Q as x, zr as y, s as z };

//# sourceMappingURL=index.esm-BPiwP3vo.js.map