import * as UI from '../../src/index'
import * as IconForClassStories from './iconForClass.stories';

import { Canvas, Meta, Story } from "@storybook/blocks";

export const iconDecorator = (Story) => {
  const items = Story();
  const lines = items.map(
    ({ className, icon }) =>
      `<tr><td>${className}</td><td>${icon}</td><td><img src="${UI.icons.iconBase}${icon}" style="width:20px; height:20px;" /></td></tr>`
  );
  return `<table><th>className</th><th>icon</th><th>Preview</th>
            ${lines.join("")}
          </table>`;
};

<Meta of={IconForClassStories} />

# iconForClass

Best logo or avatar or photo etc to represent someone or some group etc

[API docs](https://solidos.github.io/solid-ui/docs/api/modules/_widgets_buttons_.html#iconforclass)

## for solid:AppProviderClass

<Canvas withSource="open">
  <Story of={IconForClassStories.ForSolidAppProviderClass} />
</Canvas>
