...
import { phoneIcon as downArrowIcon } from '../loginButton/downArrow'
...
export class Select extends LitElement {
  ...
  private readonly _handleDocumentPointerDown = (event: Event) => {
    ...
    const eventPath = 'composedPath' in event ? (event as Event & { composedPath: () => EventTarget[] }).composedPath() : []
  ...
  static styles = [listboxStyles, css`
    :host { // default theme
    ...
    .select-trigger {
      ...
      border: var(--select-trigger-border, 1px solid var(--gray-400, #99A1AF));
    ...
    .popup-box {
      ...
      width: calc(var(--select-popup-width) + var(--select-popup-extra-width));
      min-width: calc(var(--select-popup-width) + var(--select-popup-extra-width));
      background: var(--popup-background);
      opacity: 1;
      overflow: hidden;
  ...
  render () {
    ...
    return html`
      ...
      >
        <span class="select-trigger-label" part="trigger-label">${triggerLabel}</span>
        <span class="select-trigger-icon" part="trigger-icon" aria-hidden="true">${downArrowIcon}</span>
      </button>
