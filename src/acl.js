...
UI.acl.adoptACLDefault = function (
  doc,
  aclDoc,
  defaultResource,
  defaultACLdoc
  ...
  defaults.map(function (da) {
    proposed = proposed
      ...
      .concat(
        kb.statementsMatching(da, ACL('agentClass'), undefined, defaultACLdoc)
      )
      .concat(
        kb.statementsMatching(da, ACL('agentGroup'), undefined, defaultACLdoc)
      )
      .concat(
        kb.statementsMatching(da, ACL('origin'), undefined, defaultACLdoc)
      )
      .concat(
        kb.statementsMatching(da, ACL('originClass'), undefined, defaultACLdoc)
      )
  ...
  var doList = function (list) {
    if (list.length) {
      ...
      UI.acl.getACLorDefault(doc, function (
        ...
        aclList.push(
          defa
            ? UI.widgets.readACL(defaultHolder, defaultACLDoc)
...
UI.acl.ACLbyCombination = function (ac) {
  var byCombo = []
  ;['agent', 'agentClass', 'agentGroup', 'origin', 'originClass'].map(function (
    pred
  ) {
  ...
  UI.acl.getACL(doc, function (ok, status, aclDoc, message) {
    ...
    if (!ok) {
      ...
      return callbackFunction(
        ...
        'Error ' +
          status +
          ' accessing Access Control information for ' +
          doc +
          ': ' +
          message
      )
...
UI.acl.getACL = function (doc, callbackFunction) {
  UI.store.fetcher.nowOrWhenFetched(doc, undefined, function (ok, body) {
    ...
    if (!aclDoc) {
      ...
      UI.store.fetcher.nowOrWhenFetched(aclDoc, undefined, function (
        ok,
        message,
        response
      ) {
