...
function findImageFromURI(x) {
  var iconDir = UI.icons.iconBase; // Special cases from URI scheme:
...
function trySetImage(element, thing, iconForClassMap) {
  ...
  if (typeIcon) {
    ...
    element.style = UI.style.classIconStyle; // element.style.border = '0.1em solid green;'
...
function faviconOrDefault(dom, x) {
  ...
  image.setAttribute('src', UI.icons.iconBase + (isOrigin(x) ? 'noun_15177.svg' : 'noun_681601.svg') // App symbol vs document
  );
...
function attachmentList(dom, subject, div, options) {
  ...
  var deleteAttachment = function deleteAttachment(target) {
    kb.updater.update($rdf.st(subject, predicate, target, doc), [], function (uri, ok, errorBody, _xhr) {
      if (ok) {
        refresh();
      } else {
...
function propertyTriage(kb) {
  ...
  for (var i = 0; i < ps.length; i++) {
    p = ps[i].toNT(); // UI.log.debug('propertyTriage: unknown: ' + p)
  ...
  UI.log.info("propertyTriage: ".concat(no, " non-lit, ").concat(nd, " literal. ").concat(nu, " unknown."));
...
function twoLineTrip(dom, x) {
  ...
  var box = dom.createElement('table');
  box.innerHTML = "\n    <tr>\n      <td colspan=\"2\">".concat(enc(UI.ns.dc('title')), "</td>\n    </tr>\n    <tr style=\"color: #777\">\n      <td>").concat(enc(UI.ns.cal('dtstart')), "</td>\n      <td>").concat(enc(UI.ns.cal('dtend')), "</td>\n    </tr>");
  return box;
}
/**
...
function isImage(file, kind) {
  ...
  var typeURIs = UI.store.findTypeURIs(file); // See https://github.com/linkeddata/rdflib.js/blob/d5000f/src/utils-js.js#L14
...
function fileUploadButtonDiv(dom, droppedFileHandler) {
  ...
  var buttonElt = div.appendChild(button(dom, UI.icons.iconBase + 'noun_Upload_76574_000000.svg', 'Upload files', function (_event) {
    input.click();
  }));
