...
function registrationControl(context, instance, theClass) {
  ...
  return ensureTypeIndexes(context).then(function () {
    ...
    if (context.index && context.index["public"] && context.index["public"].length > 0) {
      ...
      tbody.children[0].appendChild(_widgets["default"].buildCheckBoxForm(context.dom, _logic.solidLogicSingleton.store, "Public link to this ".concat(context.noun), null, statements, form, index));
    }

    if (context.index && context.index["private"] && context.index["private"].length > 0) {
      index = context.index["private"][0];
      statements = registrationStatements(index);
      tbody.children[1].appendChild(_widgets["default"].buildCheckBoxForm(context.dom, _logic.solidLogicSingleton.store, "Personal note of this ".concat(context.noun), null, statements, form, index));
    }
