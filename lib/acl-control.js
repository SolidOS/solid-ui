...
var aclControlModule = module.exports = {}
...
aclControlModule.preventBrowserDropEvents = function (document) {
...
aclControlModule.ACLControlBox5 = function (subject, dom, noun, kb, callback) {
  ...
  headerRow.textContent = 'Sharing for ' + noun + ' ' + utilsModule.label(subject)
  ...
  var ACLControlEditable = function (box, doc, aclDoc, kb, options) {
    ...
    options = options || {}
    var ac = aclModule.readACL(doc, aclDoc, kb, options.doingDefaults) // Note kb might not be normal one
    var byCombo
    box[defaultOrMain] = byCombo = aclModule.ACLbyCombination(ac)
    ...
    box.saveBack = function (callback) {
      var kb2 = $rdf.graph()
      if (!box.isContainer) {
        aclModule.makeACLGraphbyCombo(kb2, doc, box.mainByCombo, aclDoc, true)
      } else if (box.defaultsDiffer) { // Pair of controls
        aclModule.makeACLGraphbyCombo(kb2, doc, box.mainByCombo, aclDoc, true)
        aclModule.makeACLGraphbyCombo(kb2, doc, box.defByCombo, aclDoc, false, true)
      } else { // Linked controls
        aclModule.makeACLGraphbyCombo(kb2, doc, box.mainByCombo, aclDoc, true, true)
      }
      var updater = kb.updater || new $rdf.UpdateManager(kb)
    ...
    var renderCombo = function (byCombo, combo) {
      ...
      var syncCombo = function (combo) {
        ...
        if (arr && arr.length) {
          ...
          widgetModule.clearElement(middleTable)
  ...
  aclModule.getACLorDefault(doc, function (ok, p2, targetDoc, targetACLDoc, defaultHolder, defaultACLDoc) {
    var defa = !p2
      ...
      if (defa) {
        ...
        if (!defaults.length) {
          ...
          statusBlock.innerHTML = 'The sharing for this ' + noun + " is the default for folder <a href='" + defaultHolder.uri + "'>" +
            (defaultHolder) + '</a>.'
          var kb2 = aclModule.adoptACLDefault(doc, targetACLDoc, defaultHolder, defaultACLDoc)
        ...
        var addDefaultButton = function (prospectiveDefaultHolder) {
          ...
          useDefault.textContent = 'Stop specific sharing for this ' + noun +
            ' -- just use default' // + utilsModule.label(thisDefaultHolder)
          if (prospectiveDefaultHolder) {
            useDefault.textContent += ' for ' + utilsModule.label(prospectiveDefaultHolder)
          }
        ...
        if (targetDocDir) {
          aclModule.getACLorDefault($rdf.sym(targetDocDir), function (ok2, p22, targetDoc2, targetACLDoc2, defaultHolder2, defaultACLDoc2) {
            if (ok2) {
        ...
        box.addControlForDefaults = function () {
          ...
          var mergeButton = widgetModule.clearElement(box.offer).appendChild(dom.createElement('button'))
        ...
        box.removeControlForDefaults = function () {
          ...
          var splitButton = widgetModule.clearElement(box.offer).appendChild(dom.createElement('button'))
        ...
        if (box.isContainer) {
          var ac = aclModule.readACL(targetDoc, targetACLDoc, kb)
          var acd = aclModule.readACL(targetDoc, targetACLDoc, kb, true)
          box.defaultsDiffer = !aclModule.sameACL(ac, acd)
