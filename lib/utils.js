...
utilsModule.hashColor = function(who) {
    who = who.uri || who;
...
if (!utilsModule.audioContext) {
    if (typeof AudioContext !== 'undefined') {
        utilsModule.audioContext = AudioContext;
    } else if (typeof window !== 'undefined') {
       utilsModule.audioContext = window.AudioContext || window.webkitAudioContext;
    }
}
if (utilsModule.audioContext) {
    utilsModule.beep = (function () {
        var ctx = new(utilsModule.audioContext)
...
utilsModule.getTerm = function (target) {
  ...
  switch (className) {
    ...
    case 'selected': // header TD
      return utilsModule.getAbout(UI.store, target) // kb to be changed
...
utilsModule.predicateLabelForXML = function (p, inverse) {
  ...
  if (inverse) { // If we know an inverse predicate, use its label
    var ip = UI.store.any(p, nsModule.owl('inverseOf'))
    if (!ip) ip = UI.store.any(undefined, nsModule.owl('inverseOf'), p)
