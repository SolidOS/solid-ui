...
if (typeof thisModule.nextVariable == "undefined") thisModule.nextVariable = 0;
thisModule.newVariableName = function() {
    return 'v' + thisModule.nextVariable++;
}
thisModule.clearVariableNames = function() {
    thisModule.nextVariable = 0;
}
...
thisModule.stackString = function(e){
...
thisModule.getURIQueryParameters = function(uri){
...
thisModule.emptyNode = function(node) {
...
thisModule.getTarget = function(e) {
    var target
...
thisModule.ancestor = function(target, tagName) {
    var level;
...
thisModule.getAbout = function(kb, target) {
    var level, aa
...
thisModule.getTerm = function(target){
    ...
    switch (className){
        ...
        case 'selected': //header TD
            return thisModule.getAbout(tabulator.kb,target); //kb to be changed
        case 'undetermined selected':
            return (target.nextSibling)?st.predicate:((!statementTr.AJAR_inverse)?st.object:st.subject);
    }
}

thisModule.include = function(document,linkstr){
...
thisModule.addLoadEvent = function(func) {
...
//
thisModule.findPos = function(obj) { //C&P from http://www.quirksmode.org/js/findpos.html
...
thisModule.getEyeFocus = function(element,instantly,isBottom,myWindow) {
    if (!myWindow) myWindow=window;
    var elementPosY=thisModule.findPos(element)[1];
...
thisModule.AJARImage = function(src, alt, tt, doc) {
	if(!doc) {
	    doc=document;
    }
...
thisModule.parse_headers = function(headers) {
...
thisModule.shortName = function(uri) {
    var p = uri;
...
//
thisModule.ontologyLabel = function(term) {
...
thisModule.labelWithOntology = function(x, initialCap) {
    var t = tabulator.kb.findTypeURIs(x);
    if (t[tabulator.ns.rdf('Predicate').uri] ||
        t[tabulator.ns.rdfs('Class').uri]) {
        return thisModule.label(x, initialCap) +
            ' (' + thisModule.ontologyLabel(x) + ')';
    }
    return thisModule.label(x, initialCap);
};
...
// @returns string
//
thisModule.label = function(x, initialCap) { // x is an object
    function doCap(s) {
        //s = s.toString();
...
thisModule.escapeForXML = function(str) {
    return str.replace(/&/g, '&amp;').replace(/</g, '&lt;')
}

//  As above but escaped for XML and chopped of contains a slash
thisModule.labelForXML = function(x) {
    return thisModule.escapeForXML(thisModule.label(x));
}

// As above but for predicate, possibly inverse
thisModule.predicateLabelForXML = function(p, inverse) {
    var lab;
	...
	if (ip) return thisModule.labelForXML(ip)
    }

    lab = thisModule.labelForXML(p)
        if (inverse) {
...
thisModule.RDFComparePredicateObject = function(self, other) {
    var x = self.predicate.compareTerm(other.predicate)
    if (x !=0) return x
    return self.object.compareTerm(other.object)
}
thisModule.RDFComparePredicateSubject = function(self, other) {
    var x = self.predicate.compareTerm(other.predicate)
    if (x !=0) return x
...
thisModule.predParentOf = function(node)
{
   	var n=node;
   	while (true)
	{
...
thisModule.makeQueryRow = function(q, tr, constraint) {
    var kb = tabulator.kb
    ...
    for (level=tr.parentNode; level; level=level.parentNode) {
        if (typeof level.AJAR_statement != 'undefined') {   // level.AJAR_statement
            ...
            if (!level.AJAR_variable)
                thisModule.makeQueryRow(q, level);
            parentVar = level.AJAR_variable
            var predLevel = thisModule.predParentOf(level)
    ...
    var predtr = thisModule.predParentOf(tr)
    ///////OPTIONAL KLUDGE///////////
    ...
    v = tr.AJAR_variable? tr.AJAR_variable : kb.variable(thisModule.newVariableName());
    q.vars.push(v)
    v.label = hasParent? parentVar.label : thisModule.label(parentVar);
    v.label += " "+ thisModule.predicateLabelForXML(st.predicate, inverse);
