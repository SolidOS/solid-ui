...
function notepad(dom, padDoc, subject, me, options) {
  ...
  var updater = _store["default"].updater;
  var PAD = (0, _rdflib.Namespace)('http://www.w3.org/ns/pim/pad#');
  ...
  */
  ...
  var setPartStyle = function setPartStyle(part, colors, pending) {
    ...
    var author = kb.any(chunk, _ns["default"].dc('author'));
  ...
  */
  ...
  var addListeners = function addListeners(part, chunk) {
    ...
    var updateStore = function updateStore(part) {
      ...
      var old = kb.any(chunk, _ns["default"].sioc('content')).value;
      var del = [(0, _rdflib.st)(chunk, _ns["default"].sioc('content'), old, padDoc)];
      var ins;

      if (part.value) {
        ins = [(0, _rdflib.st)(chunk, _ns["default"].sioc('content'), part.value, padDoc)];
      }
  ...
  var newPartAfter = function newPartAfter(tr1, chunk, before) {
    ...
    var text = kb.any(chunk, _ns["default"].sioc('content'));
  ...
  */
  ...
  var newChunk = function newChunk(ele, before) {
    ...
    var ins = [(0, _rdflib.st)(prev, PAD('next'), chunk, padDoc), (0, _rdflib.st)(chunk, PAD('next'), next, padDoc), (0, _rdflib.st)(chunk, _ns["default"].dc('author'), me, padDoc), (0, _rdflib.st)(chunk, _ns["default"].sioc('content'), '', padDoc)];
  ...
  var consistencyCheck = function consistencyCheck() {
    ...
    for (;;) {
      ...
      k = kb.each(chunk, _ns["default"].sioc('content')).length;

      if (k !== 1) {
        complain2('Should be 1 not ' + k + ' contents for ' + label);
      }

      k = kb.each(chunk, _ns["default"].dc('author')).length;

      if (k !== 1) {
        complain2('Should be 1 not ' + k + ' author for ' + label);
      }

      var sts = kb.statementsMatching(undefined, _ns["default"].sioc('contents'));
      sts.map(function (st) {
  ...
  var sync = function sync() {
    ...
    for (var _chunk = kb.the(subject, PAD('next')); !_chunk.sameTerm(subject); _chunk = kb.the(_chunk, PAD('next'))) {
      var text = kb.any(_chunk, _ns["default"].sioc('content')).value; // superstitious -- don't mess with unchanged input fields
  ...
  if (exists) {
    ...
    var insertables = [(0, _rdflib.st)(subject, _ns["default"].rdf('type'), PAD('Notepad'), padDoc), (0, _rdflib.st)(subject, _ns["default"].dc('author'), me, padDoc), (0, _rdflib.st)(subject, _ns["default"].dc('created'), new Date(), padDoc), (0, _rdflib.st)(subject, PAD('next'), subject, padDoc)];
...
function notepadToHTML(pad, kb) {
  ...
  var html = '<html>\n  <head>\n';
  var title = kb.anyValue(pad, _ns["default"].dct('title'));
  ...
  chunks.forEach(function (chunk) {
    ...
    var rawContent = kb.anyJS(chunk, _ns["default"].sioc('content'));
