...
aclModule.loadUnionACL = function (subjectList, callback) {
  ...
  var doList = function (list) {
    if (list.length) {
      doc = list.shift().doc()
      UI.widgets.getACLorDefault(doc, function (ok, p2, targetDoc, targetACLDoc, defaultHolder, defaultACLDoc) {
        var defa = !p2
...
aclModule.ACLbyCombination = function (ac) {
  var byCombo = []['agent', 'agentClass', 'origin', 'originClass'].map(function (pred) {
...
aclModule.fixIndividualCardACL = function (person, log, callback) {
  var groups = UI.store.each(undefined, nsModule.vcard('hasMember'), person)
  var doc = person.doc()
  if (groups) {
...
aclModule.fixIndividualACL = function (item, subjects, log, callback) {
  log = log || console.log
  var doc = item.doc()
  UI.widgets.getACLorDefault(doc, function (ok, exists, targetDoc, targetACLDoc, defaultHolder, defaultACLDoc) {
...
aclModule.getACLorDefault = function (doc, callback) {
  UI.widgets.getACL(doc, function (ok, status, aclDoc, message) {
    ...
    var tryParent = function (uri) {
      ...
      UI.widgets.getACL(doc2, function (ok, status, defaultACLDoc) {
        if (!ok) {
