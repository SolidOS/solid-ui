...
widgetModule.personTR = function (dom, pred, obj, options) {
  ...
  if (options.link !== false) {
    ...
    linkImage.setAttribute('src', UI.icons + 'go-to-this.png')
...
widgetModule.allClassURIs = function () {
  var set = {}
  UI.store.statementsMatching(undefined, nsModule.rdf('type'), undefined)
    .map(function (st) { if (st.object.uri) set[st.object.uri] = true })
  UI.store.statementsMatching(undefined, nsModule.rdfs('subClassOf'), undefined)
    .map(function (st) {
      if (st.object.uri) set[st.object.uri] = true
      if (st.subject.uri) set[st.subject.uri] = true
    })
  UI.store.each(undefined, nsModule.rdf('type'), nsModule.rdfs('Class'))
...
widgetModule.fieldLabel = function (dom, property, form) {
  var lab = UI.store.any(form, nsModule.ui('label'))
  if (!lab) lab = UI.utils.label(property, true) // Init capital
...
widgetModule.editFormButton = function (dom, container, form, store, callback) {
  ...
  b.innerHTML = 'Edit ' + UI.utils.label(nsModule.ui('Form'))
...
widgetModule.sortBySequence = function (list) {
  var p2 = list.map(function (p) {
    var k = UI.store.any(p, nsModule.ui('sequence'))
    return [k || 9999, p]
  })
...
tabulator.panes.widget.twoLine['http://www.w3.org/2000/10/swap/pim/qif#Transaction'] = function (dom, x) {
  var failed = ''
  var enc = function (p) {
    var y = UI.store.any(x, nsModule.qu(p))
