...
**
*/
widgetModule.field[nsModule.ui('Form').uri] =
  widgetModule.field[nsModule.ui('Group').uri] = function (
    ...
    if (already[key]) { // been there done that
      ...
      var plist = [$rdf.st(subject, nsModule.owl('sameAs'), subject)] // @@ need prev subject
...
**
*/
widgetModule.field[nsModule.ui('Options').uri] = function (
  dom, container, already, subject, form, store, callback) {
  var kb = tabulator.kb
  var box = dom.createElement('div')
  ...
  var values
  if (dependingOn.sameTerm(nsModule.rdf('type'))) {
...
**
*/
widgetModule.field[nsModule.ui('Multiple').uri] = function (
  dom, container, already, subject, form, store, callback) {
  var kb = tabulator.kb
...
widgetModule.fieldParams[nsModule.ui('ColorField').uri] = {
  'size': 9 }
widgetModule.fieldParams[nsModule.ui('ColorField').uri].pattern =
  /^\s*#[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]([0-9a-f][0-9a-f])?\s*$/

widgetModule.fieldParams[nsModule.ui('DateField').uri] = {
  'size': 20, 'type': 'date', 'dt': 'date'}
widgetModule.fieldParams[nsModule.ui('DateField').uri].pattern =
  /^\s*[0-9][0-9][0-9][0-9](-[0-1]?[0-9]-[0-3]?[0-9])?Z?\s*$/

widgetModule.fieldParams[nsModule.ui('DateTimeField').uri] = {
  'size': 20, 'type': 'date', 'dt': 'dateTime'}
widgetModule.fieldParams[nsModule.ui('DateTimeField').uri].pattern =
  /^\s*[0-9][0-9][0-9][0-9](-[0-1]?[0-9]-[0-3]?[0-9])?(T[0-2][0-9]:[0-5][0-9](:[0-5][0-9])?)?Z?\s*$/

widgetModule.fieldParams[nsModule.ui('TimeField').uri] = {
  'size': 10, 'type': 'time', 'dt': 'time'}
widgetModule.fieldParams[nsModule.ui('TimeField').uri].pattern =
  /^\s*([0-2]?[0-9]:[0-5][0-9](:[0-5][0-9])?)\s*$/

widgetModule.fieldParams[nsModule.ui('IntegerField').uri] = {
  'size': 12, 'style': 'text-align: right', 'dt': 'integer' }
widgetModule.fieldParams[nsModule.ui('IntegerField').uri].pattern =
  /^\s*-?[0-9]+\s*$/

widgetModule.fieldParams[nsModule.ui('DecimalField').uri] = {
  'size': 12, 'style': 'text-align: right', 'dt': 'decimal' }
widgetModule.fieldParams[nsModule.ui('DecimalField').uri].pattern =
  /^\s*-?[0-9]*(\.[0-9]*)?\s*$/

widgetModule.fieldParams[nsModule.ui('FloatField').uri] = {
  'size': 12, 'style': 'text-align: right', 'dt': 'float' }
widgetModule.fieldParams[nsModule.ui('FloatField').uri].pattern =
  /^\s*-?[0-9]*(\.[0-9]*)?((e|E)-?[0-9]*)?\s*$/

widgetModule.fieldParams[nsModule.ui('SingleLineTextField').uri] = { }
widgetModule.fieldParams[nsModule.ui('TextField').uri] = { }

widgetModule.fieldParams[nsModule.ui('PhoneField').uri] = { 'size': 12, 'uriPrefix': 'tel:' }
widgetModule.fieldParams[nsModule.ui('PhoneField').uri].pattern =
  /^\s*\+?[ 0-9-]+[0-9]\s*$/

widgetModule.fieldParams[nsModule.ui('EmailField').uri] = { 'size': 20, 'uriPrefix': 'mailto:' }
widgetModule.fieldParams[nsModule.ui('EmailField').uri].pattern =
  /^\s*.*@.*\..*\s*$/ // @@ Get the right regexp here

widgetModule.field[nsModule.ui('PhoneField').uri] =
  widgetModule.field[nsModule.ui('EmailField').uri] =
    widgetModule.field[nsModule.ui('ColorField').uri] =
      widgetModule.field[nsModule.ui('DateField').uri] =
        widgetModule.field[nsModule.ui('DateTimeField').uri] =
          widgetModule.field[nsModule.ui('TimeField').uri] =
            widgetModule.field[nsModule.ui('NumericField').uri] =
              widgetModule.field[nsModule.ui('IntegerField').uri] =
                widgetModule.field[nsModule.ui('DecimalField').uri] =
                  widgetModule.field[nsModule.ui('FloatField').uri] =
                    widgetModule.field[nsModule.ui('TextField').uri] =
                      widgetModule.field[nsModule.ui('SingleLineTextField').uri] = function (
                        dom, container, already, subject, form, store, callback) {
                        var ui = nsModule.ui
                        var kb = tabulator.kb
...
widgetModule.field[nsModule.ui('MultiLineTextField').uri] = function (
  dom, container, already, subject, form, store, callback) {
  var ui = nsModule.ui
  var kb = tabulator.kb
  var property = kb.any(form, ui('property'))
...
widgetModule.field[nsModule.ui('BooleanField').uri] = function (
  dom, container, already, subject, form, store, callback) {
  var ui = nsModule.ui
  var kb = tabulator.kb
  var property = kb.any(form, ui('property'))
...
widgetModule.field[nsModule.ui('Classifier').uri] = function (
  dom, container, already, subject, form, store, callback) {
  var kb = tabulator.kb
  var ui = nsModule.ui
...
widgetModule.field[nsModule.ui('Choice').uri] = function (
  dom, container, already, subject, form, store, callback) {
  var ui = nsModule.ui
  var ns = nsModule
  var kb = tabulator.kb
  ...
  // tabulator.log.debug("%%% Choice field: possible.length 2 = "+possible.length)
    ...
    widgetModule.propertyTriage()
    for (p in tabulator.properties.op) possible.push(kb.fromNT(p))
    for (p in tabulator.properties.dp) possible.push(kb.fromNT(p))
...
widgetModule.fieldParams[nsModule.ui('Comment').uri] = {
  'element': 'p',
  'style': 'padding: 0.1em 1.5em; color: brown; white-space: pre-wrap;'}
widgetModule.fieldParams[nsModule.ui('Heading').uri] = {
  'element': 'h3', 'style': 'font-size: 110%; color: brown;' }

widgetModule.field[nsModule.ui('Comment').uri] =
  widgetModule.field[nsModule.ui('Heading').uri] = function (
    dom, container, already, subject, form, store, callback) {
    var ui = nsModule.ui
    var kb = tabulator.kb
...
widgetModule.allClassURIs = function () {
  var set = {}
  tabulator.kb.statementsMatching(undefined, nsModule.rdf('type'), undefined)
    .map(function (st) { if (st.object.uri) set[st.object.uri] = true })
  tabulator.kb.statementsMatching(undefined, nsModule.rdfs('subClassOf'), undefined)
    .map(function (st) {
      if (st.object.uri) set[st.object.uri] = true
      if (st.subject.uri) set[st.subject.uri] = true
    })
  tabulator.kb.each(undefined, nsModule.rdf('type'), nsModule.rdfs('Class'))
...
widgetModule.propertyTriage = function () {
  ...
  var ps = kb.each(undefined, nsModule.rdf('type'), nsModule.rdf('Property'))
...
widgetModule.fieldLabel = function (dom, property, form) {
  var lab = tabulator.kb.any(form, nsModule.ui('label'))
  if (!lab) lab = tabulator.Util.label(property, true) // Init capital
...
widgetModule.editFormButton = function (dom, container, form, store, callback) {
  ...
  b.innerHTML = 'Edit ' + tabulator.Util.label(nsModule.ui('Form'))
  b.addEventListener('click', function (e) {
    var ff = widgetModule.appendForm(dom, container,
      {}, form, nsModule.ui('FormForm'), store, callback)
    ff.setAttribute('style', nsModule.ui('FormForm').sameTerm(form)
...
widgetModule.findClosest = function findClosest (kb, cla, prop) {
  ...
  while (agenda.length > 0) {
    ...
    var supers = kb.each(c, nsModule.rdfs('subClassOf'))
...
widgetModule.formsFor = function (subject) {
  ...
  var t1
  for (t1 in t) { tabulator.log.debug('   type: ' + t1) }
...
widgetModule.sortBySequence = function (list) {
  var p2 = list.map(function (p) {
    var k = tabulator.kb.any(p, nsModule.ui('sequence'))
    return [k || 9999, p]
  })
...
widgetModule.promptForNew = function (dom, kb, subject, predicate, theClass, form, store, callback) {
  ...
  box.innerHTML = '<h3>New ' + tabulator.Util.label(theClass) + '</h3>'
  ...
  var itemDone = function (ok, body) {
    ...
    if (insertMe.length) {
      tabulator.sparql.update([], insertMe, linkDone)
    } else {
      callback(true, body)
    }
    if (!gotButton) {
...
widgetModule.makeSelectForOptions = function (dom, kb, subject, predicate,
  ...
  var getActual = function () {
    actual = {}
    if (predicate.sameTerm(nsModule.rdf('type'))) {
...
widgetModule.makeSelectForCategory = function (dom, kb, subject, category, store, callback) {
  ...
  var du = kb.any(category, nsModule.owl('disjointUnionOf'))
  var subs
  var multiple = false
  if (!du) {
    subs = kb.each(undefined, nsModule.rdfs('subClassOf'), category)
    multiple = true
  } else {
    subs = du.elements
  }
  ...
  return widgetModule.makeSelectForOptions(dom, kb, subject, nsModule.rdf('type'), subs,
...
widgetModule.makeSelectForNestedCategory = function (
  ...
  var update = function () {
    ...
    if (select.currentURI && kb.any(kb.sym(select.currentURI), nsModule.owl('disjointUnionOf'))) {
...
widgetModule.buildCheckboxForm = function (dom, kb, lab, del, ins, form, store) {
  ...
  if (del) {
    ...
    if (!state && !negation) {
      state = !!kb.any(form, nsModule.ui('default'))
    }
  }
...
tabulator.panes.widget.twoLine['http://www.w3.org/2000/10/swap/pim/qif#Transaction'] = function (dom, x) {
  var failed = ''
  var enc = function (p) {
    var y = tabulator.kb.any(x, nsModule.qu(p))
...
tabulator.panes.widget.twoLine['http://www.w3.org/ns/pim/trip#Trip'] = function (dom, x) {
  var enc = function (p) {
    var y = tabulator.kb.any(x, p)
    return y ? tabulator.Util.escapeForXML(y.value) : '?'
  }
  var box = dom.createElement('table')
  box.innerHTML = '<tr><td colspan="2">' + enc(nsModule.dc('title')) +
    '</td></tr>\n<tr style="color: #777"><td><td>' +
    enc(nsModule.cal('dtstart')) + '</td><td>' + enc(nsModule.cal('dtend')) +
    '</td></tr>'
  return box
}
