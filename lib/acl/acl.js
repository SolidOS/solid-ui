...
function fixIndividualCardACL(person, log, callbackFunction) {
  var groups = kb.each(undefined, _ns["default"].vcard('hasMember'), person); // const doc = person.doc()
...
function setACL(docURI, aclText, callbackFunction) {
  var aclDoc = kb.any(kb.sym(docURI), kb.sym(_solidLogic.ACL_LINK)); // @@ check that this get set by web.js
...
function getACL(doc, callbackFunction) {
  kb.fetcher.nowOrWhenFetched(doc, undefined, function (ok, body) {
    ...
    var aclDoc = kb.any(doc, kb.sym(_solidLogic.ACL_LINK)); // @@ check that this get set by web.js
