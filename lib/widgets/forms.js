...
forms.field[ns.ui('Form').uri] = forms.field[ns.ui('Group').uri] = function (dom, container, already, subject, form, store, callbackFunction) {
  ...
  for (var i = 0; i < p2.length; i++) {
    ...
    if (t === ui('Options').uri) {
      var dep = kb.any(_field, ui('dependingOn'));
      if (dep && kb.any(subject, dep)) original[i] = kb.any(subject, dep).toNT();
    }

    var fn = (0, _fieldFunction.fieldFunction)(dom, _field);
    ...
    var itemChanged = function itemChanged(ok, body) {
      if (ok) {
        for (var j = 0; j < p2.length; j++) {
          ...
          var _t = (0, _fieldFunction.mostSpecificClassURI)(_field2); // Field type


          if (_t === ui('Options').uri) {
            var _dep = kb.any(_field2, ui('dependingOn'));

            var newOne = fn(dom, box, already, subject, _field2, store, callbackFunction);
    ...
    eles.push(fn(dom, box, already2, subject, _field, store, itemChanged));
  }
    ...
    for (var j = 0; j < tests.length; j++) {
      if (values[tests[j].uri]) {
        ...
        if (!_field3) {
          ...
          forms.appendForm(dom, box, already, subject, _field3, store, callbackFunction);
        }
...
forms.field[ns.ui('Multiple').uri] = function (dom, container, already, subject, form, store, callbackFunction) {
  ...
  *
  ...
  var reverse = kb.anyJS(form, ui('reverse'));
  ...
  function refresh() {
    ...
    if (ordered) {
      var li = reverse ? kb.the(null, property, subject) : kb.the(subject, property);
      vals = li ? li.elements : [];
    } else {
