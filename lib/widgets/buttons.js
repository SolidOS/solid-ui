...
function allClassURIs() {
  var set = {};
  store.statementsMatching(undefined, _ns["default"].rdf('type'), undefined).map(function (st) {
    if (st.object.uri) set[st.object.uri] = true;
  });
  store.statementsMatching(undefined, _ns["default"].rdfs('subClassOf'), undefined).map(function (st) {
    if (st.object.uri) set[st.object.uri] = true;
    if (st.subject.uri) set[st.subject.uri] = true;
  });
  store.each(undefined, _ns["default"].rdf('type'), _ns["default"].rdfs('Class')).map(function (c) {
