...
exports.index = exports.iconForClass = void 0;
...
function trySetImage(element, thing, iconForClassMap) {
  ...
  if (typeIcon) {
    ...
    element.style = _style["default"].classIconStyle; // element.style.border = '0.1em solid green;'
...
function faviconOrDefault(dom, x) {
  ...
  image.style = _style["default"].iconStyle;
...
function deleteButtonWithCheck(dom, container, noun, deleteFunction) {
  ...
  deleteButtonElt.addEventListener('click', function (_event) {
    ...
    cancelButtonElt.setAttribute('style', _style["default"].buttonStyle);
    var img = cancelButtonElt.appendChild(dom.createElement('img'));
    img.setAttribute('src', cancelIconURI);
    img.setAttribute('style', _style["default"].buttonStyle);
    ...
    sureButtonElt.setAttribute('style', _style["default"].buttonStyle);
...
function button(dom, iconURI, text, handler) {
  ...
  button.setAttribute('style', _style["default"].buttonStyle); // button.innerHTML = text  // later, user preferences may make text preferred for some
...
function askName(dom, kb, container, predicate, theClass, noun) {
  ...
  return new Promise(function (resolve, _reject) {
    ...
    namefield.setAttribute('style', _style["default"].textInputStyle);
...
function openHrefInOutlineMode(e) {
  ...
  if (dom.outlineManager) {
    ...
    ;
    dom.outlineManager.GotoSubject(_store["default"].sym(uri), true, undefined, true, undefined);
  } else if (window && window.panes && window.panes.getOutliner) {
    // @@ TODO Remove the use of window as a global object
    ;
    window.panes.getOutliner().GotoSubject(_store["default"].sym(uri), true, undefined, true, undefined);
  } else {
...
function defaultAnnotationStore(subject) {
  ...
  return _store["default"].sym('http://tabulator.org/wiki/annnotation/' + s);
}
/**
...
function allClassURIs() {
  ...
  _store["default"].statementsMatching(undefined, _ns["default"].rdf('type'), undefined).map(function (st) {
    if (st.object.uri) set[st.object.uri] = true;
  });

  _store["default"].statementsMatching(undefined, _ns["default"].rdfs('subClassOf'), undefined).map(function (st) {
    if (st.object.uri) set[st.object.uri] = true;
    if (st.subject.uri) set[st.subject.uri] = true;
  });

  _store["default"].each(undefined, _ns["default"].rdf('type'), _ns["default"].rdfs('Class')).map(function (c) {
...
function propertyTriage(kb) {
  ...
  for (var i = 0; i < ps.length; i++) {
    p = ps[i].toNT(); // log.debug('propertyTriage: unknown: ' + p)
...
function twoLineTransaction(dom, x) {
  ...
  var enc = function enc(p) {
    var y = _store["default"].any(x, _ns["default"].qu(p));
...
function isImage(file, kind) {
  ...
  var typeURIs = _store["default"].findTypeURIs(file); // See https://github.com/linkeddata/rdflib.js/blob/d5000f/src/utils-js.js#L14
