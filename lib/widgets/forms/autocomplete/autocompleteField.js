...
function autocompleteField(dom, container, already, subject, form, doc, callbackFunction) {
      ...
      return _regenerator["default"].wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 8:
              deletables = oldValue ? kb.statementsMatching(subject, property, oldValue, doc).concat(kb.statementsMatching(oldValue, labelProperty, null, doc)) : [];
              console.log('autocompleteField Deletables ' + deletables.map(function (st) {
                return st.toNT();
              }));
              ...
              console.log("AC form: ".concat(deletables.length, " to delete and ").concat(insertables.length, " to insert"));
              _context.prev = 12;
              _context.next = 15;
            ...
            case 17:
              ...
              box.appendChild(widgets.errorMessageBlock(dom, 'Autocomplete form data update error:' + _context.t0));
              return _context.abrupt("return");
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            ...
            case 5:
              ...
              deletables = kb.statementsMatching(subject, property, oldValue, doc).concat(kb.statementsMatching(oldValue, labelProperty, null, doc));
              console.log('autocompleteField Deletables ' + deletables.map(function (st) {
                return st.toNT();
              }));
              insertables = [];
              console.log("AC form delete: ".concat(deletables.length, " to delete and ").concat(insertables.length, " to insert"));
              _context2.prev = 9;
              _context2.next = 12;
              return kb.updater.updateMany(deletables, insertables);
  ...
  if (!(subject instanceof _rdflib.NamedNode)) {
  ...
  */
  ...
  (0, _autocompleteBar.renderAutocompleteControl)(dom, subject, barOptions, autocompleteOptions, addOneIdAndRefresh, deleteOne).then(function (control) {
    ...
    rhs.appendChild(widgets.errorMessageBlock(dom, "Error rendering autocomplete".concat(form, ": ").concat(err)));
  });
  return box;
} // ends
