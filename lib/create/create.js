...
function newThingUI(createContext, dataBrowserContext, thePanes) {
  ...
  star.setAttribute('src', _iconBase["default"].iconBase + 'noun_34653_green.svg');
  ...
  function makeNewAppInstance(options) {
    return new Promise(function (resolve, reject) {
      ...
      function callbackWS(ws, newBase) {
        (0, _authn.logInLoadProfile)(createContext).then(function (_context) {
          ...
          options.pane.mintNew(dataBrowserContext, newPaneOptions).then(function (newPaneOptions) {
            ...
            if (newPaneOptions.folder) {
              ...
              if (isPackage) {
                kb.add(newPaneOptions.folder, _ns["default"].ldp('contains'), kb.sym(newPaneOptions.newBase), newPaneOptions.folder.doc());
              } else {
                // single file
                kb.add(newPaneOptions.folder, _ns["default"].ldp('contains'), newPaneOptions.newInstance, newPaneOptions.folder.doc()); // Ping the patch system?
      ...
      if (!options.folder) {
        ...
        _widgets["default"].askName(dom, kb, options.div, _ns["default"].foaf('name'), null, options.noun).then(gotName); // selectUI = getNameForm(dom, kb, options.noun, gotName)
  ...
  mintingPanes.forEach(function (pane) {
    ...
    var noun = pane.mintClass ? mintingClassMap[pane.mintClass.uri] > 1 ? "".concat(_utils["default"].label(pane.mintClass), " (using ").concat(pane.name, " pane)") : _utils["default"].label(pane.mintClass) : pane.name + ' @@';
